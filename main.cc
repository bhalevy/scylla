...
To start the scylla server proper, simply invoke as: scylla server (or just scylla).
    ...
    std::optional<utils::disk_space_monitor<lowres_clock>> dsm;
    ...
    return app.run(ac, av, [&] () -> future<int> {
        ...
        return seastar::async([&app, cfg, ext, &dsm, &cm, &sstm, &db, &qp, &bm, &proxy, &mapreduce_service, &mm, &mm_notifier, &ctx, &opts, &dirs,
          ...
          try {
            ...
            auto dsm_cfg = utils::disk_space_monitor<lowres_clock>::config{
                ...
                .normal_polling_interval = lowres_clock::duration(std::chrono::seconds(cfg->disk_space_monitor_normal_polling_interval_in_seconds())),
                .high_polling_interval = lowres_clock::duration(std::chrono::seconds(cfg->disk_space_monitor_high_polling_interval_in_seconds())),
                .polling_interval_threshold = cfg->disk_space_monitor_polling_interval_threshold(),
            };
            dsm.emplace(stop_signal.as_local_abort_source(), dsm_cfg);
            fs::path data_dir;
            if (data_dir_set.get_paths().size() == 0) {
                throw std::runtime_error("data_dir must be non-empty");
            } else {
                data_dir = *data_dir_set.get_paths().begin();
                if (data_dir_set.get_paths().size() > 1) {
                    startlog.warn("Multiple data directories aren't supported. Will monitor only {}", data_dir);
                }
            }
            dsm->start(data_dir);
            auto stop_dsm = defer_verbose_shutdown("disk space monitor", [&dsm] {
                dsm->stop().get();
            });
