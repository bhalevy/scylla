...
node_ops_metrics::node_ops_metrics(std::unique_ptr<tracker>& tracker)
    : _tracker(tracker)
{
...
void repair_info::check_in_shutdown() {
    repair.repair_tracker().check_in_shutdown();
}
...
repair_info::repair_info(repair_service& repair_,
...
static future<> do_repair_ranges(lw_shared_ptr<repair_info> ri) {
    ...
    return parallel_for_each(ri->ranges, [ri] (auto&& range) {
        return with_semaphore(ri->repair.repair_tracker().range_parallelism_semaphore(), 1, [ri, &range] {
            return ri->repair_range(range).then([ri] {
                if (ri->reason == streaming::stream_reason::bootstrap) {
                    ri->repair.get_metrics().bootstrap_finished_ranges++;
                } else if (ri->reason == streaming::stream_reason::replace) {
                    ri->repair.get_metrics().replace_finished_ranges++;
                } else if (ri->reason == streaming::stream_reason::rebuild) {
                    ri->repair.get_metrics().rebuild_finished_ranges++;
                } else if (ri->reason == streaming::stream_reason::decommission) {
                    ri->repair.get_metrics().decommission_finished_ranges++;
                } else if (ri->reason == streaming::stream_reason::removenode) {
                    ri->repair.get_metrics().removenode_finished_ranges++;
                } else if (ri->reason == streaming::stream_reason::repair) {
                    ri->repair.get_metrics().repair_finished_ranges_sum++;
...
static future<> repair_ranges(lw_shared_ptr<repair_info> ri) {
    ri->repair.repair_tracker().add_repair_info(ri->id.id, ri);
    return do_repair_ranges(ri).then([ri] {
        ri->check_failed_ranges();
        ri->repair.repair_tracker().remove_repair_info(ri->id.id);
        return make_ready_future<>();
...
void repair_service::abort_repair_node_ops(utils::UUID ops_uuid) {
    assert(this_shard_id() == 0);
    repair_tracker().abort_repair_node_ops(ops_uuid);
}
