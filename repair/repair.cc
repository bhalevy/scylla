    ...
    return seastar::async([this, tmptr = std::move(tmptr), source_dc = std::move(source_dc), op = std::move(op), reason, ignore_nodes = std::move(ignore_nodes)] () mutable {
        ...
        if (source_dc) {
            topology.for_each_node([&] (const locator::node* node) {
                ...
                // or it is an empty node which runs rebuild.
                ...
                case locator::replication_strategy_type::network_topology: {
                    ...
                    if (possibly_lost_replicas < rf / 2) {
                        ...
                        if (rf == 0) {
                            ...
                            auto error_msg = format("It is unsafe to rebuild keyspace={} using source_dc={}: Possibly lost {} replicas out of {}.",
                                    keyspace_name, source_dc_desc, possibly_lost_replicas, rf);
                        ...
                        auto error_msg = format("It is unsafe to repair keyspace={} using source_dc={}: Possibly lost {} replicas out of {}.",
                                keyspace_name, source_dc_desc, possibly_lost_replicas, rf);
                        rlogger.warn("{}. {}", error_msg, fallback_msg);
                        source_dc.reset();
                    }
