        ...
        rs.container().invoke_on_all([&idx, id, metas = _metas, parent_data, reason = _reason, tables = _tables] (repair_service& rs) -> future<> {
            ...
            for (auto& m : metas) {
                ...
                if (res.failed()) {
                    ...
                    sstring dropped = co_await repair::table_sync_and_check(rs.get_db().local(), rs.get_migration_manager(), m.tid) ?
                    ...
                    rlogger.warn("repair[{}]: Repair tablet for table={}.{} range={} status=failed: {}{}",
                            id.uuid(), m.keyspace_name, m.table_name, m.range,
                            dropped.empty() ? "" : format("{} does not exist any more, ignoring it, ", dropped), ep);
                    if (dropped.empty()) {
                        error = std::move(ep);
                    }
                }
