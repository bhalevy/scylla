...
std::vector<sstables::shared_sstable> compaction_manager::get_candidates(const column_family& cf, std::optional<std::vector<sstables::shared_sstable>> sstables_opt) {
    ...
    auto sstables = sstables_opt.value_or(cf.in_strategy_sstables());
...
bool compaction_backlog_tracker::sstable_belongs_to_tracker(const sstables::shared_sstable& sst) {
    return sst->is_eligable_for_compaction();
}
