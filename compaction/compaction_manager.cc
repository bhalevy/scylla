...
std::vector<sstables::shared_sstable> compaction_manager::get_candidates(const replica::table& t) {
    ...
    auto partial_run_identifiers = boost::copy_range<std::unordered_set<utils::UUID>>(get_compaction_state(&t).tasks
...
compaction_manager::compaction_state& compaction_manager::get_compaction_state(const replica::table* t) {
    try {
        return _compaction_state.at(const_cast<replica::table*>(t));
...
future<> compaction_manager::stop_ongoing_compactions(sstring reason, replica::table* t, std::optional<sstables::compaction_type> type_opt) {
    ...
    auto tasks = boost::copy_range<std::vector<shared_ptr<task>>>(get_compaction_state(t).tasks | boost::adaptors::filtered([type_opt] (auto& task) {
        return !type_opt || task.type() == *type_opt;
...
future<> compaction_manager::perform_cleanup(replica::database& db, replica::table* t) {
    auto check_for_cleanup = [this, t] {
        return boost::algorithm::any_of(get_compaction_state(t).tasks, [] (auto& task) {
            return task.type() == sstables::compaction_type::Cleanup;
        });
    };
...
const std::vector<sstables::compaction_info> compaction_manager::get_compactions(replica::table* t) const {
    ...
    return boost::copy_range<ret>(get_compaction_state(t).tasks | boost::adaptors::filtered([] (const task& task) {
...
void compaction_manager::propagate_replacement(replica::table* t,
    ...
    for (auto& task : get_compaction_state(t).tasks) {
        if (task.compaction_running()) {
