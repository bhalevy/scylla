...
future<sstables::compaction_result> compaction_manager::task::compact_sstables(sstables::compaction_descriptor descriptor, sstables::compaction_data& cdata, release_exhausted_func_t release_exhausted, can_purge_tombstones can_purge) {
    ...
    descriptor.retrieve_owned_ranges_if_required(_cm.get_compaction_state(_compacting_table));
...
public:
    rewrite_sstables_compaction_task(compaction_manager& mgr, compaction::table_state* t, sstables::compaction_type_options options, compaction::owned_ranges_ptr owned_ranges_ptr,
...
future<compaction_manager::compaction_stats_opt> compaction_manager::perform_task_on_all_files(compaction::table_state& t, sstables::compaction_type_options options, compaction::owned_ranges_ptr owned_ranges_ptr, get_candidates_func get_func, Args... args) {
    if (_state != state::enabled) {
        co_return std::nullopt;
    }
...
future<compaction_manager::compaction_stats_opt> compaction_manager::rewrite_sstables(compaction::table_state& t, sstables::compaction_type_options options, compaction::owned_ranges_ptr owned_ranges_ptr, get_candidates_func get_func, can_purge_tombstones can_purge) {
...
public:
    cleanup_sstables_compaction_task(compaction_manager& mgr, compaction::table_state* t, sstables::compaction_type_options options, compaction::owned_ranges_ptr owned_ranges_ptr,
...
bool compaction_manager::update_sstable_cleanup_state(compaction::table_state& t, const sstables::shared_sstable& sst, const dht::token_range_vector& sorted_owned_ranges) {
...
future<> compaction_manager::perform_cleanup(owned_ranges_ptr sorted_owned_ranges, compaction::table_state& t) {
    ...
    auto get_sstables = [this, &t, sorted_owned_ranges] () -> future<std::vector<sstables::shared_sstable>> {
        return seastar::async([this, &t, sorted_owned_ranges = std::move(sorted_owned_ranges)] {
            auto filter_candidates = [&] (const sstables::sstable_set& set) {
                set.for_each_sstable([&] (const sstables::shared_sstable& sst) {
                    update_sstable_cleanup_state(t, sst, *sorted_owned_ranges);
                    seastar::thread::maybe_yield();
                });
            };
            filter_candidates(t.main_sstable_set());
            filter_candidates(t.maintenance_sstable_set());
