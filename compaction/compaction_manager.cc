...
future<> compaction_manager::maybe_wait_for_sstable_count_reduction(replica::table* t) {
    ...
    auto num_runs_for_compaction = [&, this] {
        // make a new compaction_strategy object so that
        // any side effects of get_sstables_for_compaction (like in TWCS)
        // will not be applied on the table's comapction_strategy instance.
        auto cs = make_compaction_strategy(schema->compaction_strategy(), schema->compaction_strategy_options());
