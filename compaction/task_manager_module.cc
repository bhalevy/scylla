...
future<> table_reshaping_compaction_task_impl::run() {
    ...
    auto total_size = co_await _dir.map_reduce0([&] (sstables::sstable_directory& d) -> future<uint64_t> {
        ...
        auto task = co_await compaction_module.make_and_start_task<shard_reshaping_compaction_task_impl>(parent_info, _status.keyspace, _status.table, _status.id, d, _db, _mode, _creator, _filter, total_shard_size, _skip_orphaned_sstables);
        co_await task->done();
...
future<> shard_reshaping_compaction_task_impl::run() {
    ...
    for (auto& sstable : _dir.get_unshared_local_sstables()) {
        compaction_group_view* t = _skip_orphaned_sstables ?
                co_await table.maybe_compaction_group_view_for_sstable(sstable) :
                &table.compaction_group_view_for_sstable(sstable);
        if (t) {
