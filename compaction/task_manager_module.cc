...
collect_all_shared_sstables(sharded<sstables::sstable_directory>& dir, sharded<replica::database>& db, sstring ks_name, sstring table_name, owned_ranges_ptr owned_ranges_ptr) {
...
future<> reshard(sstables::sstable_directory& dir, sstables::sstable_directory::sstable_open_info_vector shared_info, replica::table& table,
                           compaction::compaction_sstable_creator_fn creator, owned_ranges_ptr owned_ranges_ptr, tasks::task_info parent_info)
{
...
future<> table_resharding_compaction_task_impl::run() {
    ...
    co_await _db.invoke_on_all(coroutine::lambda([&] (replica::database& db) -> future<> {
        ...
        owned_ranges_ptr local_owned_ranges_ptr;
