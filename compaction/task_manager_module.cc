...
future<> run_on_keyspace(sstring op, replica::database& db, std::string keyspace, std::function<future<> (replica::keyspace&)> func) {
    std::exception_ptr ex;
    tasks::tmlogger.debug("Starting {} on keyspace {}", op, keyspace);
    try {
        co_await func(db.find_keyspace(keyspace));
    } catch (const replica::no_such_keyspace& e) {
        tasks::tmlogger.warn("Skipping {} of keyspace {}: {}", op, keyspace, e.what());
    } catch (...) {
        ex = std::current_exception();
        tasks::tmlogger.error("Failed {} of keyspace {}: {}", op, keyspace, ex);
    }
    if (ex) {
        co_await coroutine::return_exception_ptr(std::move(ex));
    }
...
// Run on all tables, skipping dropped tables
future<> run_on_existing_keyspacess(sstring op, replica::database& db, std::vector<std::string> keyspaces, std::function<future<> (replica::keyspace&)> func) {
    for (const auto& ks : keyspaces) {
        co_await run_on_keyspace(op, db, ks, func);
    }
}
...
sstring major_compaction_task_impl::to_string(flush_mode fm) {
    switch (fm) {
    case flush_mode::unset: return "unset";
...
future<> global_major_compaction_task_impl::run() {
    ...
    for (const auto& [table_id, t] : tables_meta) {
        ...
        auto it = tables_by_keyspace.try_emplace(ks_name, std::vector<table_info>()).first;
        it->second.emplace_back(table_name, table_id);
    }
...
future<> table_major_keyspace_compaction_task_impl::run() {
    ...
    replica::table::do_flush flush(_flush_mode == flush_mode::compacted_tables);
    co_await run_on_table("force_keyspace_compaction", _db, _status.keyspace, _ti, [info, flush] (replica::table& t) {
        // All tables were already flushed in shard_major_keyspace_compaction_task_impl
        return t.compact_all_sstables(info, flush);
    });
}
