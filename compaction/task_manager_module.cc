...
future<> table_cleanup_keyspace_compaction_task_impl::run() {
    co_await wait_for_your_turn(_cv, _current_task, _status.id);
    auto owned_ranges_ptr = compaction::make_owned_ranges_ptr(co_await _db.get_keyspace_local_ranges(_status.keyspace));
...
future<> table_upgrade_sstables_compaction_task_impl::run() {
    co_await wait_for_your_turn(_cv, _current_task, _status.id);
    auto owned_ranges = co_await _db.maybe_get_keyspace_local_ranges(_status.keyspace);
