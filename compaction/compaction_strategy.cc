...
bool compaction_strategy_impl::worth_dropping_tombstones(const shared_sstable& sst, gc_clock::time_point compaction_time) {
    ...
    auto gc_before = sst->get_gc_before_for_drop_estimation(get_compaction_manager(), compaction_time);
...
const utils::optional_reference<const compaction_manager>& compaction_strategy::get_compaction_manager() const noexcept {
    return _compaction_strategy_impl->get_compaction_manager();
}
