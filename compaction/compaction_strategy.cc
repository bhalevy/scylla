...
void size_tiered_backlog_tracker::replace_sstables(const std::vector<sstables::shared_sstable>& old_ssts, const std::vector<sstables::shared_sstable>& new_ssts) {
    ...
    all.reserve(all.size() + new_ssts.size());

    for (auto& sst : old_ssts) {
        if (sst->data_size() > 0) {
            auto erased = all.erase(sst);
            if (erased) {
    ...
    for (auto& sst : new_ssts) {
        if (sst->data_size() > 0) {
            auto [_, inserted] = all.insert(sst);
            if (inserted) {
                total_bytes += sst->data_size();
            }
        }
...
public:
    ...
    // Provides strong exception safety guarantees
    virtual void replace_sstables(const std::vector<sstables::shared_sstable>& old_ssts, const std::vector<sstables::shared_sstable>& new_ssts) override {
        std::vector<uint64_t> size_per_level = _size_per_level;
        auto update_size_per_level = [&] (size_t level, int64_t data_size) {
            if (level >= size_per_level.size()) {
                size_per_level.resize(level+1, 0);
            }
            size_per_level[level] += data_size;
        };

        std::vector<sstables::shared_sstable> l0_old_ssts, l0_new_ssts;
        for (auto& sst : new_ssts) {
            auto level = sst->get_sstable_level();
            update_size_per_level(level, sst->data_size());
            if (level == 0) {
        ...
        std::invoke([&] () noexcept {
            _size_per_level = std::move(size_per_level);
        });
    }
};
