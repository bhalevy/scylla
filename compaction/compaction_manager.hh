...
public:
    ...
    const auto& compaction_sg() const noexcept {
        return _cfg.compaction_sched_group;
    }

    const auto& maintenance_sg() const noexcept {
        return _cfg.maintenance_sched_group;
    }

    auto available_memory() const noexcept {
        return _cfg.available_memory;
    }

    auto static_shares() const noexcept {
        return _cfg.static_shares.get();
    }

    auto throughput_mbs() const noexcept {
