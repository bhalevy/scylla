...
private:
    ...
    future<> consume_without_gc_writer(gc_clock::time_point compaction_time) {
        ...
        auto cm_opt = _table_s.get_compaction_strategy().get_compaction_manager_opt();
        return consumer(make_compacting_reader(make_sstable_reader(), compaction_time, max_purgeable_func(), cm_opt));
    }
...
get_fully_expired_sstables(const compaction_manager& cm, const table_state& table_s, const std::vector<sstables::shared_sstable>& compacting, gc_clock::time_point compaction_time) {
    ...
    for (auto& sstable : overlapping) {
        auto gc_before = sstable->get_gc_before_for_fully_expire(&cm, compaction_time);
    ...
    for (auto& candidate : compacting) {
        auto gc_before = candidate->get_gc_before_for_fully_expire(&cm, compaction_time);
