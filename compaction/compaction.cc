...
static api::timestamp_type get_max_purgeable_timestamp(const table_state& table_s, sstable_set::incremental_selector& selector,
    ...
    for (auto&& sst : boost::range::join(selector.select(dk).sstables, table_s.compacted_undeleted_sstables())) {
        ...
        if (!ts_stats.empty()) {
            auto maybe_set_min_timestamp = [&] (sstables::ext_timestamp_stats_type t) {
                if (auto it = ts_stats.find(t); it != ts_stats.end()) {
            ...
            if (is_shadowable) {
                if (!maybe_set_min_timestamp(sstables::ext_timestamp_stats_type::min_live_row_marker_timestamp)) {
                    maybe_set_min_timestamp(sstables::ext_timestamp_stats_type::min_live_timestamp);
                }
            } else {
                maybe_set_min_timestamp(sstables::ext_timestamp_stats_type::min_live_timestamp);
            }
