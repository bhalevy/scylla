...
def test_storage_service_keyspace_cleanup_with_no_owned_ranges(cql, this_dc, rest_api):
    with new_test_keyspace(cql, f"WITH REPLICATION = {{ 'class' : 'NetworkTopologyStrategy', '{this_dc}' : 1 }}") as keyspace:
        ...
        with new_test_table(cql, keyspace, schema) as t0:
            ...
            resp = rest_api.send("POST", f"storage_service/keyspace_flush/{keyspace}")
            resp.raise_for_status()
            resp = rest_api.send("POST", "storage_service/snapshots", {'kn': keyspace, 'tag': '0-after-flush'})
            resp.raise_for_status()

            cql.execute(f"ALTER KEYSPACE {keyspace} WITH REPLICATION = {{ 'class' : 'NetworkTopologyStrategy', '{this_dc}' : 0 }}")
            resp = rest_api.send("POST", "storage_service/snapshots", {'kn': keyspace, 'tag': '1-after-alter-keyspace'})
            resp.raise_for_status()

            resp = rest_api.send("POST", f"storage_service/keyspace_cleanup/{keyspace}")
            resp.raise_for_status()
            resp = rest_api.send("POST", "storage_service/snapshots", {'kn': keyspace, 'tag': '2-after-cleanup'})
            resp.raise_for_status()
            ...
            assert snapshots['0-after-flush']['total'] > 0, f"snapshots after flush should have non-zero data: {snapshots}"
            assert snapshots['1-after-alter-keyspace']['total'] == snapshots['0-after-flush']['total'], f"snapshots after alter-keyspace should have the same data as after flush: {snapshots}"
            assert snapshots['2-after-cleanup']['total'] == 0, f"snapshots after clean should have no data: {snapshots}"
