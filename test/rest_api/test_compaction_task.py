...
def test_global_major_keyspace_compaction_task(cql, this_dc, rest_api):
    ...
    # global major compaction, skip flush option
    check_compaction_task(cql, this_dc, rest_api, lambda _, __: rest_api.send("POST", f"storage_service/compact?sf=true"), "major compaction", task_tree_depth, allow_no_children=True)
...
def test_major_keyspace_compaction_task(cql, this_dc, rest_api):
    ...
    # keyspace major compaction, skip flush option
    check_compaction_task(cql, this_dc, rest_api, lambda keyspace, _: rest_api.send("POST", f"storage_service/keyspace_compaction/{keyspace}?sf=true"), "major compaction", task_tree_depth)
    # column family major compaction, skip flush option
    check_compaction_task(cql, this_dc, rest_api, lambda keyspace, table: rest_api.send("POST", f"column_family/major_compaction/{keyspace}:{table}?sf=true"), "major compaction", task_tree_depth)
