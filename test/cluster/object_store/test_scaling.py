...
async def test_scaling(manager: ManagerClient, object_storage):
    ...
    async def verify_object_storage_namespace(server, live_servers, ks):
        ...
        try:
            ...
            keyspaces = await manager.api.client.get_json("/storage_service/object_storage/keyspaces", host=server.ip_addr, params=params)
            assert ks in keyspaces

            tables = await manager.api.client.get_json("/storage_service/object_storage/tables", host=server.ip_addr, params={**params, "keyspace": ks})
            matching_tables = [table for table in tables if table["name"] == "test"]
            ...
            async def get_object_storage_refs():
                sstables = await manager.api.client.get_json("/storage_service/object_storage/sstables", host=server.ip_addr, params={**params, "keyspace": ks, "table": "test", "table_id": table_id})
                assert sstables
                refs = {}
