    ...
    return seastar::async([] {
        run_mutation_source_tests([](schema_ptr s, const std::vector<mutation>& partitions) -> mutation_source {
            ...
            for (auto&& m : partitions) {
                ...
                with_flat_mutation_reader(flat_mutation_reader_from_mutations(tests::make_permit(), {m}), [&] (flat_mutation_reader& rd) {
                    return rd.consume(fragment_scatterer{muts}, db::no_timeout);
                }).get();
                for (int i = 0; i < n; ++i) {
    ...
    return seastar::async([] {
        for_each_mutation_pair([](const mutation& m1, const mutation& m2, are_equal eq) {
            if (m1.schema()->version() != m2.schema()->version()) {
                ...
                auto from_upgrader = with_flat_mutation_reader(std::move(reader), [] (flat_mutation_reader& reader) {
