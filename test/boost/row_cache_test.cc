...
bool has_key(row_cache& cache, const dht::decorated_key& key) {
  ...
  return with_flat_mutation_reader(cache.make_reader(cache.schema(), tests::make_permit(), range), [&] (flat_mutation_reader& reader) {
   return read_mutation_from_flat_mutation_reader(reader, db::no_timeout).then([] (mutation_opt mo) {
    if (!bool(mo)) {
        return false;
    }
...
public:
    ...
    virtual future<> fill_buffer(db::timeout_clock::time_point timeout) override {
        ...
        return _delegating_reader.fill_buffer(timeout);
    }
    virtual future<> next_partition() override {
        _count_fill_buffer = false;
        ++_counter;
        return _delegating_reader.next_partition();
    }
};
...
SEASTAR_TEST_CASE(test_update_failure) {
    return seastar::async([] {
        ...
        auto has_only = [&] (const partitions_type& partitions) {
          with_flat_mutation_reader_in_thread(cache.make_reader(s, tests::make_permit(), query::full_partition_range), [&] (flat_mutation_reader& reader) {
            for (int i = 0; i < partition_count; i++) {
    ...
    private:
        ...
        public:
            reader(throttle& t, flat_mutation_reader r)
                    ...
                    : _delegating_reader(_underlying)
                    , _throttle(t)
            {}
            virtual future<> fill_buffer(db::timeout_clock::time_point timeout) override {
                return _delegating_reader.fill_buffer(timeout).finally([this] () {
...
SEASTAR_TEST_CASE(test_mvcc) {
    return seastar::async([] {
        auto test = [&] (const mutation& m1, const mutation& m2, bool with_active_memtable_reader) {
            ...
            auto close_mt1_reader_opt = defer([&mt1_reader_opt] { mt1_reader_opt->close().get(); });
...
SEASTAR_TEST_CASE(test_exception_safety_of_reads) {
    return seastar::async([] {
        ...
        auto run_queries = [&] {
            ...
            memory::with_allocation_failures([&] {
              with_flat_mutation_reader_in_thread(cache.make_reader(s, tests::make_permit(), query::full_partition_range, slice), [&] (flat_mutation_reader& rd) {
...
SEASTAR_TEST_CASE(test_exception_safety_of_transitioning_from_underlying_read_to_read_from_cache) {
    return seastar::async([] {
        ...
        memory::with_allocation_failures([&] {
          ...
          with_flat_mutation_reader_in_thread(cache.make_reader(s.schema(), tests::make_permit(), pr, slice), [&] (flat_mutation_reader& rd) {
            auto got_opt = read_mutation_from_flat_mutation_reader(rd, db::no_timeout).get0();
            BOOST_REQUIRE(got_opt);
        ...
        auto readers = parallel_for_each(boost::irange(0, n_readers), [&] (auto id) {
            ...
            return seastar::async([&, id] {
                while (!done) {
                    ...
                    auto actual_opt = with_flat_mutation_reader(make_reader(slice), [] (flat_mutation_reader& rd) {
                        return read_mutation_from_flat_mutation_reader(rd, db::no_timeout);
                    }).get0();
...
SEASTAR_TEST_CASE(test_hash_is_cached) {
    return seastar::async([] {
        ...
        with_flat_mutation_reader_in_thread(cache.make_reader(s, tests::make_permit()), [] (flat_mutation_reader& rd) {
...
SEASTAR_TEST_CASE(test_reading_progress_with_small_buffer_and_invalidation) {
    return seastar::async([] {
      ...
      with_flat_mutation_reader_in_thread(cache.make_reader(s.schema(), tests::make_permit(), pkr), [&] (flat_mutation_reader& rd3) {
