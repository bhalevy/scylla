...
mutation_source make_source_with(mutation m) {
    return mutation_source([m = std::move(m)] (schema_ptr s, reader_permit permit, const dht::partition_range&, const query::partition_slice&, const io_priority_class&, tracing::trace_state_ptr, streamed_mutation::forwarding fwd) {
        assert(m.schema() == s);
        return make_flat_mutation_reader_from_mutation_v2(s, std::move(permit), mutation(m), std::move(fwd));
    });
}
...
void test_cache_delegates_to_underlying_only_once_with_single_partition(schema_ptr s,
    ...
    row_cache cache(s, snapshot_source_from_snapshot(mutation_source([m, &secondary_calls_count] (
        ...
        if (range.contains(dht::ring_position(m.decorated_key()), dht::ring_position_comparator(*s))) {
            return make_counting_reader(make_flat_mutation_reader_from_mutation_v2(s, std::move(permit), mutation(m), std::move(fwd)), secondary_calls_count);
        } else {
            return make_counting_reader(make_empty_flat_reader_v2(s, std::move(permit)), secondary_calls_count);
        }
    })), tracker);
