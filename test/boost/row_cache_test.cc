...
mutation_source make_source_with(mutation m) {
    return mutation_source([m] (schema_ptr s, reader_permit permit, const dht::partition_range&, const query::partition_slice&, const io_priority_class&, tracing::trace_state_ptr, streamed_mutation::forwarding fwd) {
        assert(m.schema() == s);
        return flat_mutation_reader_from_mutations(permit, {m}, std::move(fwd));
    });
}
...
bool has_key(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, const dht::decorated_key& key) {
    auto range = dht::partition_range::make_singular(key);
    auto reader = cache.make_reader(cache.schema(), test_semaphore.make_permit(), range);
...
void verify_has(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, const dht::decorated_key& key) {
    BOOST_REQUIRE(has_key(cache, test_semaphore, key));
}

void verify_does_not_have(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, const dht::decorated_key& key) {
    BOOST_REQUIRE(!has_key(cache, test_semaphore, key));
}

void verify_has(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, const mutation& m) {
...
SEASTAR_TEST_CASE(test_cache_delegates_to_underlying) {
    return seastar::async([] {
        ...
        assert_that(cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range))
            .produces(m)
...
SEASTAR_TEST_CASE(test_cache_delegates_to_underlying_only_once_empty_full_range) {
    return seastar::async([] {
        ...
        cache_tracker tracker;
        row_cache cache(s, snapshot_source_from_snapshot(mutation_source([&secondary_calls_count, &test_semaphore] (
                schema_ptr s,
                reader_permit,
            ...
            return make_counting_reader(make_empty_flat_reader(s, test_semaphore.make_permit()), secondary_calls_count);
        })), tracker);
...
SEASTAR_TEST_CASE(test_cache_delegates_to_underlying_only_once_empty_single_partition_query) {
    return seastar::async([] {
        ...
        assert_that(cache.make_reader(s, test_semaphore.make_permit(), range))
            .produces_end_of_stream();
        BOOST_REQUIRE_EQUAL(secondary_calls_count, 1);
        assert_that(cache.make_reader(s, test_semaphore.make_permit(), range))
...
void test_cache_delegates_to_underlying_only_once_with_single_partition(reader_concurrency_semaphore_for_tests& test_semaphore,
                                                                        schema_ptr s,
                                                                        const mutation& m,
                                                                        const dht::partition_range& range,
    ...
    int secondary_calls_count = 0;
    cache_tracker tracker;
    row_cache cache(s, snapshot_source_from_snapshot(mutation_source([m, &secondary_calls_count, &test_semaphore] (
            schema_ptr s,
            reader_permit,
            const dht::partition_range& range,
            const query::partition_slice&,
        ...
        if (range.contains(dht::ring_position(m.decorated_key()), dht::ring_position_comparator(*s))) {
            return make_counting_reader(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m}, std::move(fwd)), secondary_calls_count);
        } else {
            return make_counting_reader(make_empty_flat_reader(s, test_semaphore.make_permit()), secondary_calls_count);
        }
    })), tracker);
...
SEASTAR_TEST_CASE(test_cache_delegates_to_underlying_only_once_single_key_range) {
    return seastar::async([] {
        ...
        auto s = make_schema();
        auto m = make_new_mutation(s);
        test_cache_delegates_to_underlying_only_once_with_single_partition(test_semaphore, s, m,
...
SEASTAR_TEST_CASE(test_cache_delegates_to_underlying_only_once_full_range) {
    return seastar::async([] {
        ...
        auto s = make_schema();
        auto m = make_new_mutation(s);
        test_cache_delegates_to_underlying_only_once_with_single_partition(test_semaphore, s, m, query::full_partition_range, 2);
    });
}
...
SEASTAR_TEST_CASE(test_cache_delegates_to_underlying_only_once_range_open) {
    return seastar::async([] {
        ...
        test_cache_delegates_to_underlying_only_once_with_single_partition(test_semaphore, s, m, range, 2);
    });
}
...
SEASTAR_TEST_CASE(test_cache_delegates_to_underlying_only_once_multiple_mutations) {
    return seastar::async([] {
        ...
        auto make_cache = [&tracker, &mt, &test_semaphore] (schema_ptr s, int& secondary_calls_count) -> lw_shared_ptr<row_cache> {
            auto secondary = mutation_source([&mt, &secondary_calls_count, &test_semaphore] (schema_ptr s, reader_permit, const dht::partition_range& range,
                ...
                return make_counting_reader(mt->make_flat_reader(s, test_semaphore.make_permit(), range, slice, pc, std::move(trace), std::move(fwd)), secondary_calls_count);
            });

            return make_lw_shared<row_cache>(s, snapshot_source_from_snapshot(secondary), tracker);
        };

        auto make_ds = [&make_cache, &test_semaphore] (schema_ptr s, int& secondary_calls_count) -> mutation_source {
            auto cache = make_cache(s, secondary_calls_count);
            return mutation_source([cache, &test_semaphore] (schema_ptr s, reader_permit, const dht::partition_range& range,
                    const query::partition_slice& slice, const io_priority_class& pc, tracing::trace_state_ptr trace, streamed_mutation::forwarding fwd) {
                return cache->make_reader(s, test_semaphore.make_permit(), range, slice, pc, std::move(trace), std::move(fwd));
            });
        };

        auto do_test = [&s, &partitions, &test_semaphore] (const mutation_source& ds, const dht::partition_range& range,
                                          int& secondary_calls_count, int expected_calls) {
            assert_that(ds.make_reader(s, test_semaphore.make_permit(), range))
        ...
        }
        {
            ...
            auto range2 = dht::partition_range::make(
                {partitions[0].decorated_key(), true},
                {partitions[1].decorated_key(), false});
            assert_that(ds.make_reader(s, test_semaphore.make_permit(), range2))
            ...
            auto range3 = dht::partition_range::make(
                {dht::ring_position::starting_at(key_before_all.token())},
                {partitions[2].decorated_key(), false});
            assert_that(ds.make_reader(s, test_semaphore.make_permit(), range3))
        ...
        }
        {
            ...
            auto cache = make_cache(s, secondary_calls_count);
            auto ds = mutation_source([cache, &test_semaphore] (schema_ptr s, reader_permit, const dht::partition_range& range,
                    const query::partition_slice& slice, const io_priority_class& pc, tracing::trace_state_ptr trace, streamed_mutation::forwarding fwd) {
                    return cache->make_reader(s, test_semaphore.make_permit(), range, slice, pc, std::move(trace), std::move(fwd));
            });
            ...
            assert_that(ds.make_reader(s, test_semaphore.make_permit(), query::full_partition_range))
...
SEASTAR_TEST_CASE(test_query_of_incomplete_range_goes_to_underlying) {
    return seastar::async([] {
        ...
        assert_that(cache.make_reader(s, test_semaphore.make_permit(), key0_range))
            .produces(mutations[0])
            .produces_end_of_stream();

        // Populate cache for last key
        assert_that(cache.make_reader(s, test_semaphore.make_permit(), key2_range))
...
SEASTAR_TEST_CASE(test_single_key_queries_after_population_in_reverse_order) {
    return seastar::async([] {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            assert_that(cache.make_reader(s, test_semaphore.make_permit(), key1_range))
                .produces(mutations[1])
                .produces_end_of_stream();
...
SEASTAR_TEST_CASE(test_partition_range_population_with_concurrent_memtable_flushes) {
    return seastar::async([] {
...

        {
            ...
            assert_that(cache.make_reader(s, test_semaphore.make_permit(), pr))
                .produces(mutations[1])
                .produces_end_of_stream();
        }
...
SEASTAR_TEST_CASE(test_row_cache_conforms_to_mutation_source) {
    return seastar::async([] {
        ...
        run_mutation_source_tests([&tracker, &test_semaphore] (schema_ptr s, const std::vector<mutation>& mutations) -> mutation_source {
            auto mt = make_lw_shared<memtable>(s);
            ...
            return mutation_source([cache, &test_semaphore] (schema_ptr s,
                    reader_permit,
                    const dht::partition_range& range,
                ...
                return cache->make_reader(s, test_semaphore.make_permit(), range, slice, pc, std::move(trace_state), fwd, fwd_mr);
            });
        });
    });
}
...
SEASTAR_TEST_CASE(test_reading_from_random_partial_partition) {
    return seastar::async([] {
        ...
        memtable_snapshot_source underlying(gen.schema(), test_semaphore);
        ...
        auto rd1 = cache.make_reader(gen.schema(), test_semaphore.make_permit());
        ...
        auto rd2 = cache.make_reader(gen.schema(), test_semaphore.make_permit());
...
SEASTAR_TEST_CASE(test_random_partition_population) {
    return seastar::async([] {
        ...
        assert_that(cache.make_reader(gen.schema(), test_semaphore.make_permit()))
            .produces(m1)
            .produces_end_of_stream();
        ...
        assert_that(cache.make_reader(gen.schema(), test_semaphore.make_permit(), pr))
            .produces(m1 + m2)
            .produces_end_of_stream();
    });
}
...
SEASTAR_TEST_CASE(test_eviction) {
    return seastar::async([] {
        ...
        for (auto&& key : keys) {
            ...
            auto rd = cache.make_reader(s, test_semaphore.make_permit(), pr);
...
SEASTAR_TEST_CASE(test_eviction_from_invalidated) {
    return seastar::async([] {
        ...
        for (auto&& key : keys) {
            cache.make_reader(s, test_semaphore.make_permit(), dht::partition_range::make_singular(key)).close().get();
        }
...
SEASTAR_TEST_CASE(test_eviction_after_schema_change) {
    return seastar::async([] {
...

        {
            ...
            auto rd = cache.make_reader(s2, test_semaphore.make_permit(), pr);
            auto close_rd = deferred_close(rd);
            rd.set_max_buffer_size(1);
            rd.fill_buffer(db::no_timeout).get();
        }
        ...
        verify_does_not_have(cache, test_semaphore, m.decorated_key());
    });
}
...
SEASTAR_TEST_CASE(test_single_partition_update) {
    return seastar::async([] {
        ...
        auto ck4 = make_ck(4);
        auto ck7 = make_ck(7);
        memtable_snapshot_source cache_mt(s, test_semaphore);
        {
            mutation m(s, pk);
...

        {
            auto slice = partition_slice_builder(*s)
                ...
                .build();
            auto reader = cache.make_reader(s, test_semaphore.make_permit(), range, slice);
        ...
        {
            auto reader = cache.make_reader(s, test_semaphore.make_permit(), range);
...
SEASTAR_TEST_CASE(test_update) {
    return seastar::async([] {
        ...
        for (auto&& key : keys_not_in_cache) {
            verify_has(cache, test_semaphore, key);
        }
        ...
        for (auto&& key : keys_not_in_cache) {
            verify_does_not_have(cache, test_semaphore, key);
        }
        ...
        for (auto&& m : new_mutations) {
            verify_has(cache, test_semaphore, m);
        }
    });
}
...
SEASTAR_TEST_CASE(test_update_failure) {
    return seastar::async([] {
        ...
        auto has_only = [&] (const partitions_type& partitions) {
            auto reader = cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range);
            auto close_reader = deferred_close(reader);
            for (int i = 0; i < partition_count; i++) {
...
private:
    ...
    public:
        ...
        flat_mutation_reader make_reader(schema_ptr s, const dht::partition_range& pr,
                const query::partition_slice& slice, const io_priority_class& pc, tracing::trace_state_ptr trace, streamed_mutation::forwarding fwd) {
            return make_flat_mutation_reader<reader>(_throttle, _underlying.make_reader(s, _test_semaphore.make_permit(), pr, slice, pc, std::move(trace), std::move(fwd)));
        }
    };
...
SEASTAR_TEST_CASE(test_continuity_flag_and_invalidate_race) {
    return seastar::async([] {
        ...
        auto rd = assert_that(cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range));
        rd.produces(ring[0]);
        ...
        rd = assert_that(cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range));
        rd.produces(ring[0])
          .produces(ring[1])
          .produces(ring[2]);
...
SEASTAR_TEST_CASE(test_cache_population_and_update_race) {
    return seastar::async([] {
        reader_concurrency_semaphore_for_tests test_semaphore;
        auto s = make_schema();
        memtable_snapshot_source memtables(s, test_semaphore);
        throttle thr;
        ...
        auto rd1 = cache.make_reader(s, test_semaphore.make_permit(), m0_range);
        rd1.set_max_buffer_size(1);
        auto rd1_fill_buffer = rd1.fill_buffer(db::no_timeout);

        auto rd2 = cache.make_reader(s, test_semaphore.make_permit());
        ...
        mt2_copy->apply(*mt2, test_semaphore.make_permit()).get();
        auto update_future = cache.update(row_cache::external_updater([&] { memtables.apply(mt2_copy); }), *mt2);

        auto rd3 = cache.make_reader(s, test_semaphore.make_permit());
        ...
        assert_that(cache.make_reader(s, test_semaphore.make_permit()))
                .produces(ring2[0])
                .produces(ring2[1])
                .produces(ring2[2])
                .produces_end_of_stream();
...
SEASTAR_TEST_CASE(test_cache_population_and_clear_race) {
    return seastar::async([] {
        ...
        auto rd1 = cache.make_reader(s, test_semaphore.make_permit());
        rd1.set_max_buffer_size(1);
        auto rd1_fill_buffer = rd1.fill_buffer(db::no_timeout);
...
SEASTAR_TEST_CASE(test_mvcc) {
    return seastar::async([] {
        ...
        auto test = [&] (const mutation& m1, const mutation& m2, bool with_active_memtable_reader) {
            ...
            memtable_snapshot_source underlying(s, test_semaphore);
...
SEASTAR_TEST_CASE(test_slicing_mutation_reader) {
    return seastar::async([] {
        ...
        auto run_tests = [&] (auto& ps, std::deque<int> expected) {
            ...
            auto reader = cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, ps);
            test_sliced_read_row_presence(std::move(reader), s, expected);

            reader = cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, ps);
            test_sliced_read_row_presence(std::move(reader), s, expected);

            auto dk = dht::decorate_key(*s, pk);
            auto singular_range = dht::partition_range::make_singular(dk);

            reader = cache.make_reader(s, test_semaphore.make_permit(), singular_range, ps);
            test_sliced_read_row_presence(std::move(reader), s, expected);
...
SEASTAR_TEST_CASE(test_lru) {
    return seastar::async([] {
        ...
        rd = cache.make_reader(s, test_semaphore.make_permit(), pr);
        assert_that(std::move(rd))
                .produces(partitions[0])
                .produces(partitions[1])
                .produces(partitions[2])
        ...
        rd = cache.make_reader(s, test_semaphore.make_permit());
        assert_that(std::move(rd))
                .produces(partitions[0])
                .produces(partitions[1])
                .produces(partitions[2])
...
SEASTAR_TEST_CASE(test_update_invalidating) {
    return seastar::async([] {
        reader_concurrency_semaphore_for_tests test_semaphore;
        auto s = test_semaphore.make_simple_schema();
        cache_tracker tracker;
        memtable_snapshot_source underlying(s.schema(), test_semaphore);
        ...
        assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit()))
            .produces(m1)
            .produces(m2)
            .produces_end_of_stream();
        ...
        mt_copy->apply(*mt, test_semaphore.make_permit()).get();
...
SEASTAR_TEST_CASE(test_scan_with_partial_partitions) {
    return seastar::async([] {
        ...
        // partially populate all up to middle of m1
        {
            ...
            assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), prange, slice))
...
SEASTAR_TEST_CASE(test_cache_populates_partition_tombstone) {
    return seastar::async([] {
        ...
        // singular range case
        {
            ...
            assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), prange))
                .produces(m1)
                .produces_end_of_stream();

            assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), prange)) // over populated
                .produces(m1)
                .produces_end_of_stream();
        }
        ...
        // range scan case
        {
            ...
            assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit())) // over populated
                .produces(m1)
                .produces(m2)
                .produces_end_of_stream();
        }
...
SEASTAR_TEST_CASE(test_tombstone_merging_in_partial_partition) {
    return seastar::async([] {
...

        {
            ...
            assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), pr, slice))
...

        {
            ...
            assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), pr, slice)).has_monotonic_positions();
        }
    });
}
...
static void populate_range(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore,
    ...
    const query::clustering_range& r = query::full_clustering_range)
{
    ...
    auto rd = cache.make_reader(cache.schema(), test_semaphore.make_permit(), pr, slice);
    auto close_rd = deferred_close(rd);
    consume_all(rd);
}

static void apply(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, memtable_snapshot_source& underlying, const mutation& m) {
    auto mt = make_lw_shared<memtable>(m.schema());
    mt->apply(m);
    cache.update(row_cache::external_updater([&] { underlying.apply(m); }), *mt).get();
}

static void apply(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, memtable_snapshot_source& underlying, memtable& m) {
    auto mt1 = make_lw_shared<memtable>(m.schema());
    mt1->apply(m, test_semaphore.make_permit()).get();
...
SEASTAR_TEST_CASE(test_readers_get_all_data_after_eviction) {
    return seastar::async([] {
        ...
        auto apply = [&] (mutation m) {
            ::apply(cache, test_semaphore, underlying, m);
        };

        auto make_reader = [&] (const query::partition_slice& slice) {
            auto rd = cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, slice);
            rd.set_max_buffer_size(1);
            rd.fill_buffer(db::no_timeout).get();
...
SEASTAR_TEST_CASE(test_single_tombstone_with_small_buffer) {
    return seastar::async([] {
        ...
        populate_range(cache, test_semaphore);

        auto rd = cache.make_reader(s.schema(), test_semaphore.make_permit(), pr);
        rd.set_max_buffer_size(1);
...
SEASTAR_TEST_CASE(test_tombstones_are_not_missed_when_range_is_invalidated) {
    return seastar::async([] {
        ...
        auto make_reader = [&] (const query::partition_slice& slice) {
            auto rd = cache.make_reader(s.schema(), test_semaphore.make_permit(), pr, slice);
            rd.set_max_buffer_size(1);
            rd.fill_buffer(db::no_timeout).get();
            return assert_that(std::move(rd));
        };
        ...
        // populate using reader created after invalidation
        {
            ...
            populate_range(cache, test_semaphore, pr, query::clustering_range::make_starting_with(s.make_ckey(5)));
...
SEASTAR_TEST_CASE(test_exception_safety_of_update_from_memtable) {
    return seastar::async([] {
        ...
        memory::with_allocation_failures([&] {
            ...
            populate_range(cache, test_semaphore, population_range);
            ...
            auto d = defer([&] {
                memory::scoped_critical_alloc_section dfg;
                assert_that(cache.make_reader(cache.schema(), test_semaphore.make_permit()))
                    .produces(orig)
                    .produces_end_of_stream();
            ...
            snap = mt->make_flat_reader(s.schema(), test_semaphore.make_permit(), pr);
    ...
    return seastar::async([] {
        ...
        auto run_queries = [&] {
            ...
            memory::with_allocation_failures([&] {
                ...
                assert_that(cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, slice))
                    .produces(mut, ranges);
            });
        };
        ...
        memory::with_allocation_failures([&] {
            {
                memory::scoped_critical_alloc_section dfg;
                cache.evict();
                populate_range(cache, test_semaphore, pr, s.make_ckey_range(6, 10));
            }
        ...
        memory::with_allocation_failures([&] {
            {
                memory::scoped_critical_alloc_section dfg;
                cache.evict();
                populate_range(cache, test_semaphore, dht::partition_range::make_singular(pkeys[1]));
                populate_range(cache, test_semaphore, dht::partition_range::make({pkeys[3]}, {pkeys[5]}));
            }
...
SEASTAR_TEST_CASE(test_concurrent_population_before_latest_version_iterator) {
    return seastar::async([] {
        ...
        {
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(0, 1));
            ...
            apply(cache, test_semaphore, underlying, m2);
            ...
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(3, 3));
            ...
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(2, 3));
        ...
        {
            cache.evict();
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(4, 4));
            ...
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(2, 4));
...
SEASTAR_TEST_CASE(test_concurrent_populating_partition_range_reads) {
    return seastar::async([] {
        ...
        populate_range(cache, test_semaphore, dht::partition_range::make_singular({keys[0]}));
        populate_range(cache, test_semaphore, dht::partition_range::make_singular({keys[1]}));
        populate_range(cache, test_semaphore, dht::partition_range::make_singular({keys[6]}));

        // FIXME: When readers have buffering across partitions, limit buffering to 1
        auto rd1 = assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), range1));
        rd1.produces(muts[0]);

        auto rd2 = assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), range2));
        rd2.produces(muts[4]);
...
static void populate_range(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, const dht::partition_range& pr, const query::clustering_row_ranges& ranges) {
    for (auto&& r : ranges) {
        populate_range(cache, test_semaphore, pr, r);
    }
}

static void check_continuous(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, const dht::partition_range& pr, const query::clustering_range& r = query::full_clustering_range) {
...
static void check_continuous(row_cache& cache, reader_concurrency_semaphore_for_tests& test_semaphore, dht::partition_range& pr, const query::clustering_row_ranges& ranges) {
    for (auto&& r : ranges) {
        check_continuous(cache, test_semaphore, pr, r);
    }
}
...
SEASTAR_TEST_CASE(test_random_row_population) {
    return seastar::async([] {
        ...
        auto make_reader = [&] (const query::partition_slice* slice = nullptr) {
            auto rd = cache.make_reader(s.schema(), test_semaphore.make_permit(), pr, slice ? *slice : s.schema()->full_slice());
            rd.set_max_buffer_size(1);
            rd.fill_buffer(db::no_timeout).get();
            return rd;
        };
        ...
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make({s.make_ckey(0)}, {s.make_ckey(9)}));
    });
}
...
SEASTAR_TEST_CASE(test_no_misses_when_read_is_repeated) {
    return seastar::async([] {
        ...
        for (auto n_ranges : {1, 2, 4}) {
            ...
            populate_range(cache, test_semaphore, pr, ranges);
            check_continuous(cache, test_semaphore, pr, ranges);
...
SEASTAR_TEST_CASE(test_continuity_is_populated_when_read_overlaps_with_older_version) {
    return seastar::async([] {
...

        {
            ...
            populate_range(cache, test_semaphore, pr, query::clustering_range::make({s.make_ckey(2)}, {s.make_ckey(4)}));

            apply(m2);

            populate_range(cache, test_semaphore, pr, s.make_ckey_range(3, 5));
            check_continuous(cache, test_semaphore, pr, s.make_ckey_range(2, 5));

            populate_range(cache, test_semaphore, pr, s.make_ckey_range(3, 7));
            check_continuous(cache, test_semaphore, pr, s.make_ckey_range(2, 7));

            populate_range(cache, test_semaphore, pr, s.make_ckey_range(3, 8));
            check_continuous(cache, test_semaphore, pr, s.make_ckey_range(2, 8));

            populate_range(cache, test_semaphore, pr, s.make_ckey_range(3, 9));
            check_continuous(cache, test_semaphore, pr, s.make_ckey_range(2, 9));

            populate_range(cache, test_semaphore, pr, s.make_ckey_range(0, 1));
            check_continuous(cache, test_semaphore, pr, s.make_ckey_range(0, 1));
            check_continuous(cache, test_semaphore, pr, s.make_ckey_range(2, 9));

            populate_range(cache, test_semaphore, pr, s.make_ckey_range(1, 2));
            check_continuous(cache, test_semaphore, pr, s.make_ckey_range(0, 9));

            populate_range(cache, test_semaphore, pr, query::full_clustering_range);
            check_continuous(cache, test_semaphore, pr, query::full_clustering_range);

            assert_that(cache.make_reader(s.schema(), test_semaphore.make_permit(), pr))
                .produces(m1 + m2)
                .produces_end_of_stream();
        }
        ...
        {
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(2, 2));
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(5, 5));
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(8, 8));
        ...
        { // singular range case
            populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(4)));
            populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(7)));
...
SEASTAR_TEST_CASE(test_continuity_population_with_multicolumn_clustering_key) {
    return seastar::async([] {
        ...
        auto make_reader = [&] (const query::partition_slice* slice = nullptr) {
            auto rd = cache.make_reader(s, test_semaphore.make_permit(), pr, slice ? *slice : s->full_slice());
            rd.set_max_buffer_size(1);
            rd.fill_buffer(db::no_timeout).get();
            return rd;
        };

        {
            ...
            populate_range(cache, test_semaphore, pr, range_3_4);
            check_continuous(cache, test_semaphore, pr, range_3_4);
            ...
            auto slice34 = partition_slice_builder(*s)
                ...
                .build();
            assert_that(cache.make_reader(s, test_semaphore.make_permit(), pr, slice34))
...
SEASTAR_TEST_CASE(test_continuity_is_populated_for_single_row_reads) {
    return seastar::async([] {
        ...
        populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(2)));
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(2)));

        populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(6)));
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(6)));

        populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(3)));
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(3)));

        populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(4)));
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(4)));

        populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(1)));
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(1)));

        populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(5)));
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(5)));

        populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(7)));
        check_continuous(cache, test_semaphore, pr, query::clustering_range::make_singular(s.make_ckey(7)));
...
SEASTAR_TEST_CASE(test_concurrent_setting_of_continuity_on_read_upper_bound) {
    return seastar::async([] {
...

        {
            ...
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(0, 0));
            populate_range(cache, test_semaphore, pr, s.make_ckey_range(3, 3));
            ...
            // check_continuous(cache, test_semaphore, pr, s.make_ckey_range(0, 4));
...
SEASTAR_TEST_CASE(test_tombstone_merging_of_overlapping_tombstones_in_many_versions) {
    return seastar::async([] {
        ...
        auto make_reader = [&] {
            auto rd = cache.make_reader(s.schema(), test_semaphore.make_permit());
            rd.set_max_buffer_size(1);
            rd.fill_buffer(db::no_timeout).get();
            return rd;
        };

        apply(cache, test_semaphore, underlying, m1);
        populate_range(cache, test_semaphore, pr, s.make_ckey_range(0, 3));
...
SEASTAR_TEST_CASE(test_concurrent_reads_and_eviction) {
    return seastar::async([] {
        ...
        auto make_reader = [&] (const query::partition_slice& slice) {
            auto rd = cache.make_reader(s, test_semaphore.make_permit(), pr, slice);
...
SEASTAR_TEST_CASE(test_alter_then_preempted_update_then_memtable_read) {
    return seastar::async([] {
        ...
        memtable_snapshot_source underlying(ss.schema(), test_semaphore);
        ...
        assert_that(cache.make_reader(s, test_semaphore.make_permit(), pr)).produces(m);
        ...
        auto mt2_reader = mt2->make_flat_reader(s, test_semaphore.make_permit(), pr, s->full_slice(), default_priority_class(),
            nullptr, streamed_mutation::forwarding::no, mutation_reader::forwarding::no);
        auto cache_reader = cache.make_reader(s, test_semaphore.make_permit(), pr, s->full_slice(), default_priority_class(),
            nullptr, streamed_mutation::forwarding::no, mutation_reader::forwarding::no);
        ...
        assert_that(cache.make_reader(s, test_semaphore.make_permit())).produces(m + m2);
    });
}
...
SEASTAR_TEST_CASE(test_cache_update_and_eviction_preserves_monotonicity_of_memtable_readers) {
    ...
    return seastar::async([] {
        ...
        auto mt_rd1 = mt->make_flat_reader(s, test_semaphore.make_permit());
        ...
        auto mt_rd2 = mt->make_flat_reader(s, test_semaphore.make_permit());
        mt_rd2.set_max_buffer_size(1);
        mt_rd2.fill_buffer(db::no_timeout).get();

        apply(cache, test_semaphore, underlying, *mt);

        assert_that(std::move(mt_rd1))
            .produces(m1);

        auto c_rd1 = cache.make_reader(s, test_semaphore.make_permit());
        c_rd1.set_max_buffer_size(1);
        c_rd1.fill_buffer(db::no_timeout).get();

        apply(cache, test_semaphore, underlying, m2);

        auto c_rd2 = cache.make_reader(s, test_semaphore.make_permit());
...
SEASTAR_TEST_CASE(test_hash_is_cached) {
    return seastar::async([] {
        ...
        {
            auto rd = cache.make_reader(s, test_semaphore.make_permit());
            auto close_rd = deferred_close(rd);
...
SEASTAR_TEST_CASE(test_random_population_with_many_versions) {
    return seastar::async([] {
        ...
        auto make_reader = [&] () {
            auto rd = cache.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, s->full_slice());
            rd.set_max_buffer_size(1);
            rd.fill_buffer(db::no_timeout).get();
            return assert_that(std::move(rd));
        };
        ...
        {
            apply(cache, test_semaphore, underlying, m1);
            populate_range(cache, test_semaphore, query::full_partition_range, gen.make_random_ranges(1));
            ...
            apply(cache, test_semaphore, underlying, m3);
            populate_range(cache, test_semaphore, query::full_partition_range, gen.make_random_ranges(1));

            auto snap3 = make_reader();

            populate_range(cache, test_semaphore, query::full_partition_range, gen.make_random_ranges(1));
            populate_range(cache, test_semaphore, query::full_partition_range, gen.make_random_ranges(2));
            populate_range(cache, test_semaphore, query::full_partition_range, gen.make_random_ranges(3));
    ...
    return seastar::async([] {
...

        {
            auto slice = partition_slice_builder(*s)
                .with_ranges({})
                .build();
            assert_that(cache.make_reader(s, test_semaphore.make_permit(), dht::partition_range::make_singular(keys[0]), slice))
                .produces(m1);
        }

        evict_one_partition(tracker); // should evict keys[1], not keys[0]

        verify_does_not_have(cache, test_semaphore, keys[1]);
        verify_has(cache, test_semaphore, keys[0]);
        verify_has(cache, test_semaphore, keys[2]);
    });
}
...
SEASTAR_TEST_CASE(test_eviction_after_old_snapshot_touches_overriden_rows_keeps_later_snapshot_consistent) {
    return seastar::async([] {
...

        {
            ...
            auto rd1 = cache.make_reader(s, test_semaphore.make_permit(), pr1);
            rd1.set_max_buffer_size(1);
            rd1.fill_buffer(db::no_timeout).get();
            ...
            auto rd2 = cache.make_reader(s, test_semaphore.make_permit(), pr2);
            rd2.set_max_buffer_size(1);
        ...
        }
        {
            ...
            populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(table.make_ckey(0)));
            populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(table.make_ckey(1)));
            populate_range(cache, test_semaphore, pr, query::clustering_range::make_singular(table.make_ckey(2)));

            auto rd1 = cache.make_reader(s, test_semaphore.make_permit(), pr);
            rd1.set_max_buffer_size(1);
            rd1.fill_buffer(db::no_timeout).get();

            apply(cache, test_semaphore, underlying, m2);

            auto rd2 = cache.make_reader(s, test_semaphore.make_permit(), pr);
            rd2.set_max_buffer_size(1);
...
SEASTAR_TEST_CASE(test_reading_progress_with_small_buffer_and_invalidation) {
    return seastar::async([] {
        ...
        populate_range(cache, test_semaphore, pkr, s.make_ckey_range(5, 7));
        populate_range(cache, test_semaphore, pkr, s.make_ckey_range(4, 7));
        populate_range(cache, test_semaphore, pkr, s.make_ckey_range(3, 7));

        auto rd3 = cache.make_reader(s.schema(), test_semaphore.make_permit(), pkr);
