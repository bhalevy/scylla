...
SEASTAR_THREAD_TEST_CASE(test_large_collection_allocation) {
    ...
    for (size_t i = 0; i != sizes_kb.size(); ++i) {
        ...
        res_mut_opt->partition().compact_for_query(*schema, std::nullopt, res_mut_opt->decorated_key(), gc_clock::now(), {query::full_clustering_range}, true, false,
...
static mutation compacted(const mutation& m, gc_clock::time_point now) {
    auto result = m;
    result.partition().compact_for_compaction(*result.schema(), std::nullopt, always_gc, result.decorated_key(), now);
    return result;
}
...
SEASTAR_TEST_CASE(test_tombstone_purge) {
    ...
    m.partition().compact_for_compaction(*s, std::nullopt, always_gc, m.decorated_key(), gc_clock::now());
...
SEASTAR_TEST_CASE(test_trim_rows) {
    return seastar::async([] {
        ...
        auto compact_and_expect_empty = [&] (mutation m, std::vector<query::clustering_range> ranges) {
            mutation m2 = m;
            m.partition().compact_for_query(*s, std::nullopt, m.decorated_key(), now, ranges, false, false, query::max_rows);
            BOOST_REQUIRE(m.partition().clustered_rows().empty());

            std::reverse(ranges.begin(), ranges.end());
            m2.partition().compact_for_query(*s, std::nullopt, m2.decorated_key(), now, ranges, false, true, query::max_rows);
...
SEASTAR_TEST_CASE(test_mutation_diff_with_random_generator) {
    return seastar::async([] {
        ...
        for_each_mutation_pair([&] (auto m1, auto m2, are_equal eq) {
            ...
            m1.partition().compact_for_compaction(*s, std::nullopt, never_gc, m1.decorated_key(), now);
            m2.partition().compact_for_compaction(*s, std::nullopt, never_gc, m2.decorated_key(), now);
            auto m12 = m1;
            m12.apply(m2);
...
void run_compaction_data_stream_split_test(const schema& schema, reader_permit permit, gc_clock::time_point query_time,
    ...
    for (auto& mut : mutations) {
        mut.partition().compact_for_compaction(schema, std::nullopt, never_gc, mut.decorated_key(), query_time);
    }
...
SEASTAR_THREAD_TEST_CASE(test_compactor_range_tombstone_spanning_many_pages) {
    ...
    mutation ref_mut(s, pk);
    {
        ...
        ref_mut.partition().compact_for_query(*s, std::nullopt, pk, query_time, {query::clustering_range::make_open_ended_both_sides()}, true, false, max_rows);
    }
    ...
    testlog.info("non-paged v2");
    {
        ...
        auto c = compact_for_query_v2<consumer_v2>(*s, std::nullopt, query_time, s->full_slice(), max_rows, max_partitions, consumer_v2{permit, res_mut, max_rows});
        auto reader = make_flat_mutation_reader_from_fragments(s, permit, make_frags());
        auto close_reader = deferred_close(reader);
    ...
    {
        mutation res_mut(s, pk);
        auto compaction_state = make_lw_shared<compact_mutation_state<compact_for_sstables::no>>(*s, std::nullopt, query_time, s->full_slice(), 1, max_partitions);
        auto reader = make_flat_mutation_reader_from_fragments(s, permit, make_frags());
        auto close_reader = deferred_close(reader);
    ...
    {
        mutation res_mut(s, pk);
        auto compaction_state = make_lw_shared<compact_mutation_state<compact_for_sstables::no>>(*s, std::nullopt, query_time, s->full_slice(), max_rows, max_partitions);
        auto reader = make_flat_mutation_reader_from_fragments(s, permit, make_frags());
        auto close_reader = deferred_close(reader);
