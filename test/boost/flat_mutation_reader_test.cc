    ...
    return seastar::async([] {
        for_each_mutation([&] (const mutation& m) {
            ...
            for (size_t depth = 1; depth <= fragments_in_m + 1; ++depth) {
              with_flat_mutation_reader_in_thread(flat_mutation_reader_from_mutations(tests::make_permit(), {m}), [&m, depth] (flat_mutation_reader& r) {
    ...
    return seastar::async([] {
        auto test = [] (mutation m1, mutation m2) {
            ...
            for (size_t depth = 1; depth < fragments_in_m1; ++depth) {
                ...
                auto close_r2 = defer([&r2] { r2.close().get(); });
...
SEASTAR_THREAD_TEST_CASE(test_flat_mutation_reader_move_buffer_content_to) {
    ...
    auto close_dummy_reader = defer([&dummy_reader] { dummy_reader.close().get(); });
...
SEASTAR_THREAD_TEST_CASE(test_mutation_reader_from_fragments_as_mutation_source) {
    auto populate = [] (schema_ptr, const std::vector<mutation> &muts) {
        return mutation_source([=] (
          ...
          with_flat_mutation_reader_in_thread(flat_mutation_reader_from_mutations(tests::make_permit(), squash_mutations(muts)), [&fragments] (flat_mutation_reader& rd) {
            return rd.consume_pausable([&fragments] (mutation_fragment mf) {
