...
struct mock_consumer {
    ...
    stop_iteration consume(static_row&& sr) {
        _result._fragments.push_back(mutation_fragment(_schema, ssft.local_test_semaphore().make_permit(), std::move(sr)));
        return update_depth();
    }
    stop_iteration consume(clustering_row&& cr) {
        _result._fragments.push_back(mutation_fragment(_schema, ssft.local_test_semaphore().make_permit(), std::move(cr)));
        return update_depth();
    }
    stop_iteration consume(range_tombstone&& rt) {
        _result._fragments.push_back(mutation_fragment(_schema, ssft.local_test_semaphore().make_permit(), std::move(rt)));
        return update_depth();
    }
...
static size_t count_fragments(mutation m) {
    auto r = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), {m});
    ...
    return seastar::async([] {
        for_each_mutation([&] (const mutation& m) {
            ...
            for (size_t depth = 1; depth <= fragments_in_m + 1; ++depth) {
                ...
                auto r2 = assert_that(flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), {m}));
    ...
    return seastar::async([] {
        auto test = [] (mutation m1, mutation m2) {
            ...
            for (size_t depth = 1; depth < fragments_in_m1; ++depth) {
                auto r = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), {m1, m2});
                ...
                auto r2 = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), {m1, m2});
...
SEASTAR_TEST_CASE(test_fragmenting_and_freezing) {
    return seastar::async([] {
        ...
        for_each_mutation([&] (const mutation& m) {
            ...
            fragment_and_freeze(flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), { mutation(m) }), [&] (auto fm, bool frag) {
                BOOST_REQUIRE(!frag);
        ...
        auto test_random_streams = [&ssft] (random_mutation_generator&& gen) {
            for (auto i = 0; i < 4; i++) {
                auto muts = gen(4);
                ...
                // Freeze all
                fragment_and_freeze(flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), muts), [&] (auto fm, bool frag) {
                    BOOST_REQUIRE(!frag);
    ...
    return seastar::async([] {
        for_each_mutation_pair([] (auto&& m1, auto&& m2, are_equal eq) {
            auto get_hash = [] (mutation m) {
                return partition_checksum::compute(flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), { m }),
        ...
        auto test_random_streams = [] (random_mutation_generator&& gen) {
            for (auto i = 0; i < 4; i++) {
                ...
                while (!muts2.empty()) {
                    auto chk = partition_checksum::compute(flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), muts2),
                                                           repair_checksum::streamed).get0();
                ...
                for (auto k = 0u; k < muts.size(); k++) {
                    auto chk = partition_checksum::compute(flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), { muts[k] }),
                                                           repair_checksum::streamed).get0();
...
SEASTAR_THREAD_TEST_CASE(test_flat_mutation_reader_move_buffer_content_to) {
    ...
    auto reader = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), {mut_orig}, dht::partition_range::make_open_ended_both_sides());
    auto close_reader = deferred_close(reader);
    auto dummy_impl = std::make_unique<dummy_reader_impl>(s.schema(), ssft.local_test_semaphore().make_permit());
...
SEASTAR_TEST_CASE(test_multi_range_reader) {
    return seastar::async([] {
        ...
        auto source = mutation_source([&] (schema_ptr, reader_permit, const dht::partition_range& range) {
            return flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), ms, range);
        });
        ...
        auto run_test = [&] (auto make_empty_ranges, auto make_single_ranges, auto make_multiple_ranges) {
            ...
            assert_that(make_flat_multi_range_reader(s.schema(), ssft.local_test_semaphore().make_permit(), source, make_empty_ranges(), s.schema()->full_slice()))
            ...
            assert_that(make_flat_multi_range_reader(s.schema(), ssft.local_test_semaphore().make_permit(), source, make_single_ranges(), s.schema()->full_slice()))
                    .produces(ms[1])
                    .produces(ms[2])
                    .produces_end_of_stream()
                    .fast_forward_to(fft_range)
            ...
            assert_that(make_flat_multi_range_reader(s.schema(), ssft.local_test_semaphore().make_permit(), source, make_multiple_ranges(), s.schema()->full_slice()))
                    .produces(ms[1])
                    .produces(ms[2])
                    .produces(ms[4])
                    .produces(ms[6])
...
public:
    ...
    stop_iteration consume(static_row&& sr) {
        ...
        _mutations.back().partition().apply(*_schema, mutation_fragment(*_schema, ssft.local_test_semaphore().make_permit(), std::move(sr)));
    ...
    stop_iteration consume(clustering_row&& cr) {
        ...
        _mutations.back().partition().apply(*_schema, mutation_fragment(*_schema, ssft.local_test_semaphore().make_permit(), std::move(cr)));
        return stop_iteration(bool(_skip_partition));
    }
    stop_iteration consume(range_tombstone&& rt) {
        BOOST_REQUIRE(_inside_partition);
        ...
        _mutations.back().partition().apply(*_schema, mutation_fragment(*_schema, ssft.local_test_semaphore().make_permit(), std::move(rt)));
        return stop_iteration(bool(_skip_partition));
    }
    stop_iteration consume_end_of_partition() {
        BOOST_REQUIRE(_inside_partition);
...
void test_flat_stream(schema_ptr s, std::vector<mutation> muts, reversed_partitions reversed, in_thread thread) {
...

  {
    ...
    auto fmr = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), muts);
...
SEASTAR_TEST_CASE(test_make_forwardable) {
    return seastar::async([] {
        ...
        auto make_reader = [&] (auto& range) {
            return assert_that(
                make_forwardable(flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), ms, range, streamed_mutation::forwarding::no)));
        };
...
SEASTAR_TEST_CASE(test_abandoned_flat_mutation_reader_from_mutation) {
    return seastar::async([] {
        for_each_mutation([&] (const mutation& m) {
            auto rd = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), {mutation(m)});
...
SEASTAR_THREAD_TEST_CASE(test_mutation_reader_from_mutations_as_mutation_source) {
    auto populate = [] (schema_ptr, const std::vector<mutation> &muts) {
        return mutation_source([=] (
            ...
            return flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), squash_mutations(muts), range, slice, fwd_sm);
        });
    };
    ...
    auto populate = [] (schema_ptr, const std::vector<mutation> &muts) {
        return mutation_source([=] (
            ...
            auto get_fragments = [&muts] {
                ...
                auto rd = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), squash_mutations(muts));
                auto close_rd = deferred_close(rd);
            ...
            auto rd = make_flat_mutation_reader_from_fragments(schema, ssft.local_test_semaphore().make_permit(), get_fragments(), range, slice);
            if (fwd_sm) {
...
SEASTAR_THREAD_TEST_CASE(test_reverse_reader_memory_limit) {
    ...
    auto test_with_partition = [&] (bool with_static_row) {
        ...
        auto reader = flat_mutation_reader_from_mutations(ssft.local_test_semaphore().make_permit(), {mut});
