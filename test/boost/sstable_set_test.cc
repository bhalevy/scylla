...
namespace dht {
    dht::partition_range make_partition_range(const dht::token_range& tr) {
        ...
        std::optional<dht::partition_range::bound> start_bound;
        if (tr.start()) {
            const auto& token = tr.start()->value();
            start_bound = dht::ring_position::starting_at(tr.start()->is_inclusive() ? token : dht::next_token(token));
            if (!tr.end()) {
                return dht::partition_range::make_starting_with(*start_bound);
            }
        }
        std::optional<dht::partition_range::bound> end_bound;
        if (tr.end()) {
            const auto& token = tr.end()->value();
            end_bound = dht::ring_position::ending_at(tr.end()->is_inclusive() ? token : !token.is_last() ? dht::next_token(token) : dht::maximum_token());
            if (!tr.start()) {
                return dht::partition_range::make_ending_with(*end_bound);
            }
        }
        return dht::partition_range::make(std::move(*start_bound), std::move(*end_bound));
    }
}
