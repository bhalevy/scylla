...
static future<sstables::shared_sstable> open_sstable(test_env& env, schema_ptr schema, sstables::generation_type generation) {
    return env.reusable_sst(std::move(schema), generation);
}
...
SEASTAR_TEST_CASE(compact) {
    return sstables::test_env::do_with_async([] (sstables::test_env& env) {
        ...
        shared_sstable sst = open_sstable(env, s, new_sstables[0]->generation()).get();
...
static future<std::vector<sstables::generation_type>> compact_sstables(test_env& env, std::vector<sstables::generation_type> generations_to_compact,
        sstables::generation_factory& gen_factory, bool create_sstables, uint64_t min_sstable_size, compaction_strategy_type strategy) {
    BOOST_REQUIRE(smp::count == 1);
    return seastar::async(
            [&env, generations = std::move(generations_to_compact), &gen_factory, create_sstables, min_sstable_size, strategy] () mutable {
        ...
        const auto& tmpdir_path = sst_gen.tmp.path().native();

        auto cf = env.make_table_for_tests(s, tmpdir_path);
        auto stop_cf = deferred_stop(cf);

        std::vector<sstables::shared_sstable> sstables;
        std::vector<sstables::generation_type> created;

        if (!create_sstables) {
            auto opened_sstables = open_sstables(env, s, tmpdir_path, generations).get();
        ...
        auto new_sstable = [&] {
            auto sst = sst_gen();
            created.push_back(sst->generation());
            return sst;
        };
        ...
        if (strategy == compaction_strategy_type::size_tiered) {
            ...
            compact_sstables(sstables::compaction_descriptor(std::move(sstables_to_compact),
            ...
            compact_sstables(sstables::compaction_descriptor(std::move(candidate.sstables),
...
static future<sstables::generation_type> compact_sstables(test_env& env, std::vector<sstables::generation_type> generations_to_compact, sstables::generation_factory& gen_factory, bool create_sstables = true) {
    uint64_t min_sstable_size = 50;
    return compact_sstables(env, std::move(generations_to_compact), gen_factory, create_sstables, min_sstable_size,
                            compaction_strategy_type::size_tiered).then([] (auto ret) {
        // size tiered compaction will output at most one sstable, let's assert that.
        BOOST_REQUIRE(ret.size() == 1);
        return make_ready_future<sstables::generation_type>(ret[0]);
    });
}

static future<> check_compacted_sstables(test_env& env, sstables::generation_type generation, std::vector<sstables::generation_type> compacted_generations) {
    auto s = make_shared_schema({}, some_keyspace, some_column_family,
        {{"p1", utf8_type}}, {{"c1", utf8_type}}, {{"r1", utf8_type}}, {}, utf8_type);
    auto tmpdir_path = env.tempdir().path().native();

    auto generations = make_lw_shared<std::vector<sstables::generation_type>>(std::move(compacted_generations));

    return env.reusable_sst(s, tmpdir_path, generation).then([&env, s, generations] (shared_sstable sst) {
...
SEASTAR_TEST_CASE(compact_02) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        sstables::generation_factory gen_factory(sstables::generation_type(18));
        std::vector<sstables::generation_type> all_generations;
        std::vector<sstables::generation_type> generations_to_compact;
        std::vector<sstables::generation_type> compacted;

        auto populate_batch = [&] (size_t count) {
            generations_to_compact.clear();
            generations_to_compact.reserve(count);
            for (auto i = 0; i < count; ++i) {
                generations_to_compact.emplace_back(gen_factory());
            }
            std::copy(generations_to_compact.begin(), generations_to_compact.end(), std::back_inserter(all_generations));
            return generations_to_compact;
        };

        auto compact_and_verify = [&] (size_t count) {
            ...
            auto gen = compact_sstables(env, populate_batch(count), gen_factory).get();
            compacted.emplace_back(gen);
            // Check that generation 22 contains all keys of generations 18, 19, 20 and 21.
            check_compacted_sstables(env, gen, generations_to_compact).get();
        };
        ...
        auto gen = compact_sstables(env, compacted, gen_factory, false).get();
        // Check that the compacted sstable contains all keys.
        check_compacted_sstables(env, gen, all_generations).get();
    });
}
...
SEASTAR_TEST_CASE(leveled_02) {
  ...
  return test_env::do_with_async([] (test_env& env) {
    ...
    auto gens = generations_from_values<std::set<sstables::generation_type>>({ 1, 2, 3 });
    for (auto& sst : candidate.sstables) {
...
SEASTAR_TEST_CASE(leveled_03) {
  ...
  return test_env::do_with_async([] (test_env& env) {
    ...
    std::set<std::pair<sstables::generation_type::int_t, uint32_t>> gen_and_level = { {1,0}, {2,0}, {3,1} };
    for (auto& sst : candidate.sstables) {
        std::pair<sstables::generation_type::int_t, uint32_t> pair(generation_value(sst->generation()), sst->get_sstable_level());
...
SEASTAR_TEST_CASE(leveled_05) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        for (auto i = 0; i < sstables_in_round; ++i) {
            generations_to_compact.emplace_back(gen_factory());
        }

        // Check compaction code with leveled strategy. In this test, two sstables of level 0 will be created.
        auto generations = compact_sstables(env, generations_to_compact, gen_factory, true, 1024*1024, compaction_strategy_type::leveled).get();
...
SEASTAR_TEST_CASE(test_sstable_max_local_deletion_time_2) {
    ...
    return test_env::do_with_async([] (test_env& env) {
            ...
            for (auto version : writable_sstable_versions) {
                ...
                auto tmpdir_path = sst_gen.tmp.path().native();
                auto cf = env.make_table_for_tests(s);
                auto close_cf = deferred_stop(cf);
                auto mt = make_lw_shared<replica::memtable>(s);
...
SEASTAR_TEST_CASE(date_tiered_strategy_test_2) {
  return test_env::do_with_async([] (test_env& env) {
    ...
    auto sstables = manifest.get_next_sstables(cf.as_table_state(), candidates, gc_before);
    std::unordered_set<sstables::generation_type::int_t> gens;
    for (auto sst : sstables) {
...
SEASTAR_TEST_CASE(time_window_strategy_ts_resolution_check) {
  return test_env::do_with([] (test_env& env) {
...

    {
        ...
        auto sst = env.make_sstable(s, "", gen_factory());
...
SEASTAR_TEST_CASE(size_tiered_beyond_max_threshold_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    for (auto i = 0; i < (max_threshold+1); i++) { // (max_threshold+1) sstables of similar size
        auto sst = env.make_sstable(cf.schema(), "", gen_factory());
...
SEASTAR_TEST_CASE(max_ongoing_compaction_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sst_gen = [&, gen = sstables::generation_factory()] (size_t idx) mutable {
            auto s = schemas[idx];
            auto t = tables[idx];
...
SEASTAR_TEST_CASE(compound_sstable_set_incremental_selector_test) {
    return test_env::do_with([] (test_env& env) {
        ...
        sstables::generation_factory gen_factory;
        {
            ...
            std::vector<shared_sstable> ssts;
            ssts.push_back(new_sstable(set1, gen_factory(), 0, 1, 1));
            ssts.push_back(new_sstable(set2, gen_factory(), 0, 1, 1));
            ssts.push_back(new_sstable(set1, gen_factory(), 3, 4, 1));
            ssts.push_back(new_sstable(set2, gen_factory(), 4, 4, 1));
            ssts.push_back(new_sstable(set1, gen_factory(), 4, 5, 1));
...

        {
            ...
            auto set2 = make_lw_shared<sstable_set>(cs.make_sstable_set(s));
            std::vector<shared_sstable> ssts;
            ssts.push_back(new_sstable(set1, gen_factory(), 0, 1, 0));
            ssts.push_back(new_sstable(set2, gen_factory(), 0, 1, 1));
            ssts.push_back(new_sstable(set1, gen_factory(), 0, 1, 1));
            ssts.push_back(new_sstable(set2, gen_factory(), 3, 4, 1));
            ssts.push_back(new_sstable(set1, gen_factory(), 4, 4, 1));
            ssts.push_back(new_sstable(set2, gen_factory(), 4, 5, 1));
...

        {
            ...
            auto incremental_selection_test = [&] (strategy_param param) {
                ...
                new_sstable(set1, gen_factory(), 1, 1, 1);
                new_sstable(set2, gen_factory(), 0, 2, 1);
                new_sstable(set2, gen_factory(), 3, 3, 1);
                new_sstable(set2, gen_factory(), 4, 4, 1);
...
SEASTAR_TEST_CASE(simple_backlog_controller_test) {
    auto run_controller_test = [] (sstables::compaction_strategy_type compaction_strategy_type, test_env& env) {
        ...
        auto add_sstable = [&env, gen = sstables::generation_factory()] (table_for_tests& t, uint64_t data_size, int level) mutable {
            auto sst = env.make_sstable(t.schema(), gen());
...
SEASTAR_TEST_CASE(test_large_partition_splitting_on_compaction) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        for (auto& sst : ret.new_sstables) {
            sst = env.reusable_sst(s, sst->generation()).get0();
