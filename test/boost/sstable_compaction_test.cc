...
SEASTAR_TEST_CASE(mv_tombstone_purge_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto make_delete_row = [&] (partition_key key, int32_t ck = 0, gc_clock::time_point deletion_time = gc_clock::now(), std::optional<api::timestamp_type> timestamp = std::nullopt, std::optional<api::timestamp_type> deleted_at = std::nullopt) {
            mutation m(s, key);
            if (!timestamp) {
                deleted_at = timestamp = next_timestamp();
            } else if (!deleted_at) {
...

        {
            ...
            auto mut2 = make_delete_row(alpha, 1, gc_clock::now(), api::timestamp_type(1), api::timestamp_type(2));
            auto mut3 = make_insert(alpha, 2, 2, api::timestamp_type(1), api::timestamp_type(2));
            auto mut4 = make_delete_row(alpha, 2, gc_clock::now(), api::timestamp_type(1), api::timestamp_type(3));
            auto mut5 = make_insert(alpha, 3, 3, api::timestamp_type(1), api::timestamp_type(3));
