...
static std::vector<sstables::shared_sstable> get_candidates_for_leveled_strategy(replica::column_family& cf) {
    return cf.get_sstables()->sstables();
}
...
static shared_sstable get_sstable(const lw_shared_ptr<replica::column_family>& cf, int64_t generation) {
    return cf->get_sstables()->at(generation);
}
...
SEASTAR_TEST_CASE(partial_sstable_run_filtered_out_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto generation_exists = [&cf] (sstables::generation_type generation) {
            return cf->get_sstables()->contains(generation);
        };
    ...
    return test_env::do_with([] (test_env& env) {
...

        {
            ...
            auto incremental_selection_test = [&] (strategy_param param) {
                auto set1 = make_lw_shared<sstable_set>(sstables::make_partitioned_sstable_set(s, make_lw_shared<sstable_map>(), false));
                auto set2 = make_lw_shared<sstable_set>(sstables::make_partitioned_sstable_set(s, make_lw_shared<sstable_map>(), bool(param)));
...
SEASTAR_TEST_CASE(test_major_does_not_miss_data_in_memtable) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        BOOST_REQUIRE(cf->get_sstables()->size() == 1);
        auto [gen, new_sst] = *(cf->get_sstables()->begin());
