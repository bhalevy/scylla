...
SEASTAR_TEST_CASE(compact) {
    return sstables::test_env::do_with_async([] (sstables::test_env& env) {
        ...
        auto sst = env.reusable_sst(s, new_sstables[0]->generation()).get();
        ...
        return verify_mutation([&] (const mutation_opt& m) {
            BOOST_REQUIRE(m);
...
SEASTAR_TEST_CASE(leveled_05) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        for (auto gen : generations) {
            auto fname = sstable::filename(env.tempdir().path().native(), "ks", "cf", sstables::get_highest_sstable_version(), gen, big, component_type::Data);
