...
SEASTAR_TEST_CASE(test_view_row_marker_timestamp_on_non_key_update) {
    ...
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        // The view key cell (v1) is unchanged, so the view row already has
        // the row marker computed from it. The view update must not carry
        // a (stale) row marker.
        BOOST_REQUIRE(!md.HasMember("marker"));
        BOOST_REQUIRE_EQUAL(view_min_memtable_live_row_marker_timestamp(e, "vcf"), api::max_timestamp);

        // Re-setting the view key column to the same value with a newer
        // timestamp does refresh the row marker.
        e.execute_cql("insert into cf (p, v1) values (0, 0) using timestamp 300").get();
        eventually([&] {
            frags = get_view_row_fragments(e, "vcf", "p = 0 and v1 = 0");
            auto it = std::ranges::find_if(frags, [] (auto& kv) { return kv.first.starts_with("memtable"); });
            BOOST_REQUIRE(it != frags.end());
            BOOST_REQUIRE(it->second.HasMember("marker"));
            BOOST_REQUIRE_EQUAL(rjson::get<int64_t>(rjson::get(it->second, "marker"), "timestamp"), 300);
        });
        BOOST_REQUIRE_EQUAL(view_min_memtable_live_row_marker_timestamp(e, "vcf"), 300);
    }, cfg);
}

// End-to-end version of the scenario from CUSTOMER-189: an expired
// shadowable tombstone in a view sstable must be purged by compaction even
// while the view memtable holds an update to another view row of the same
// partition, generated by a base update of a non-key view column. Before
// the fix, such an update re-sent the view row marker with the (old)
// timestamp of the view key column, so the memtable looked like it held
// live data older than the tombstone and the tombstone was never purged.
SEASTAR_TEST_CASE(test_view_shadowable_tombstone_gc_with_non_key_update_in_memtable) {
    cql_test_config cfg;
    cfg.db_config->enable_cache(false);
    return do_with_cql_env_thread([] (cql_test_env& e) {
        e.execute_cql("create table cf (p int, c int, v1 int, v2 int, primary key (p, c)) "
                      "with tombstone_gc = {'mode': 'timeout'}").get();
        e.execute_cql("create materialized view vcf as select * from cf "
                      "where p is not null and c is not null and v1 is not null "
                      "primary key (p, v1, c) with gc_grace_seconds = 0 and tombstone_gc = {'mode': 'timeout'}").get();
        auto I = [] (int32_t v) { return int32_type->decompose(v); };
        auto expect_view = [&] (std::vector<std::vector<bytes_opt>> rows) {
            eventually([&] {
                assert_that(e.execute_cql("select p, v1, c, v2 from vcf").get()).is_rows().with_rows(rows);
            });
        };

        e.execute_cql("insert into cf (p, c, v1, v2) values (0, 1, 0, 0) using timestamp 100").get();
        e.execute_cql("insert into cf (p, c, v1, v2) values (0, 2, 0, 0) using timestamp 120").get();
        expect_view({{I(0), I(0), I(1), I(0)}, {I(0), I(0), I(2), I(0)}});
        // Move the second row's view key: shadowable tombstone on (0, 0, 2).
        e.execute_cql("update cf using timestamp 130 set v1 = 1 where p = 0 and c = 2").get();
        expect_view({{I(0), I(0), I(1), I(0)}, {I(0), I(1), I(2), I(0)}});
        // Let the tombstone expire (gc_grace_seconds = 0) before flushing,
        // so that the memtable created by the flush is newer than the
        // tombstone and does not hold back gc_before via the commitlog
        // check (see tombstone_gc_state::check_min()).
        forward_jump_clocks(std::chrono::seconds(2));
        flush_all(e);
        // Also make sure the following write lands in a fresh commitlog
        // segment: tombstones are not purged while the table has unflushed
        // data in a commitlog segment it started writing to before the
        // tombstone was written (see tombstone_gc_state::check_min()).
        e.db().invoke_on_all([] (replica::database& db) -> future<> {
            co_await db.commitlog()->force_new_active_segment();
            co_await db.flush_all_memtables();
        }).get();
        {
            auto frags = get_view_row_fragments(e, "vcf", "p = 0 and v1 = 0 and c = 2");
            BOOST_REQUIRE_EQUAL(frags.size(), 1);
            BOOST_REQUIRE(frags.begin()->first.starts_with("sstable"));
            BOOST_REQUIRE(frags.begin()->second.HasMember("shadowable_tombstone"));
        }

        // Update a non-key view column of the first row: the update lands in
        // the view memtable, in the same partition as the tombstone.
        e.execute_cql("update cf using timestamp 200 set v2 = 1 where p = 0 and c = 1").get();
        expect_view({{I(0), I(0), I(1), I(1)}, {I(0), I(1), I(2), I(0)}});
        BOOST_REQUIRE_EQUAL(view_min_memtable_live_row_marker_timestamp(e, "vcf"), api::max_timestamp);

        // Major-compact the view without flushing the memtable.
        e.db().invoke_on_all([] (replica::database& db) {
            return db.find_column_family("ks", "vcf").compact_all_sstables(tasks::make_empty_task_info(), replica::table::do_flush::no);
        }).get();

        // The expired shadowable tombstone must be gone.
        auto frags = get_view_row_fragments(e, "vcf", "p = 0 and v1 = 0 and c = 2");
        BOOST_REQUIRE(frags.empty());
        expect_view({{I(0), I(0), I(1), I(1)}, {I(0), I(1), I(2), I(0)}});
    }, cfg);
}

// A base update touching only non-key view columns generates a view update
// carrying only the updated cells. Such view data must survive being
// compacted on its own, i.e. without the sstable holding the view row
// marker: in a view whose key contains a base regular column, a row without
// a live marker has all its cells erased on compaction (see
// dead_marker_shadows_row()), and a missing marker is not a live marker.
SEASTAR_TEST_CASE(test_view_non_key_update_survives_single_sstable_compaction) {
    cql_test_config cfg;
    cfg.db_config->enable_cache(false);
    return do_with_cql_env_thread([] (cql_test_env& e) {
        e.execute_cql("create table cf (p int primary key, v1 int, v2 int)").get();
        ...
        flush_all(e);
        // One sstable holds the row marker, the other only the updated cell.
        auto frags_before = get_view_row_fragments(e, "vcf", "p = 0 and v1 = 0");
        BOOST_REQUIRE_EQUAL(frags_before.size(), 2);

        // Rewrite every sstable on its own, as `nodetool upgradesstables` does.
        e.db().invoke_on_all([] (replica::database& db) -> future<> {
            auto& t = db.find_column_family("ks", "vcf");
            const auto& ks = db.find_keyspace("ks");
            compaction::owned_ranges_ptr owned_ranges;
            if (!ks.get_replication_strategy().is_per_table()) {
                owned_ranges = compaction::make_owned_ranges_ptr(co_await db.get_keyspace_local_ranges(ks.get_static_effective_replication_map()));
            }
            co_await t.parallel_foreach_compaction_group_view([&] (compaction::compaction_group_view& ts) -> future<> {
                co_await t.get_compaction_manager().perform_sstable_upgrade(owned_ranges, ts, false, tasks::make_empty_task_info());
            });
        }).get();

        auto frags = get_view_row_fragments(e, "vcf", "p = 0 and v1 = 0");
        // The sstables must have been rewritten.
        for (const auto& [source, _] : frags) {
            BOOST_REQUIRE(!frags_before.contains(source));
        }
        expect_view({{I(0), I(0), I(1)}});
        BOOST_REQUIRE(std::ranges::any_of(frags, [] (auto& kv) {
            const auto& cols = rjson::get(kv.second, "columns");
            return cols.HasMember("v2") && rjson::get<int64_t>(rjson::get(cols, "v2"), "timestamp") == 200;
        }));
    }, cfg);
}
...
static future<> run_test_view_key_change_after_non_key_update(bool with_flush) {
    cql_test_config cfg;
    cfg.db_config->enable_cache(false);
...
SEASTAR_TEST_CASE(test_view_key_change_after_non_key_update) {
    return run_test_view_key_change_after_non_key_update(false);
}

SEASTAR_TEST_CASE(test_view_key_change_after_non_key_update_with_flush) {
    return run_test_view_key_change_after_non_key_update(true);
}
