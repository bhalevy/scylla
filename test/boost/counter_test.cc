...
SEASTAR_TEST_CASE(test_transfer_updates_to_shards) {
    return seastar::async([] {
        ...
        auto m0 = m1;
        transform_counter_updates_to_shards(m0, nullptr, 0, host_id::create_null_id());

        auto empty = mutation(s, pk);
        auto m = m1;
        transform_counter_updates_to_shards(m, &empty, 0, host_id::create_null_id());
        BOOST_REQUIRE_EQUAL(m, m0);
        ...
        BOOST_REQUIRE(ac.is_live());
      {
        ...
        m = m2;
        transform_counter_updates_to_shards(m, &m0, 0, host_id::create_null_id());
