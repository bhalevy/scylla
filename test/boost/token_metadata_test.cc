...
namespace {
    ...
    mutable_keyspace_erm_ptr create_erm(mutable_token_metadata_ptr tmptr, replication_strategy_config_options opts = {}) {
...
SEASTAR_THREAD_TEST_CASE(test_endpoints_for_reading_when_bootstrap_with_replicas) {
    ...
    auto check_endpoints = [](mutable_keyspace_erm_ptr erm, int64_t t,
    ...
    auto check_no_endpoints = [](mutable_keyspace_erm_ptr erm, int64_t t,
        seastar::compat::source_location sl = seastar::compat::source_location::current())
    {
        BOOST_TEST_INFO("line: " << sl.line());
