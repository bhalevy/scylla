...
SEASTAR_TEST_CASE(test_host_id_or_endpoint_equality) {
    ...
    // Test that without valid host_id on both, we compare both the host_id and address, in a symmetrical way
    v0.id = values[2].id;
    BOOST_REQUIRE(v0.id);
    BOOST_REQUIRE(!v1.id);
    BOOST_REQUIRE(v0 != v1);
    BOOST_REQUIRE(v1 != v0);
    // Test that with valid host_id on both, we compare both the host_id and address, in a symmetrical way
    v1.id = values[3].id;
    BOOST_REQUIRE(v0.id);
    BOOST_REQUIRE(v1.id);
    ...
    BOOST_REQUIRE(v0 != v1);
    BOOST_REQUIRE(v1 != v0);
    // Test that with equal host_id and endpoint, the variables are equal, in a symmetrical way
    v1 = v0;
    BOOST_REQUIRE(v0 == v1);
    BOOST_REQUIRE(v1 == v0);
    // Test that with equal host_id on both, we compare both the host_id and address, in a symmetrical way
