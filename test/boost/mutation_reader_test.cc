...
SEASTAR_TEST_CASE(test_combining_two_readers_with_the_same_row) {
    return seastar::async([] {
        ...
        assert_that(make_combined_reader(s, permit, make_flat_mutation_reader_from_mutation_v2(s, permit, std::move(m1)), make_flat_mutation_reader_from_mutations_v2(s, permit, {m2})))
            .produces(m2)
...
SEASTAR_TEST_CASE(test_combining_two_non_overlapping_readers) {
    return seastar::async([] {
        ...
        auto cr = make_combined_reader(s, permit, make_flat_mutation_reader_from_mutation_v2(s, permit, mutation(m1)), make_flat_mutation_reader_from_mutation_v2(s, permit, mutation(m2)));
...
SEASTAR_THREAD_TEST_CASE(combined_reader_galloping_within_partition_test) {
    ...
    std::vector<flat_mutation_reader_v2> v;
    v.push_back(make_flat_mutation_reader_from_mutation_v2(s.schema(), permit, mutation(make_partition(boost::irange(0, 5)))));
    v.push_back(make_flat_mutation_reader_from_mutation_v2(s.schema(), permit, mutation(make_partition(boost::irange(5, 10)))));
    ...
    return seastar::async([] {
        ...
        auto make_combined_populator = [&semaphore] (int n_sources) mutable {
            return [=, &semaphore] (schema_ptr s, const std::vector<mutation>& muts) mutable {
                ...
                for (auto&& m : muts) {
                    auto rd = make_flat_mutation_reader_from_mutation_v2(s, semaphore.make_permit(), mutation(m));
                    auto close_rd = deferred_close(rd);
...
static reader_bounds make_reader_bounds(
    ...
    return reader_bounds {
        .r = mb.m ? make_flat_mutation_reader_from_mutation_v2(s, permit, std::move(*mb.m), *slice, fwd)
...
SEASTAR_THREAD_TEST_CASE(test_clustering_order_merger_in_memory) {
    ...
    auto make_authority = [s = g._s, &semaphore] (std::optional<mutation> mut, streamed_mutation::forwarding fwd) {
        auto permit = semaphore.make_permit();
        if (mut) {
            return make_flat_mutation_reader_from_mutation_v2(s, permit, std::move(*mut), fwd);
        }
...
static future<> do_test_clustering_order_merger_sstable_set(bool reversed) {
  return sstables::test_env::do_with_async([reversed] (sstables::test_env& env) {
    ...
    auto make_authority = [&env, &query_schema, &query_slice] (mutation mut, streamed_mutation::forwarding fwd) {
        return make_flat_mutation_reader_from_mutation_v2(query_schema, env.make_reader_permit(), std::move(mut), query_slice, fwd);
    };
