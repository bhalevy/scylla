...
SEASTAR_TEST_CASE(test_combining_two_readers_with_the_same_row) {
    return seastar::async([] {
        ...
        assert_that(make_combined_reader(s, test_semaphore.make_permit(), flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m1}), flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m2})))
            .produces(m2)
...
SEASTAR_TEST_CASE(test_combining_two_non_overlapping_readers) {
    return seastar::async([] {
        ...
        auto cr = make_combined_reader(s, test_semaphore.make_permit(), flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m1}), flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m2}));
...
SEASTAR_TEST_CASE(test_combining_two_partially_overlapping_readers) {
    return seastar::async([] {
        ...
        assert_that(make_combined_reader(s, test_semaphore.make_permit(), flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m1, m2}), flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m2, m3})))
            .produces(m1)
            .produces(m2)
            .produces(m3)
            .produces_end_of_stream();
...
SEASTAR_TEST_CASE(test_combining_one_reader_with_many_partitions) {
    return seastar::async([] {
        ...
        v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m1, m2, m3}));
        assert_that(make_combined_reader(s, test_semaphore.make_permit(), std::move(v), streamed_mutation::forwarding::no, mutation_reader::forwarding::no))
            .produces(m1)
            .produces(m2)
            .produces(m3)
            .produces_end_of_stream();
...
SEASTAR_THREAD_TEST_CASE(combined_reader_galloping_within_partition_test) {
    ...
    std::vector<flat_mutation_reader> v;
    v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {make_partition(boost::irange(0, 5))}));
    v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {make_partition(boost::irange(5, 10))}));
    assert_that(make_combined_reader(s.schema(), test_semaphore.make_permit(), std::move(v), streamed_mutation::forwarding::no, mutation_reader::forwarding::no))
...
SEASTAR_THREAD_TEST_CASE(combined_mutation_reader_galloping_over_multiple_partitions_test) {
    ...
    std::vector<flat_mutation_reader> v;
    v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {
...
SEASTAR_TEST_CASE(test_filtering) {
    return seastar::async([] {
        ...
        // All pass
        assert_that(make_filtering_reader(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m1, m2, m3, m4}),
...
SEASTAR_TEST_CASE(test_combining_two_readers_with_one_reader_empty) {
    return seastar::async([] {
        ...
        assert_that(make_combined_reader(s, test_semaphore.make_permit(), flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m1}), make_empty_flat_reader(s, test_semaphore.make_permit())))
            .produces(m1)
            .produces_end_of_stream();
    });
}
...
SEASTAR_TEST_CASE(test_combining_two_empty_readers) {
    return seastar::async([] {
        reader_concurrency_semaphore_for_tests test_semaphore;
        auto s = make_schema();
        assert_that(make_combined_reader(s, test_semaphore.make_permit(), make_empty_flat_reader(s, test_semaphore.make_permit()), make_empty_flat_reader(s, test_semaphore.make_permit())))
            .produces_end_of_stream();
    });
}
...
SEASTAR_TEST_CASE(test_combining_one_empty_reader) {
    return seastar::async([] {
        ...
        reader_concurrency_semaphore_for_tests test_semaphore;
        auto s = make_schema();
        v.push_back(make_empty_flat_reader(s, test_semaphore.make_permit()));
        assert_that(make_combined_reader(s, test_semaphore.make_permit(), std::move(v), streamed_mutation::forwarding::no, mutation_reader::forwarding::no))
            .produces_end_of_stream();
    });
}
...
SEASTAR_TEST_CASE(test_fast_forwarding_combining_reader) {
    return seastar::async([] {
        ...
        auto make_reader = [&] (const dht::partition_range& pr) {
            return make_combined_reader(s, test_semaphore.make_permit(), ranges::to<std::vector<flat_mutation_reader>>(mutations | boost::adaptors::transformed([&pr, &test_semaphore] (auto& ms) {
                return flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {ms}, pr);
            })));
        };
...
SEASTAR_THREAD_TEST_CASE(test_fast_forwarding_combining_reader_with_galloping) {
    ...
    std::vector<flat_mutation_reader> v;
    v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), make_n_mutations(boost::irange(0, 5), 7), pr));
    v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), make_n_mutations(boost::irange(5, 10), 7), pr));

    assert_that(make_combined_reader(s.schema(), test_semaphore.make_permit(), std::move(v), streamed_mutation::forwarding::no, mutation_reader::forwarding::yes))
...
SEASTAR_TEST_CASE(test_sm_fast_forwarding_combining_reader) {
    return seastar::async([] {
        ...
        auto& test_semaphore = ssft.local_test_semaphore();
        auto s = test_semaphore.make_simple_schema();
        ...
        for (auto& mutations : readers_mutations) {
            readers.emplace_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), mutations, streamed_mutation::forwarding::yes));
        }

        assert_that(make_combined_reader(s.schema(), test_semaphore.make_permit(), std::move(readers), streamed_mutation::forwarding::yes, mutation_reader::forwarding::no))
...
SEASTAR_THREAD_TEST_CASE(test_sm_fast_forwarding_combining_reader_with_galloping) {
    ...
    auto pr = dht::partition_range::make(ring[0], ring[0]);
    std::vector<flat_mutation_reader> v;
    v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), make_n_mutations(boost::irange(0, 5), 3), streamed_mutation::forwarding::yes));
    v.push_back(flat_mutation_reader_from_mutations(test_semaphore.make_permit(), make_n_mutations(boost::irange(5, 10), 3), streamed_mutation::forwarding::yes));

    auto reader = make_combined_reader(s.schema(), test_semaphore.make_permit(), std::move(v), streamed_mutation::forwarding::yes, mutation_reader::forwarding::no);
...
SEASTAR_THREAD_TEST_CASE(combined_mutation_reader_test) {
  sstables::test_env::do_with_async([] (sstables::test_env& env) {
    ...
    auto list_reader = make_combined_reader(s.schema(), test_semaphore.make_permit(),
...
class dummy_incremental_selector : public reader_selector {
    ...
    flat_mutation_reader pop_reader() {
        ...
        return flat_mutation_reader_from_mutations(_semaphore.make_permit(), std::move(muts), _pr, _fwd);
    }
public:
    ...
    dummy_incremental_selector(reader_concurrency_semaphore_for_tests& semaphore,
            schema_ptr s,
...
SEASTAR_TEST_CASE(reader_selector_gap_between_readers_test) {
    return seastar::async([] {
        ...
        auto reader = make_combined_reader(s.schema(), test_semaphore.make_permit(),
                std::make_unique<dummy_incremental_selector>(test_semaphore, s.schema(), std::move(readers_mutations)),
                streamed_mutation::forwarding::no,
                mutation_reader::forwarding::no);
...
SEASTAR_TEST_CASE(reader_selector_fast_forwarding_test) {
    return seastar::async([] {
        ...
        auto reader = make_combined_reader(s.schema(), test_semaphore.make_permit(),
                std::make_unique<dummy_incremental_selector>(test_semaphore, s.schema(),
...
public:
    reader_wrapper(
        ...
        : _reader(make_empty_flat_reader(schema, tests::make_permit(semaphore)))
        , _timeout(timeout)
    {
...
SEASTAR_TEST_CASE(restricted_reader_reading) {
    return sstables::test_env::do_with_async([&] (sstables::test_env& env) {
        ...
        {
            auto s = simple_schema(semaphore);
            auto tmp = tmpdir();
...
SEASTAR_TEST_CASE(test_fast_forwarding_combined_reader_is_consistent_with_slicing) {
    return sstables::test_env::do_with_async([&] (sstables::test_env& env) {
        ...
        flat_mutation_reader rd = make_combined_reader(s, test_semaphore.make_permit(), std::move(readers),
            streamed_mutation::forwarding::yes,
            mutation_reader::forwarding::yes);
...
SEASTAR_TEST_CASE(test_combined_reader_slicing_with_overlapping_range_tombstones) {
    return sstables::test_env::do_with_async([&] (sstables::test_env& env) {
...

        {
            ...
            readers.push_back(ds1.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, slice));
            readers.push_back(ds2.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, slice));

            auto rd = make_combined_reader(s, test_semaphore.make_permit(), std::move(readers),
        ...
        // Check fast_forward_to()
        {
            ...
            readers.push_back(ds1.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, s->full_slice(), default_priority_class(),
                nullptr, streamed_mutation::forwarding::yes));
            readers.push_back(ds2.make_reader(s, test_semaphore.make_permit(), query::full_partition_range, s->full_slice(), default_priority_class(),
                nullptr, streamed_mutation::forwarding::yes));
...
SEASTAR_TEST_CASE(test_combined_mutation_source_is_a_mutation_source) {
    return seastar::async([] {
        ...
        auto make_combined_populator = [&test_semaphore] (int n_sources) {
            return [n_sources, &test_semaphore] (schema_ptr s, const std::vector<mutation>& muts) {
                ...
                int source_index = 0;
                for (auto&& m : muts) {
                    auto rd = flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {m});
                    auto close_rd = deferred_close(rd);
                    rd.consume_pausable([&] (mutation_fragment&& mf) {
...
SEASTAR_THREAD_TEST_CASE(test_foreign_reader_as_mutation_source) {
    ...
    do_with_cql_env([] (cql_test_env& env) -> future<> {
        auto populate = [&env] (schema_ptr s, const std::vector<mutation>& mutations) {
            auto& test_semaphore = env.local_test_semaphore();
            ...
            auto reader_factory = [remote_shard, remote_mt = std::move(remote_mt), &env] (schema_ptr s,
                    const dht::partition_range& range,
                    const query::partition_slice& slice,
                    const io_priority_class& pc,
                ...
                auto remote_reader = smp::submit_to(remote_shard,
                    ...
                    return make_foreign(std::make_unique<flat_mutation_reader>(remote_mt->make_flat_reader(s.get(),
                            env.local_test_semaphore().make_permit(),
                            range,
                            slice,
                            pc,
                            trace_state.get(),
                ...
                return make_foreign_reader(s, env.local_test_semaphore().make_permit(), std::move(remote_reader), fwd_sm);
            };
...
SEASTAR_THREAD_TEST_CASE(test_multishard_combining_reader_reading_empty_table) {
    ...
    do_with_cql_env([&] (cql_test_env& env) -> future<> {
        ...
        auto s = env.local_test_semaphore().make_simple_schema();
        auto factory = [&shards_touched, &env] (
                schema_ptr s,
                const dht::partition_range& range,
                const query::partition_slice& slice,
                const io_priority_class& pc,
            ...
            return make_empty_flat_reader(s, env.local_test_semaphore().make_permit());
        };
...
private:
    ...
    bool maybe_push_next_partition() {
        ...
        push_mutation_fragment(*_s.schema(), _semaphore.make_permit(), partition_start(_s.make_pkey(_pkeys.at(_partition_index++)), {}));
        return true;
    }
    ...
    void do_fill_buffer() {
        ...
        while (!is_buffer_full()) {
            push_mutation_fragment(*_s.schema(), _semaphore.make_permit(), _s.make_row(_s.make_ckey(ck++), make_random_string(2 << 5)));
        }

        push_mutation_fragment(*_s.schema(), _semaphore.make_permit(), partition_end());
    }

public:
    puppet_reader(reader_concurrency_semaphore_for_tests& semaphore, simple_schema s, control& ctrl, std::vector<fill_buffer_action> actions, std::vector<uint32_t> pkeys)
        : impl(s.schema(), semaphore.make_permit())
        , _semaphore(semaphore)
        , _s(std::move(s))
        , _ctrl(ctrl)
...
SEASTAR_THREAD_TEST_CASE(test_stopping_reader_with_pending_read_ahead) {
    ...
    do_with_cql_env([] (cql_test_env& env) -> future<> {
        ...
        auto s = test_semaphore.make_simple_schema();
        auto remote_control_remote_reader = smp::submit_to(shard_of_interest, [&env] {
            ...
            auto reader = make_foreign(std::make_unique<flat_mutation_reader>(make_flat_mutation_reader<puppet_reader>(env.local_test_semaphore(),
                    env.local_test_semaphore().make_simple_schema(),
                    *control,
        ...
        auto& remote_reader = std::get<1>(remote_control_remote_reader);
            ...
            auto reader = make_foreign_reader(s.schema(), test_semaphore.make_permit(), std::move(remote_reader));
...
multishard_reader_for_read_ahead prepare_multishard_reader_for_read_ahead_test(cql_test_env& env, test_reader_lifecycle_policy::operations_gate& operations_gate) {
    auto& test_semaphore = env.local_test_semaphore();
    ...
    auto factory = [&env, remote_controls = std::move(remote_control_refs), shard_pkeys = std::move(shard_pkeys)] (
            schema_ptr,
            const dht::partition_range& range,
            const query::partition_slice& slice,
            const io_priority_class& pc,
        ...
        auto& test_semaphore = env.local_test_semaphore();
        return make_flat_mutation_reader<puppet_reader>(test_semaphore, test_semaphore.make_simple_schema(), *remote_controls.at(shard), std::move(actions), shard_pkeys.at(shard));
    };
    ...
    auto reader = make_multishard_combining_reader_for_tests(*sharder, seastar::make_shared<test_reader_lifecycle_policy>(std::move(factory), operations_gate),
            s.schema(), test_semaphore.make_permit(), *pr, s.schema()->full_slice(), service::get_local_sstable_query_read_priority());
...
SEASTAR_THREAD_TEST_CASE(test_multishard_combining_reader_destroyed_with_pending_read_ahead) {
    ...
    do_with_cql_env([&] (cql_test_env& env) -> future<> {
        auto reader_sharder_remote_controls__ = prepare_multishard_reader_for_read_ahead_test(env, operations_gate);
        ...
        // Destroy reader.
        reader.reassign(flat_mutation_reader(nullptr)).get();
...
SEASTAR_THREAD_TEST_CASE(test_multishard_combining_reader_fast_forwarded_with_pending_read_ahead) {
    ...
    do_with_cql_env([&] (cql_test_env& env) -> future<> {
        auto reader_sharder_remote_controls_pr = prepare_multishard_reader_for_read_ahead_test(env, operations_gate);
...
SEASTAR_THREAD_TEST_CASE(test_multishard_combining_reader_next_partition) {
    ...
    do_with_cql_env([&] (cql_test_env& env) -> future<> {
        ...
        auto& test_semaphore = env.local_test_semaphore();
        auto factory = [db = &env.db(), max_buffer_size, &test_semaphore] (
                schema_ptr schema,
                const dht::partition_range& range,
                const query::partition_slice& slice,
                const io_priority_class& pc,
...
std::deque<mutation_fragment> make_fragments_with_non_monotonic_positions(reader_concurrency_semaphore_for_tests& test_semaphore, dht::decorated_key pkey, size_t max_buffer_size,
        gc_clock::time_point tombstone_deletion_time) {
    auto s = test_semaphore.make_simple_schema();
    std::deque<mutation_fragment> fragments;

    fragments.emplace_back(*s.schema(), test_semaphore.make_permit(), partition_start{std::move(pkey), {}});
    ...
    for (int buffers = 0; buffers < 2; ++buffers) {
        while (mem_usage < max_buffer_size) {
            fragments.emplace_back(*s.schema(), test_semaphore.make_permit(),
    ...
    fragments.emplace_back(*s.schema(), test_semaphore.make_permit(), s.make_row(s.make_ckey(0), "v"));
...
SEASTAR_THREAD_TEST_CASE(test_multishard_combining_reader_non_strictly_monotonic_positions) {
    ...
    {
        auto fragments = make_fragments_with_non_monotonic_positions(test_semaphore, s.make_pkey(pk), max_buffer_size, tombstone_deletion_time);
        auto rd = make_flat_mutation_reader_from_fragments(s.schema(), test_semaphore.make_permit(), std::move(fragments));
        auto close_rd = deferred_close(rd);
    ...
    do_with_cql_env([=, &operations_gate, s = std::move(s)] (cql_test_env& env) mutable -> future<> {
        auto factory = [=, &env] (
            ...
            if (s.schema()->get_sharder().shard_of(pkey.token()) != this_shard_id()) {
                return make_empty_flat_reader(s.schema(), test_semaphore.make_permit());
            }
            auto fragments = make_fragments_with_non_monotonic_positions(env.local_test_semaphore(), std::move(pkey), max_buffer_size, tombstone_deletion_time);
            auto rd = make_flat_mutation_reader_from_fragments(s.schema(), test_semaphore.make_permit(), std::move(fragments), range, slice);
            rd.set_max_buffer_size(max_buffer_size);
            return rd;
        };
...
SEASTAR_THREAD_TEST_CASE(test_multishard_streaming_reader) {
    ...
    do_with_cql_env([&] (cql_test_env& env) -> future<> {
        ...
        auto reader_factory = [db = &env.db(), &env] (
                schema_ptr s,
                const dht::partition_range& range,
                const query::partition_slice& slice,
                const io_priority_class& pc,
            ...
            return table.as_mutation_source().make_reader(std::move(s), env.local_test_semaphore().make_permit(), range, slice, pc, std::move(trace_state),
        ...
        auto reference_reader = make_filtering_reader(
                make_multishard_combining_reader(seastar::make_shared<test_reader_lifecycle_policy>(std::move(reader_factory), operations_gate),
                    schema, test_semaphore.make_permit(), partition_range, schema->full_slice(), service::get_local_sstable_query_read_priority()),
...
SEASTAR_THREAD_TEST_CASE(test_queue_reader) {
    ...
    // Simultaneous read and write
    {
        ...
        auto write_all = [&test_semaphore] (queue_reader_handle& handle, const std::vector<mutation>& muts) {
            return async([&] {
                auto reader = flat_mutation_reader_from_mutations(test_semaphore.make_permit(), muts);
                auto close_reader = deferred_close(reader);
        ...
        auto p = make_queue_reader(gen.schema(), test_semaphore.make_permit());
    ...
    // abort() -- check that consumer is aborted
    {
        ...
        auto expected_reader = flat_mutation_reader_from_mutations(test_semaphore.make_permit(), expected_muts);
        ...
        BOOST_REQUIRE_THROW(handle.push(mutation_fragment(*gen.schema(), test_semaphore.make_permit(), partition_end{})).get(), std::runtime_error);
        BOOST_REQUIRE(!reader.is_end_of_stream());
    }
    ...
    // Life-cycle, relies on ASAN for error reporting
    {
        ...
        auto close_reader = deferred_close(reader);
        {
            auto throwaway_p = make_queue_reader(gen.schema(), test_semaphore.make_permit());
            ...
            auto another_throwaway_p = make_queue_reader(gen.schema(), test_semaphore.make_permit());
...
SEASTAR_THREAD_TEST_CASE(test_compacting_reader_as_mutation_source) {
    reader_concurrency_semaphore_for_tests test_semaphore;
    auto make_populate = [&test_semaphore] (bool single_fragment_buffer) {
        return [single_fragment_buffer, &test_semaphore] (schema_ptr s, const std::vector<mutation>& mutations, gc_clock::time_point query_time) mutable {
            auto mt = make_lw_shared<memtable>(s);
            for (auto& mut : mutations) {
                mt->apply(mut);
            }
            return mutation_source([=, &test_semaphore] (
                    schema_ptr s,
                    reader_permit,
                    const dht::partition_range& range,
                    const query::partition_slice& slice,
                    ...
                    streamed_mutation::forwarding fwd_sm,
                    mutation_reader::forwarding fwd_mr) mutable {
                auto source = mt->make_flat_reader(s, test_semaphore.make_permit(), range, slice, pc, std::move(trace_state), streamed_mutation::forwarding::no, fwd_mr);
...
SEASTAR_THREAD_TEST_CASE(test_compacting_reader_next_partition) {
    reader_concurrency_semaphore_for_tests test_semaphore;
    auto ss = test_semaphore.make_simple_schema(simple_schema::with_static::no);
    ...
    auto mr = [&] () {
        ...
        mfs.emplace_back(*ss.schema(), test_semaphore.make_permit(), partition_start(dk0, tombstone{}));

        auto i = 0;
        size_t mfs_size = 0;
        while (mfs_size <= buffer_size) {
            mfs.emplace_back(*ss.schema(), test_semaphore.make_permit(), ss.make_row(ss.make_ckey(i++), "v"));
            mfs_size += mfs.back().memory_usage();
        }
        mfs.emplace_back(*ss.schema(), test_semaphore.make_permit(), partition_end{});

        mfs.emplace_back(*ss.schema(), test_semaphore.make_permit(), partition_start(dk1, tombstone{}));
        mfs.emplace_back(*ss.schema(), test_semaphore.make_permit(), ss.make_row(ss.make_ckey(0), "v"));
        mfs.emplace_back(*ss.schema(), test_semaphore.make_permit(), partition_end{});

        for (const auto& mf : mfs) {
            expected.emplace_back(*ss.schema(), test_semaphore.make_permit(), mf);
        }

        auto mr = make_compacting_reader(make_flat_mutation_reader_from_fragments(ss.schema(), test_semaphore.make_permit(), std::move(mfs)),
...
SEASTAR_THREAD_TEST_CASE(test_manual_paused_evictable_reader_is_mutation_source) {
    ...
    public:
        maybe_pausing_reader(
                ...
                tracing::trace_state_ptr trace_state,
                mutation_reader::forwarding fwd_mr)
            : impl(mt.schema(), permit), _reader(nullptr) {
...
SEASTAR_THREAD_TEST_CASE(test_evictable_reader_trim_range_tombstones) {
    reader_concurrency_semaphore_for_tests test_semaphore;
    auto s = test_semaphore.make_simple_schema();
    auto permit = test_semaphore.make_permit(s.schema().get(), get_name());
...
SEASTAR_THREAD_TEST_CASE(test_clustering_order_merger_in_memory) {
    ...
    auto make_authority = [&test_semaphore] (mutation mut, streamed_mutation::forwarding fwd) {
        return flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {std::move(mut)}, fwd);
    };

    auto make_tested = [s = g._s, &test_semaphore] (std::vector<mutation_bounds> ms, streamed_mutation::forwarding fwd) {
        auto rs = boost::copy_range<std::vector<reader_bounds>>(std::move(ms)
                | boost::adaptors::transformed([fwd, &test_semaphore] (auto&& mb) {
                    return reader_bounds{
                        flat_mutation_reader_from_mutations(test_semaphore.make_permit(), {std::move(mb.m)}, fwd),
                        std::move(mb.lower), std::move(mb.upper)};
                }));
        auto q = std::make_unique<simple_position_reader_queue>(*s, std::move(rs));
        return make_clustering_combined_reader(s, test_semaphore.make_permit(), fwd, std::move(q));
    };
    ...
    for (auto fwd: {streamed_mutation::forwarding::no, streamed_mutation::forwarding::yes}) {
        auto r = make_clustering_combined_reader(g._s, test_semaphore.make_permit(), fwd,
...
SEASTAR_THREAD_TEST_CASE(test_clustering_order_merger_sstable_set) {
  sstables::test_env::do_with_async([] (sstables::test_env& env) {
    ...
    auto make_tested = [s = g._s, pr = dht::partition_range::make_singular(dht::ring_position(g.decorated_pk())), &test_semaphore]
        ...
        auto q = sst_set.make_min_position_reader_queue(
            [s, &pr, fwd, &test_semaphore] (sstable& sst) {
                return sst.make_reader(s, test_semaphore.make_permit(), pr,
...
SEASTAR_THREAD_TEST_CASE(clustering_combined_reader_mutation_source_test) {
    ...
    reader_concurrency_semaphore_for_tests test_semaphore;
    auto populate = [&test_semaphore] (schema_ptr s, const std::vector<mutation>& muts) {
