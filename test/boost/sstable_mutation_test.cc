  ...
  test_env::do_with_async([] (test_env& env) {
    env.reusable_sst(uncompressed_schema(), uncompressed_dir(), 1).then([] (auto sstp) {
        return do_with(make_dkey(uncompressed_schema(), "invalid_key"), [sstp] (auto& key) {
            ...
            return with_flat_mutation_reader(sstp->read_row_flat(s, tests::make_permit(), key), [sstp, s, &key] (flat_mutation_reader& rd) {
              return rd(db::no_timeout).then([sstp, s, &key] (auto mutation) {
...
future<> test_no_clustered(sstables::test_env& env, bytes&& key, std::unordered_map<bytes, data_value> &&map) {
    return env.reusable_sst(uncompressed_schema(), uncompressed_dir(), 1).then([k = std::move(key), map = std::move(map)] (auto sstp) mutable {
        return do_with(make_dkey(uncompressed_schema(), std::move(k)), [sstp, map = std::move(map)] (auto& key) {
            auto s = uncompressed_schema();
            return with_flat_mutation_reader(sstp->read_row_flat(s, tests::make_permit(), key), [sstp, s, &key, map = std::move(map)] (flat_mutation_reader& rd) mutable {
              return read_mutation_from_flat_mutation_reader(rd, db::no_timeout).then([sstp, s, &key, map = std::move(map)] (auto mutation) {
                BOOST_REQUIRE(mutation);
...
future<mutation> generate_clustered(sstables::test_env& env, bytes&& key) {
    return env.reusable_sst(complex_schema(), "test/resource/sstables/complex", Generation).then([k = std::move(key)] (auto sstp) mutable {
        return do_with(make_dkey(complex_schema(), std::move(k)), [sstp] (auto& key) {
            ...
            return with_flat_mutation_reader(sstp->read_row_flat(s, tests::make_permit(), key), [sstp, s, &key] (flat_mutation_reader& rd) {
              return read_mutation_from_flat_mutation_reader(rd, db::no_timeout).then([sstp, s, &key] (auto mutation) {
                BOOST_REQUIRE(mutation);
...
future<> test_range_reads(sstables::test_env& env, const dht::token& min, const dht::token& max, std::vector<bytes>& expected) {
    return env.reusable_sst(uncompressed_schema(), uncompressed_dir(), 1).then([min, max, &expected] (auto sstp) mutable {
        ...
        return do_with(dht::partition_range::make(dht::ring_position::starting_at(min),
          ...
          return with_flat_mutation_reader(sstp->read_range_rows_flat(s, tests::make_permit(), pr), [&, sstp] (flat_mutation_reader& mutations) {
...
SEASTAR_TEST_CASE(test_sstable_can_write_and_read_range_tombstone) {
    return seastar::async([] {
      test_env::do_with_async([] (test_env& env) {
        ...
        sst->load().get();
        auto mut = with_flat_mutation_reader(sst->read_rows_flat(s, tests::make_permit()), [] (flat_mutation_reader& mr) {
...
SEASTAR_THREAD_TEST_CASE(broken_ranges_collection) {
  test_env::do_with_async([] (test_env& env) {
    env.reusable_sst(peers_schema(), "test/resource/sstables/broken_ranges", 2).then([] (auto sstp) {
        auto s = peers_schema();
        return with_flat_mutation_reader(sstp->as_mutation_source().make_reader(s, tests::make_permit(), query::full_partition_range), [s] (flat_mutation_reader& reader) {
...
SEASTAR_THREAD_TEST_CASE(tombstone_in_tombstone) {
  test_env::do_with_async([] (test_env& env) {
    ka_sst(env, tombstone_overlap_schema(), "test/resource/sstables/tombstone_overlap", 1).then([] (auto sstp) {
        ...
        return with_flat_mutation_reader(sstp->read_rows_flat(s, tests::make_permit()), [sstp, s] (auto& reader) {
...
SEASTAR_THREAD_TEST_CASE(test_large_index_pages_do_not_cause_large_allocations) {
  test_env::do_with_async([] (test_env& env) {
    ...
    mutation expected = *with_flat_mutation_reader(mt->make_flat_reader(s, tests::make_permit(), pr), [] (flat_mutation_reader& mt_reader) {
    ...
    mutation actual = *with_flat_mutation_reader(sst->as_mutation_source().make_reader(s, tests::make_permit(), pr), [] (flat_mutation_reader& sst_reader) {
