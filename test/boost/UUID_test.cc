...
BOOST_AUTO_TEST_CASE(test_get_time_uuid_v1) {
    ...
    BOOST_CHECK(uuid.is_timestamp_v1());
...
BOOST_AUTO_TEST_CASE(test_get_time_uuid_v7) {
    ...
    BOOST_CHECK(uuid.is_timestamp_v7());
...
BOOST_AUTO_TEST_CASE(test_timeuuid_v7_msb_is_monotonic) {
    ...
    auto tri_compare = [] (const UUID& u1, const UUID& u2) {
        ...
        auto res2 = utils::timeuuid_cmp(b1, b2).tri_compare();
        assert(res0 == res2);
    ...
    for (int64_t *scale = scale_list; *scale; scale++) {
        ...
        for (int64_t i = step + (*scale == 1); i < 3697; i += step) {
            ...
            bool t2 = c2 > 0;
            if (!t1 || !t2) {
                BOOST_CHECK_MESSAGE(t1 && t2, seastar::format("a UUID_v7 {}{} later is not greater than at test start: <=>prev={} <=>first={}", i, str(scale), to_string(c1), to_string(c2)));
                BOOST_CHECK_MESSAGE(t1 && t2, seastar::format("first={} ({:016x}:{:016x} ts={})", first, (uint64_t)first.get_most_significant_bits(), (uint64_t)first.get_least_significant_bits(), first.timestamp()));
                BOOST_CHECK_MESSAGE(t1 && t2, seastar::format(" prev={} ({:016x}:{:016x} ts={})", prev, (uint64_t)prev.get_most_significant_bits(), (uint64_t)prev.get_least_significant_bits(), prev.timestamp()));
                BOOST_CHECK_MESSAGE(t1 && t2, seastar::format(" next={} ({:016x}:{:016x} ts={})", next, (uint64_t)next.get_most_significant_bits(), (uint64_t)next.get_least_significant_bits(), next.timestamp()));
                ...
                BOOST_CHECK_MESSAGE(t1 && t2, seastar::format("first_timestamp=0x{:x} prev_timestamp=0x{:x} next_timestamp=0x{:x} next_decimicros=0x{:x} {} millis={} nanos={} sub_millis={}",
