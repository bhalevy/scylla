...
void topology::test_compare_endpoints(const inet_address& address, const inet_address& a1, const inet_address& a2) const {
    std::optional<std::strong_ordering> expected;
    const auto& loc = get_location(address);
    const auto& loc1 = get_location(a1);
    const auto& loc2 = get_location(a2);
    if (a1 == a2) {
        expected = std::strong_ordering::equal;
    } else {
        if (a1 == address) {
            expected = std::strong_ordering::less;
        } else if (a2 == address) {
            expected = std::strong_ordering::greater;
        } else {
            if (loc1.dc == loc.dc) {
                if (loc2.dc != loc.dc) {
                    expected = std::strong_ordering::less;
    ...
    testlog.debug("compare_endpoint: address={} [{}/{}] a1={} [{}/{}] a2={} [{}/{}]: res={} expected={} expected_value={}",
            ...
            res, bool(expected), expected.value_or(std::strong_ordering::equivalent));
    if (expected) {
