...
SEASTAR_TEST_CASE(test_float_sum_overflow) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        assert_that(result)
            .is_rows()
            .with_size(1)
            .with_row({float_type->decompose(3.4028234e+38f)});
        ...
        assert_that(result)
            .is_rows()
            .with_size(1)
            .with_row({float_type->decompose(std::numeric_limits<float>::infinity())});
        ...
        assert_that(result)
            .is_rows()
            .with_size(1)
            .with_row({float_type->decompose(-2.4028234e+38f)});
...
SEASTAR_TEST_CASE(test_double_sum_overflow) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        assert_that(result)
            .is_rows()
            .with_size(1)
            .with_row({double_type->decompose(1.79769313486231570814527423732E308)});
        BOOST_TEST_MESSAGE("cause overflow");
        ...
        assert_that(result)
            .is_rows()
            .with_size(1)
            .with_row({double_type->decompose(std::numeric_limits<double>::infinity())});
        BOOST_TEST_MESSAGE("test maximum negative value");
        cquery_nofail(e, "insert into cf (pk, val) values ('a', -1.79769313486231570814527423732e+308);");
        result = e.execute_cql("select sum(val) from cf;").get0();
        assert_that(result)
            .is_rows()
            .with_size(1)
            .with_row({double_type->decompose(-1.29769313486231570814527423732e+308)});
        BOOST_TEST_MESSAGE("cause negative overflow");
