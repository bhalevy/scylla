    ...
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto insert_data = [&] (uint32_t begin, uint32_t end) {
            return parallel_for_each(boost::irange(begin, end), [&] (auto i) -> future<> {
                ...
                auto fm = freeze(m);
                co_await e.db().invoke_on(shard, [&] (replica::database& db) -> future<> {
                    co_await db.apply(s, fm, tracing::trace_state_ptr(), db::commitlog::force_sync::no, db::no_timeout);
                    co_await db.find_column_family(uuid).flush();
                });
                ++count;
            });
        };
        ...
        auto f1 = do_until([&] { return count >= num_keys; }, [&] () -> future<> {
            co_await e.db().invoke_on_all([&, ts = db_clock::now()] (replica::database& db) {
