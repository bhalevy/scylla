...
SEASTAR_TEST_CASE(populate_from_quarantine_works) {
    ...
    co_await do_with_some_data([] (cql_test_env& e) -> future<> {
        co_await e.db().invoke_on_all([] (database& db) {
            auto& cf = db.find_column_family("ks", "cf");
            return cf.flush();
        });
        ...
        for (auto i = 0; i < smp::count && !found; i++) {
            found = co_await e.db().invoke_on((shard + i) % smp::count, [] (database& db) -> future<bool> {
                auto& cf = db.find_column_family("ks", "cf");
    ...
    co_await do_with_some_data([] (cql_test_env& e) -> future<> {
        ...
        BOOST_REQUIRE(rows);
        BOOST_REQUIRE_EQUAL(rows->rs().result_set().size(), 6);
    }, tmpdir_for_data);
}
...
SEASTAR_TEST_CASE(snapshot_with_quarantine_works) {
    return do_with_some_data([] (cql_test_env& e) -> future<> {
        ...
        co_await take_snapshot(e, true /* skip_flush */);
