...
future<> take_snapshot(sharded<replica::database>& db, bool skip_flush = false, sstring ks_name = "ks", sstring cf_name = "cf", sstring snapshot_name = "test", bool snap_views = false) {
    try {
        co_await replica::database::snapshot_table_on_all_shards(db, ks_name, cf_name, snapshot_name, skip_flush, snap_views);
    } catch (...) {
...
SEASTAR_TEST_CASE(drop_table_with_explicit_snapshot) {
    ...
    co_await do_with_some_data({table_name}, [&] (cql_test_env& e) -> future<> {
        ...
        co_await replica::database::snapshot_table_on_all_shards(e.db(), ks_name, table_name, snapshot_tag, false, false);
        auto cf_dir = e.local_db().find_column_family(ks_name, table_name).dir();
