...
SEASTAR_THREAD_TEST_CASE(test_distributed_loader_with_pending_delete) {
    ...
    auto component_basename = [&ks, &cf] (sstables::generation_type::value_type gen, component_type ctype) {
        return sst::component_basename(ks, cf, sst::version_types::mc, generation_from_value(gen), sst::format_types::big, ctype);
    };

    auto gen_filename = [&sst_dir, &ks, &cf] (sstables::generation_type::value_type gen, component_type ctype) {
