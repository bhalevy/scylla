...
SEASTAR_TEST_CASE(clear_multiple_snapshots) {
    ...
    co_await do_with_some_data({table_name}, [&] (cql_test_env& e) {
        ...
        e.local_db().drop_table_on_all(ks_name, table_name, [ts = db_clock::now()] { return make_ready_future<db_clock::time_point>(ts); }).get();
...
SEASTAR_TEST_CASE(database_drop_column_family_clears_querier_cache) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto f = e.local_db().drop_table_on_all("ks", "cf", [ts] { return make_ready_future<db_clock::time_point>(ts); });
...
static future<> test_drop_table_with_auto_snapshot(bool auto_snapshot) {
    ...
    co_await do_with_some_data({table_name}, [&] (cql_test_env& e) -> future<> {
        ...
        co_await e.local_db().drop_table_on_all(ks_name, table_name, [ts = db_clock::now()] { return make_ready_future<db_clock::time_point>(ts); }, true);
...
SEASTAR_TEST_CASE(drop_table_with_no_snapshot) {
    ...
    co_await do_with_some_data({table_name}, [&] (cql_test_env& e) -> future<> {
        ...
        co_await e.local_db().drop_table_on_all(ks_name, table_name, [ts = db_clock::now()] { return make_ready_future<db_clock::time_point>(ts); }, false);
        auto cf_dir_exists = co_await file_exists(cf_dir);
