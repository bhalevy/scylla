  ...
  return sstables::test_env::do_with([] (sstables::test_env& env) {
    ...
    return test_setup::do_with_tmp_directory([s, generation, cf, cm] (test_env& env, sstring tmpdir_path) {
        return open_sstables(env, s, "test/resource/sstables/compaction", {1,2,3}).then([&env, tmpdir_path, s, cf, cm, generation] (auto sstables) {
            ...
            return compact_sstables(sstables::compaction_descriptor(std::move(sstables), std::move(all_sstables), default_priority_class()), *cf, new_sstable).then([&env, s, generation, cf, cm, tmpdir_path] (auto) {
...
static future<std::vector<unsigned long>> compact_sstables(test_env& env, sstring tmpdir_path, std::vector<unsigned long> generations_to_compact,
    ...
    return f.then([&env, generations, sstables, s, create_sstables, min_sstable_size, tmpdir_path] () mutable {
        ...
        if (strategy == compaction_strategy_type::size_tiered) {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            return compact_sstables(sstables::compaction_descriptor(std::move(sstables_to_compact), std::move(all_sstables),
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            return compact_sstables(sstables::compaction_descriptor(std::move(candidate.sstables), std::move(all_sstables),
...
SEASTAR_TEST_CASE(tombstone_purge_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto compact = [&, s] (std::vector<shared_sstable> all, std::vector<shared_sstable> to_compact) -> std::vector<shared_sstable> {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            return compact_sstables(sstables::compaction_descriptor(to_compact, std::move(all_sstables), default_priority_class()), *cf, sst_gen).get0().new_sstables;
        };
...
SEASTAR_TEST_CASE(sstable_rewrite) {
    ...
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        return write_memtable_to_sstable_for_test(*mt, sst).then([&env, s, sst, tmpdir_path] {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            return compact_sstables(sstables::compaction_descriptor(std::move(sstables), std::move(all_sstables), default_priority_class()), *cf, creator).then([s, key, new_tables] (auto) {
    ...
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        return seastar::async([&env, tmpdir_path] {
            for (auto version : all_sstable_versions) {
                ...
                auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
                auto info = compact_sstables(sstables::compaction_descriptor({sst1, sst2}, std::move(all_sstables), default_priority_class()), *cf, creator).get0();
...
SEASTAR_TEST_CASE(compaction_with_fully_expired_table) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
        auto ret = compact_sstables(sstables::compaction_descriptor(ssts, std::move(all_sstables), default_priority_class()), *cf, sst_gen).get0();
...
SEASTAR_TEST_CASE(min_max_clustering_key_test_2) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        for (const auto version : all_sstable_versions) {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            auto info = compact_sstables(sstables::compaction_descriptor({sst, sst2}, std::move(all_sstables), default_priority_class()), *cf, creator).get0();
            BOOST_REQUIRE(info.new_sstables.size() == 1);
...
SEASTAR_TEST_CASE(sstable_expired_data_ratio) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
        auto info = compact_sstables(sstables::compaction_descriptor({ sst }, std::move(all_sstables), default_priority_class()), *cf, creator).get0();
        BOOST_REQUIRE(info.new_sstables.size() == 1);
...
SEASTAR_TEST_CASE(compaction_correctness_with_partitioned_sstable_set) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto compact = [&, s] (std::vector<shared_sstable> all) -> std::vector<shared_sstable> {
            ...
            column_family_for_tests cf(env.manager(), s);
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            return compact_sstables(sstables::compaction_descriptor(std::move(all), std::move(all_sstables), default_priority_class(), 0, 0 /*std::numeric_limits<uint64_t>::max()*/),
...
SEASTAR_TEST_CASE(sstable_cleanup_correctness_test) {
    return do_with_cql_env([] (auto& e) {
        return test_env::do_with_async([&db = e.local_db()] (test_env& env) {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            auto descriptor = sstables::compaction_descriptor({std::move(sst)}, std::move(all_sstables), default_priority_class(), compaction_descriptor::default_level,
...
SEASTAR_TEST_CASE(sstable_run_based_compaction_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto compact = [&, s] (std::vector<shared_sstable> all, auto replacer) -> std::vector<shared_sstable> {
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            return compact_sstables(sstables::compaction_descriptor(std::move(all), std::move(all_sstables), default_priority_class(), 1, 0), *cf, sst_gen, replacer).get0().new_sstables;
        };
        auto make_insert = [&] (auto p) {
            auto key = partition_key::from_exploded(*s, {to_bytes(p.first)});
            mutation m(s, key);
...
SEASTAR_TEST_CASE(backlog_tracker_correctness_after_stop_tracking_compaction) {
    return test_env::do_with_async([] (test_env& env) {
...

        {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            auto fut = compact_sstables(sstables::compaction_descriptor(ssts, std::move(all_sstables), default_priority_class()), *cf, sst_gen);
...
SEASTAR_TEST_CASE(incremental_compaction_data_resurrection_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        try {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
            auto result = compact_sstables(sstables::compaction_descriptor(sstables, std::move(all_sstables), default_priority_class(), 0, max_sstable_size), *cf, sst_gen, replacer).get0().new_sstables;
...
SEASTAR_TEST_CASE(twcs_major_compaction_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
        auto ret = compact_sstables(sstables::compaction_descriptor({original_together}, std::move(all_sstables), default_priority_class()), *cf, sst_gen, replacer_fn_no_op()).get0();
        BOOST_REQUIRE(ret.new_sstables.size() == 1);

        auto original_apart = make_sstable_containing(sst_gen, {mut1, mut2});
        all_sstables = std::make_optional<sstables::sstable_set>(cf->get_sstable_set().clone());
        ret = compact_sstables(sstables::compaction_descriptor({original_apart}, std::move(all_sstables), default_priority_class()), *cf, sst_gen, replacer_fn_no_op()).get0();
...
SEASTAR_TEST_CASE(test_bug_6472) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto ret = compact_sstables(sstables::compaction_descriptor(sstables_spanning_many_windows,
            std::move(all_sstables), default_priority_class()), *cf, sst_gen, replacer_fn_no_op()).get0();
        BOOST_REQUIRE(ret.new_sstables.size() == 1);
        return make_ready_future<>();
    });
}
