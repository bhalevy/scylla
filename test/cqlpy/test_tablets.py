...
def test_tablet_hints(cql, skip_without_tablets):
    ...
    with new_test_keyspace(cql, ksdef) as keyspace:
        ...
        with new_test_table(cql, keyspace, "pk int PRIMARY KEY, c int", extra=f" WITH {tablet_hints}") as table:
            ...
            assert "tablet_hints" not in describe_table(cql, table)
...
def test_tablet_hints_with_view(cql, skip_without_tablets):
    ...
    with new_test_keyspace(cql, ksdef) as keyspace, \
         ...
         new_materialized_view(cql, table, '*', 'c, pk', 'pk is not null and c is not null', extra=f" WITH {tablet_hints}") as view:
        assert tablet_hints in describe_view(cql, table), f"{tablet_hints} not found in {describe_view(cql, table)}"
