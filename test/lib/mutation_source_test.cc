...
public:
    ...
    range_tombstone make_random_range_tombstone() {
        ...
        case 0: {
            ...
            auto end = bound_view(std::move(prefix), bound_kind::incl_end);
            return range_tombstone(std::move(start), std::move(end), std::move(t));
        }
        case 1: {
            // unbound start
            auto start = bound_view::bottom();
            auto end = bound_view(make_random_prefix(), _bool_dist(_gen) ? bound_kind::incl_end : bound_kind::excl_end);
            return range_tombstone(std::move(start), std::move(end), std::move(t));
        }
        case 2: {
            // unbound end
            auto start = bound_view(make_random_prefix(), _bool_dist(_gen) ? bound_kind::incl_start : bound_kind::excl_start);
