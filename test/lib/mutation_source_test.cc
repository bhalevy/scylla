...
static void test_slicing_and_fast_forwarding(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);

    auto s = semaphore.make_simple_schema();
...
static void test_streamed_mutation_forwarding_is_consistent_with_slicing(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);
...
static void test_streamed_mutation_forwarding_guarantees(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);

    auto table = semaphore.make_simple_schema();
    ...
    auto new_stream = [&ms, s, &m, &semaphore] () -> flat_reader_assertions {
...
static void test_fast_forwarding_across_partitions_to_empty_range(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);
...
static void test_streamed_mutation_slicing_returns_only_relevant_tombstones(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);
...
static void test_streamed_mutation_forwarding_across_range_tombstones(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);
...
static void test_range_queries(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
...
void test_all_data_is_read_back(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);

    const auto query_time = gc_clock::now();

    for_each_mutation([&populate, query_time, &semaphore] (const mutation& m) mutable {
...
void test_mutation_reader_fragments_have_monotonic_positions(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);

    for_each_mutation([&populate, &semaphore] (const mutation& m) {
...
static void test_date_tiered_clustering_slicing(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);

    auto ss = semaphore.make_simple_schema();
...
static void test_clustering_slices(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    testlog.info(__PRETTY_FUNCTION__);
    auto s = schema_builder("ks", "cf")
        .with_column("key", bytes_type, column_kind::partition_key)
...
static void test_query_only_static_row(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
...
static void test_query_no_clustering_ranges_no_static_columns(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    BOOST_TEST_MESSAGE(__PRETTY_FUNCTION__);

    auto s = semaphore.make_simple_schema(simple_schema::with_static::no);
...
void test_streamed_mutation_forwarding_succeeds_with_no_data(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    auto s = semaphore.make_simple_schema();
...
static
void test_slicing_with_overlapping_range_tombstones(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    auto ss = semaphore.make_simple_schema();
    auto s = ss.schema();
...
void run_mutation_reader_tests(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    test_slicing_and_fast_forwarding(populate, semaphore);
    test_date_tiered_clustering_slicing(populate, semaphore);
    test_fast_forwarding_across_partitions_to_empty_range(populate, semaphore);
    test_clustering_slices(populate, semaphore);
    test_mutation_reader_fragments_have_monotonic_positions(populate, semaphore);
    test_streamed_mutation_forwarding_across_range_tombstones(populate, semaphore);
    test_streamed_mutation_forwarding_guarantees(populate, semaphore);
    test_all_data_is_read_back(populate, semaphore);
    test_streamed_mutation_slicing_returns_only_relevant_tombstones(populate, semaphore);
    test_streamed_mutation_forwarding_is_consistent_with_slicing(populate, semaphore);
    test_range_queries(populate, semaphore);
    test_query_only_static_row(populate, semaphore);
    test_query_no_clustering_ranges_no_static_columns(populate, semaphore);
}

void test_next_partition(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    auto s = semaphore.make_simple_schema();
...
void run_flat_mutation_reader_tests(populate_fn_ex populate, reader_concurrency_semaphore_for_tests& semaphore) {
    test_next_partition(populate, semaphore);
    test_streamed_mutation_forwarding_succeeds_with_no_data(populate, semaphore);
    test_slicing_with_overlapping_range_tombstones(populate, semaphore);
}
...
void run_mutation_source_tests(populate_fn_ex populate) {
    reader_concurrency_semaphore_for_tests semaphore("mutation_source_tests");
    run_mutation_reader_tests(populate, semaphore);
    run_flat_mutation_reader_tests(populate, semaphore);
}
