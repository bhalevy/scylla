...
public:
    ...
    reader_permit make_reader_permit(const schema* const s = nullptr, const char* n = "test", db::timeout_clock::time_point timeout = db::no_timeout) {
        return _semaphore->make_tracking_only_permit(s, n, timeout);
    }
    reader_permit make_reader_permit(db::timeout_clock::time_point timeout) {
        return make_reader_permit(nullptr, "test", timeout);
    }
