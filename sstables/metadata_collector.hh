...
private:
    ...
    lw_shared_ptr<clustering_key_prefix> _min_clustering_key;
    lw_shared_ptr<clustering_key_prefix> _max_clustering_key;
    bound_view _min_bound;
    bound_view _max_bound;
...
public:
    explicit metadata_collector(const schema& schema, sstring name)
        : _schema(schema)
        , _name(name)
        , _min_bound(bound_view::top())
        , _max_bound(bound_view::bottom())
