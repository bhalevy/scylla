...
class compaction_job_exception : public std::exception {
    sstring _msg;
public:
    compaction_job_exception(sstring msg) : _msg(std::move(msg)) {}
    const char *what() const noexcept {
        return _msg.c_str();
    }
};
...
class compaction_abort_exception : public compaction_job_exception {
public:
    compaction_abort_exception(sstring ks, sstring cf, sstring reason)
        : compaction_job_exception(format("Compaction for {}/{} aborted: {}", ks, cf, reason)) {}
};
