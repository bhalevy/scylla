...
public:
    ...
    mp_row_consumer_k_l(mp_row_consumer_reader_k_l* reader,
                        ...
                        abort_source* asp,
                        const shared_sstable& sst)
        : mp_row_consumer_k_l(reader, schema, std::move(permit), schema->full_slice(), pc, std::move(trace_state), fwd, asp, sst) { }
...
public:
    ...
    data_consume_rows_context(const schema&,
                ...
                : continuous_data_consumer(consumer.permit(), std::move(input), start, maxlen, consumer.abort_source())
                , _consumer(consumer)
                , _sst(std::move(sst))
