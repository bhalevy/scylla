...
public:
    ...
    writer(sstable& sst, const schema& s, uint64_t estimated_partitions,
        ...
        , _large_data_stats({{
                ...
                {
                    large_data_type::items_in_collection,
                    large_data_stats_entry{
                        .threshold = _sst.get_large_data_handler().get_collection_items_count_threshold(),
                    }
                },
            }})
    {
...
void writer::maybe_record_large_cells(const sstables::sstable& sst, const sstables::key& partition_key,
    ...
    auto& collection_items_entry = _large_data_stats.map.at(large_data_type::items_in_collection);
    if (collection_items_entry.max_value < collection_items) {
        collection_items_entry.max_value = collection_items;
    }
    if (_sst.get_large_data_handler().maybe_record_large_cells(_sst, *_partition_key, clustering_key, cdef, cell_size, collection_items).get0()) {
        if (cell_size > cell_size_entry.threshold) {
            cell_size_entry.above_threshold++;
        }
        if (collection_items > collection_items_entry.threshold) {
            collection_items_entry.above_threshold++;
        }
    };
}
...
void writer::write_collection(bytes_ostream& writer, const clustering_key_prefix* clustering_key,
    ...
    collection.with_deserialized(*cdef.type, [&] (collection_mutation_view_description mview) {
        ...
        collection_items = mview.cells.size();
        write_vint(writer, collection_items);
        ...
        _c_stats.cells_count += collection_items;
    });
    uint64_t size = writer.size() - current_pos;
    maybe_record_large_cells(_sst, *_partition_key, clustering_key, cdef, size, collection_items);
}
