...
void writer::consume_end_of_stream() {
    ...
    scylla_metadata::large_data_stats lds;
    lds.map.emplace(large_data_type::partition_size, std::move(_partition_size_entry));
    lds.map.emplace(large_data_type::row_size, std::move(_row_size_entry));
    lds.map.emplace(large_data_type::cell_size, std::move(_cell_size_entry));
    lds.map.emplace(large_data_type::rows_in_partition, std::move(_rows_in_partition_entry));
    lds.map.emplace(large_data_type::elements_in_collection, std::move(_elements_in_collection_entry));
    std::optional<scylla_metadata::large_data_stats> ld_stats(std::move(lds));
