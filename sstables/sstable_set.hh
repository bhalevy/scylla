...
public:
    ...
    future<> for_each_sstable_gently(Func&& func) const {
        ...
        // It is possible that the sstable_set will be updated by cloning the impl
        // and setting a new sstable_set_impl_ptr, when `func` yields.
        // So hold on to the existing impl ptr since it is never updated in place.
        auto impl = _impl;
