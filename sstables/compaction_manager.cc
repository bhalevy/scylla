...
future<> compaction_manager::perform_cleanup(database& db, column_family* cf) {
    ...
    return rs.get_ranges_async(utils::fb_utilities::get_broadcast_address()).then([this, cf, schema] (dht::token_range_vector sorted_owned_ranges) {
        ...
        if (sorted_owned_ranges.empty()) {
            return make_ready_future<std::vector<sstables::shared_sstable>>(candidates);
        }
        return do_with(std::vector<sstables::shared_sstable>{}, std::move(sorted_owned_ranges),
                    [this, &candidates, schema] (std::vector<sstables::shared_sstable>& sstables, dht::token_range_vector& sorted_owned_ranges) {
