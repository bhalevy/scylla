...
private:
    ...
    virtual flat_mutation_reader::filter consider_partition() const {
...
public:
    ...
    virtual flat_mutation_reader::filter consider_partition() const override {
        return [] (const dht::decorated_key& dk){
...
public:
    ...
    flat_mutation_reader::filter consider_partition() const override {
...
future<compaction_info> compaction::run(std::unique_ptr<compaction> c) {
    return seastar::async([c = std::move(c)] () mutable {
        ...
        try {
            ...
            r.consume_in_thread(std::move(cfc), c->consider_partition(), db::no_timeout);
        } catch (...) {
