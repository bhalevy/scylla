...
public:
    ...
    sstables::compaction_descriptor get_descriptor_for_level(int level, const std::vector<std::optional<dht::decorated_key>>& last_compacted_keys,
        ...
        if (!info.candidates.empty()) {
            ...
            auto all_sstables = std::make_optional<sstables::sstable_set>(_table.get_sstable_set().clone());
            return sstables::compaction_descriptor(std::move(info.candidates), std::move(all_sstables),
        ...
        for (auto i = _generations.size() - 1; i > 0; i--) {
            ...
            if (get_level_size(0) > MAX_COMPACTING_L0) {
                ...
                if (!most_interesting.empty()) {
                    ...
                    auto all_sstables = std::make_optional<sstables::sstable_set>(_table.get_sstable_set().clone());
                    return sstables::compaction_descriptor(std::move(most_interesting), std::move(all_sstables),
