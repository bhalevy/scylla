...
protected:
    ...
    object_name make_ref_object_name(const sstable& sst, generation_type gen, locator::host_id host_id) const {
        return make_ref_object_name(gen, host_id);
    }
...
future<size_t> object_storage_base::num_references(generation_type gen) const {
    auto refs = co_await list_object_storage_references(*_client, _bucket, prefix().as_string(), gen);
    co_return refs.size();
}
...
future<entry_descriptor> object_storage_base::clone(const sstable& sst, generation_type gen, bool leave_unsealed) const {
    ...
    auto ref_name = make_ref_object_name(sst, gen, node_owner);
