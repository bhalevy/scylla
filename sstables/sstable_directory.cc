                           ...
                           compaction::owned_ranges_ptr owned_ranges_ptr)
{
    ...
    co_await coroutine::parallel_for_each(shared_info, [this, sstables_per_job, num_jobs, &buckets, &owned_ranges_ptr] (sstables::foreign_sstable_open_info& info) -> future<> {
        auto sst = _manager.make_sstable(_schema, _sstable_dir.native(), info.generation, info.version, info.format, gc_clock::now(), _error_handler_gen);
        co_await sst->load(std::move(info));
        if (owned_ranges_ptr) {
            update_sstable_cleanup_state(sst, *owned_ranges_ptr);
        }
    ...
    co_await coroutine::parallel_for_each(buckets, [&] (std::vector<sstables::shared_sstable>& sstlist) {
        return cm.run_custom_job(t, compaction_type::Reshard, "Reshard compaction", [&] (sstables::compaction_data& info) -> future<> {
