...
void sstable::validate_min_max_metadata() {
    ...
    // - column name is stored instead of clustering value.
    // - clustering component is stored as composite.
    // Note: now that we store min/max metadata for range tombstones:
    ...
    auto check_composite = _schema->is_compound() && _schema->clustering_key_size() > 1 && _schema->is_dense();
    for (auto i = 0U; i < min_column_names.size(); i++) {
        if (_schema->get_column_definition(min_column_names[i].value) ||
            (check_composite && composite_view(min_column_names[i].value).is_valid())) {
            clear_incorrect_min_max_column_names();
            return;
        }
    }

    for (auto i = 0U; i < max_column_names.size(); i++) {
        if (_schema->get_column_definition(max_column_names[i].value) ||
            (check_composite && composite_view(max_column_names[i].value).is_valid())) {
            clear_incorrect_min_max_column_names();
            return;
        }
    }
