...
// record.
gc_clock::time_point sstable::get_gc_before_for_drop_estimation(utils::optional_reference<const compaction_manager> cm, const gc_clock::time_point& compaction_time) const {
    auto s = get_schema();
    ...
    return get_gc_before_for_range(s, cm, range, compaction_time).max_gc_before;
}
...
gc_clock::time_point sstable::get_gc_before_for_fully_expire(utils::optional_reference<const compaction_manager> cm, const gc_clock::time_point& compaction_time) const {
    ...
    auto res = get_gc_before_for_range(s, cm, range, compaction_time);
