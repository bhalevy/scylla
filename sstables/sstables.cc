...
static future<> maybe_close(file f) {
    if (!f) {
        return make_ready_future<>();
    }
    return f.close().then_wrapped([f] (future<> fut) {
...
future<> sstable::open_or_create_data(std::function<future<file>(component_type)> func) {
    return when_all_succeed(func(component_type::Index).then([this] (file f) { _index_file = f; }),
                            func(component_type::Data).then([this] (file f) { _data_file = f; })).then_wrapped([this] (future<> fut) {
        if (__builtin_expect(!fut.failed(), true)) {
            return make_ready_future<>();
        }
        return maybe_close(_index_file).finally([this, ex = fut.get_exception()] {
            return maybe_close(_data_file).finally([ex] {
...
future<> sstable::open_data() {
    return open_or_create_data([this] (component_type c) { return open_file(c, open_flags::ro); }).then([this] {
...
future<> sstable::create_data() {
    ...
    return open_or_create_data([this, oflags, opt = std::move(opt)] (component_type c) { return open_file(c, oflags, std::move(opt)); });
}
