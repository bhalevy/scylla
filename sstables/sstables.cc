...
// record.
gc_clock::time_point sstable::get_gc_before_for_drop_estimation(compaction_manager_opt cm_opt, const gc_clock::time_point& compaction_time) const {
    auto s = get_schema();
    ...
    return get_gc_before_for_range(s, cm_opt, range, compaction_time).max_gc_before;
}
...
gc_clock::time_point sstable::get_gc_before_for_fully_expire(compaction_manager_opt cm_opt, const gc_clock::time_point& compaction_time) const {
    ...
    auto res = get_gc_before_for_range(s, cm_opt, range, compaction_time);
