...
future<> sstable::open_or_create_data(open_flags oflags, file_open_options options) {
    return when_all_succeed(open_file(component_type::Index, oflags, options).then([this] (file f) { _index_file = f; }),
                            open_file(component_type::Data, oflags, options).then([this] (file f) { _data_file = f; }))
            .then_wrapped([this] (future<> fut) {
        if (!fut.failed()) {
            return make_ready_future<>();
        }

        auto maybe_close = [this] (file f, component_type c) {
            if (!f) {
                return make_ready_future<>();
            }
            return f.close().then_wrapped([this, f, c] (future<> fut) {
                if (fut.failed()) {
        ...
        return when_all_succeed(maybe_close(_index_file, component_type::Index),
                                maybe_close(_data_file, component_type::Data)).finally([ex = fut.get_exception()] {
