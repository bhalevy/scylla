...
void
sstable::write_scylla_metadata(const io_priority_class& pc, shard_id shard, sstable_enabled_features features, struct run_identifier identifier, scylla_metadata::large_data_counters ldcs) {
    ...
    _components->scylla_metadata->data.set<scylla_metadata_type::LargeDataCounters>(std::move(ldcs));
