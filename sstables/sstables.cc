...
future<> sstable::read_toc() {
    ...
    return new_sstable_component_file(_read_error_handler, component_type::TOC, open_flags::ro).then([this, file_path] (file f) {
      auto fut = futurize_invoke([this, file_path, f] () mutable {
...
future<> sstable::open_or_create_data(open_flags oflags, file_open_options options) {
    return when_all_succeed(open_file(component_type::Index, oflags, options).then([this] (file f) { _index_file = std::move(f); }),
        ...
        auto close_if_needed = [this] (file& f, component_type c) {
            if (!f) {
                return make_ready_future<>();
            }
