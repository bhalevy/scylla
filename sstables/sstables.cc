...
future<> sstable::read_toc() {
    ...
    return utils::with_file(new_sstable_component_file(_read_error_handler, component_type::TOC, open_flags::ro), [this] (file f) {
...
future<> sstable::open_or_create_data(open_flags oflags, file_open_options options) noexcept {
    return when_all_succeed(open_file(component_type::Index, oflags, options).then([this] (file f) { _index_file = std::move(f); }),
        ...
        auto close_if_needed = [this] (file& f, component_type c) noexcept {
            if (!f) {
                return make_ready_future<>();
            }
            return f.close().then_wrapped([this, &f, c] (future<> fut) mutable {
                if (fut.failed()) {
                    sstlog.warn("Couldn't close {}: {}. Ignoring.", filename(c), fut.get_exception());
                }
                // reset the file to prevent double close in ~sstable
                f = file();
                return make_ready_future<>();
            });
        };

        return when_all_succeed(close_if_needed(_index_file, component_type::Index),
                                close_if_needed(_data_file, component_type::Data)).finally([this, ex = fut.get_exception()] {
