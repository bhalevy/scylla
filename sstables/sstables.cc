...
future<> sstable::create_links_common(sstring dir, int64_t generation, bool mark_for_removal) const {
    sstlog.trace("create_links: {} -> {} generation={} mark_for_removal={}", get_filename(), dir, generation, mark_for_removal);
    return do_with(std::move(dir), all_components(), [this, generation, mark_for_removal] (const sstring& dir, auto& comps) {
...
future<> sstable::create_links(sstring dir, int64_t generation) const {
    return create_links_common(std::move(dir), generation, false /* mark_for_removal */);
}

future<> sstable::create_links_and_mark_for_removal(sstring dir, int64_t generation) const {
    return create_links_common(std::move(dir), generation, true /* mark_for_removal */);
}
