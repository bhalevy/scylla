...
void metadata_collector::update_min_max_components(const clustering_key_prefix& key) {
    ...
    auto res = cmp(key, bound_view(*_min_clustering_key, bound_kind::incl_start));
    if (res < 0) {
        mdclogger.trace("{}: setting min_clustering_key={}", _name, clustering_key_prefix::with_schema_wrapper(_schema, key));
        _min_clustering_key.emplace(key);
    }

    res = cmp(key, bound_view(*_max_clustering_key, bound_kind::incl_end));
    if (res > 0) {
...
void metadata_collector::update_min_max_components(const range_tombstone& rt) {
    ...
    } else if (cmp(bound_view(rt.start, rt.start_kind), bound_view(*_min_clustering_key, bound_kind::incl_start)) < 0) {
    ...
    } else if (cmp(bound_view(rt.end, rt.end_kind), bound_view(*_max_clustering_key, bound_kind::incl_end)) > 0) {
