...
class load_balancer {
    ...
    struct cluster_resize_load {
        ...
        static bool table_needs_split(const table_size_desc& d) {
            lblogger.debug("table_needs_split: tablet_count={}, avg_tablet_size={}, target_max_tablet_size={} min_tablet_count={}",
                           d.tablet_count, d.avg_tablet_size, d.target_max_tablet_size(), d.min_tablet_count);
...
public:
    ...
    future<table_resize_plan> make_resize_plan(const migration_plan& plan) {
        ...
        for (auto&& [table, tmap_] : _tm->tablets().all_tables()) {
            ...
            const auto* rs = erm->get_replication_strategy().maybe_as_tablet_aware();
            if (!rs) {
                // FIXME: rs can be null in unit tests
                lblogger.debug("Table {}.{} has no tablet_aware_replication_strategy: uses_tablets={}", t->schema()->ks_name(), t->schema()->cf_name(), erm->get_replication_strategy().uses_tablets());
            }
            ...
            table_size_desc size_desc {
                ...
                .min_tablet_count = rs ? rs->calculate_min_tablet_count(t->schema(), erm->get_token_metadata_ptr(), _target_tablet_size) : 0,
            };
