...
future<> raft_sys_table_storage::store_term_and_vote(raft::term_t term, raft::server_id vote) {
    return execute_with_linearization_point([this, term, vote] {
        ...
        return _qp.execute_internal(
            store_cql, cql3::query_processor::cache_internal::yes,
...
future<std::pair<raft::term_t, raft::server_id>> raft_sys_table_storage::load_term_and_vote() {
    ...
    ::shared_ptr<cql3::untyped_result_set> rs = co_await _qp.execute_internal(load_cql, cql3::query_processor::cache_internal::yes, _group_id.id);
    if (rs->empty()) {
...
future<raft::snapshot_descriptor> raft_sys_table_storage::load_snapshot_descriptor() {
    ...
    ::shared_ptr<cql3::untyped_result_set> id_rs = co_await _qp.execute_internal(load_id_cql, cql3::query_processor::cache_internal::yes, _group_id.id);
    ...
    ::shared_ptr<cql3::untyped_result_set> snp_rs = co_await _qp.execute_internal(load_snp_info_cql, cql3::query_processor::cache_internal::yes, _group_id.id);
    ...
    ::shared_ptr<cql3::untyped_result_set> cfg_rs = co_await _qp.execute_internal(load_cfg_cql, cql3::query_processor::cache_internal::yes, _group_id.id);
...
future<> raft_sys_table_storage::store_snapshot_descriptor(const raft::snapshot_descriptor& snap, size_t preserve_log_entries) {
    ...
    return execute_with_linearization_point([this, &snap, preserve_log_entries] () -> future<> {
        ...
        co_await _qp.execute_internal(delete_raft_cfg_cql, cql3::query_processor::cache_internal::yes, _group_id.id);
...
future<> raft_sys_table_storage::truncate_log(raft::index_t idx) {
    return execute_with_linearization_point([this, idx] {
        static const auto truncate_cql = format("DELETE FROM system.{} WHERE group_id = ? AND \"index\" >= ?",
            db::system_keyspace::RAFT); 
        return _qp.execute_internal(truncate_cql, cql3::query_processor::cache_internal::yes, _group_id.id, int64_t(idx)).discard_result();
    });
}
