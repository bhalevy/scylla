...
future<> storage_service::node_ops_cmd_heartbeat_updater(node_ops_cmd cmd, node_ops_id uuid, std::unordered_set<locator::node_ptr> nodes, lw_shared_ptr<bool> heartbeat_updater_done) {
...
future<> storage_service::node_ops_cmd_heartbeat_updater(node_ops_cmd cmd, node_ops_id uuid, std::list<gms::inet_address> nodes, lw_shared_ptr<bool> heartbeat_updater_done) {
    ...
    while (!(*heartbeat_updater_done)) {
        ...
        try {
          co_await coroutine::parallel_for_each(nodes, [this, ops, uuid, &req] (const locator::node_ptr& node) {
            return _messaging.local().send_node_ops_cmd(node, req).then([ops, uuid, node] (node_ops_cmd_response resp) {
...
future<> storage_service::decommission() {
    return run_with_api_lock(sstring("decommission"), [] (storage_service& ss) {
        return seastar::async([&ss] {
            ...
            std::unordered_set<locator::node_ptr> nodes;
            ...
            std::unordered_set<locator::node_ptr> gossip_nodes_down;
            for (auto& node : nodes) {
            ...
            if (!gossip_nodes_down.empty()) {
                auto msg = format("decommission[{}]: Rejected decommission operation, leaving node={}, sync_nodes={}, ignore_nodes={}, nodes_down={}",
                        uuid, leaving_node, nodes, ignore_nodes, gossip_nodes_down);
                slogger.warn("{}", msg);
                throw std::runtime_error(msg);
            }

            // Step 2: Prepare to sync data
            std::unordered_set<locator::node_ptr> nodes_unknown_verb;
            std::unordered_set<locator::node_ptr> nodes_down;
            ...
            try {
                parallel_for_each(nodes, [&ss, &req, &nodes_unknown_verb, &nodes_down, uuid] (const locator::node_ptr& node) {
                    return ss._messaging.local().send_node_ops_cmd(node, req).then([uuid, node] (node_ops_cmd_response resp) {
...
void storage_service::run_bootstrap_ops(std::unordered_set<token>& bootstrap_tokens) {
    ...
    std::unordered_set<locator::node_ptr> ignore_nodes;
    std::unordered_set<locator::node_ptr> sync_nodes;
    ...
    for (;;) {
        ...
        parallel_for_each(sync_nodes, [this, req, uuid, &pending_ops] (const locator::node_ptr& node) {
            return _messaging.local().send_node_ops_cmd(node, req).then([uuid, node, &pending_ops] (node_ops_cmd_response resp) {
    ...
    std::unordered_set<locator::node_ptr> nodes_unknown_verb;
    std::unordered_set<locator::node_ptr> nodes_down;
    std::unordered_set<locator::node_ptr> nodes_aborted;
    ...
    try {
        ...
        parallel_for_each(sync_nodes, [this, &req, &nodes_unknown_verb, &nodes_down, uuid] (const locator::node_ptr& node) {
            return _messaging.local().send_node_ops_cmd(node, req).then([uuid, node] (node_ops_cmd_response resp) {
        ...
        parallel_for_each(sync_nodes, [this, &req, &nodes_aborted, uuid] (const locator::node_ptr& node) {
            return _messaging.local().send_node_ops_cmd(node, req).then([&nodes_aborted, uuid, node] (node_ops_cmd_response resp) {
...
future<> storage_service::removenode(locator::host_id host_id, std::list<locator::host_id_or_endpoint> ignore_nodes_params) {
    return run_with_api_lock(sstring("removenode"), [host_id, ignore_nodes_params = std::move(ignore_nodes_params)] (storage_service& ss) mutable {
        return seastar::async([&ss, host_id, ignore_nodes_params = std::move(ignore_nodes_params)] () mutable {
            ...
            std::unordered_set<locator::node_ptr> nodes;
            for (const auto& [ep, id] : tmptr->get_endpoint_to_host_id_map_for_reading()) {
                seastar::thread::maybe_yield();
                auto node = topo.find_node(id, locator::topology::must_exist::yes);
                if (node != node_to_remove && !ignore_nodes.contains(node)) {
                    nodes.emplace(std::move(node));
                }
            }
            slogger.info("removenode[{}]: Started removenode operation, removing node={}/{}, sync_nodes={}, ignore_nodes={}", uuid, host_id, endpoint, nodes, ignore_nodes);
            ...
            std::unordered_set<locator::node_ptr> nodes_unknown_verb;
            std::unordered_set<locator::node_ptr> nodes_down;
            auto req = node_ops_cmd_request::make_removenode_prepare(uuid, ignore_nodes, node_to_remove);
            try {
                parallel_for_each(nodes, [&ss, &req, &nodes_unknown_verb, &nodes_down, uuid] (const locator::node_ptr& node) {
                    return ss._messaging.local().send_node_ops_cmd(node, req).then([uuid, node] (node_ops_cmd_response resp) {
