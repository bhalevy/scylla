...
future<> storage_service::merge_topology_snapshot(raft_snapshot snp) {
    ...
    if (it != snp.mutations.end()) {
        ...
        // we ensure that the lack of atomicity isn't a problem here.
        {
            ...
            auto apply_mutation = [&] (frozen_mutation m) -> future<> {
                auto units = get_units(sem, 1);
                co_await _db.local().apply(s, m, {}, db::commitlog::force_sync::yes, db::no_timeout);
            };
            auto apply_in_background = [&] (frozen_mutation m) {
                done = done.then([&, m = std::move(m)] () mutable {
