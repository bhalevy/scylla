...
std::unordered_multimap<dht::token_range, inet_address> storage_service::get_changed_ranges_for_leaving(sstring keyspace_name, inet_address endpoint) {
    ...
    for (auto& r : ranges) {
        ...
        auto eps = ers->get_natural_endpoints(end_token);
...
future<> storage_service::load_and_stream(sstring ks_name, sstring cf_name,
    ...
    auto ers = _db.local().find_keyspace(ks_name).get_effective_replication_strategy();
        ...
        try {
            ...
            while (auto mf = co_await reader()) {
                ...
                if (is_partition_start) {
                    ...
                    current_targets = ers->get_natural_endpoints(current_dk.token());
...
storage_service::construct_range_to_endpoint_map(
    ...
    auto ers = _db.local().find_keyspace(keyspace).get_effective_replication_strategy();
    for (auto r : ranges) {
        res[r] = ers->get_natural_endpoints(
...
storage_service::get_ranges_for_endpoint(const sstring& name, const gms::inet_address& ep) const {
    return _db.local().find_keyspace(name).get_effective_replication_strategy()->get_ranges(ep);
}
...
storage_service::get_natural_endpoints(const sstring& keyspace, const token& pos) const {
    return _db.local().find_keyspace(keyspace).get_effective_replication_strategy()->get_natural_endpoints(pos);
}
