    ...
    if (should_bootstrap()) {
        ...
        if (_bootstrap_tokens.empty()) {
            ...
            if (initial_tokens.size() < 1) {
                _bootstrap_tokens = boot_strapper::get_random_tokens(*tmptr, num_tokens);
                if (num_tokens == 1) {
    ...
    if (!_cdc_streams_ts && db().local().get_config().check_experimental(db::experimental_features_t::CDC)) {
        ...
        if (!db().local().is_replacing()
            ...
            _cdc_streams_ts = cdc::make_new_cdc_generation(db().local().get_config(),
                    _bootstrap_tokens, *tmptr, _gossiper,
...
void storage_service::bootstrap() {
    ...
    if (!db().local().is_replacing()) {
        ...
        if (db().local().get_config().check_experimental(db::experimental_features_t::CDC)) {
            ...
            tmptr->add_bootstrap_tokens(_bootstrap_tokens, get_broadcast_address());
            update_pending_ranges().get();
    ...
    if (is_repair_based_node_ops_enabled()) {
        if (db().local().is_replacing()) {
            replace_with_repair(_db, _messaging, tmptr, _bootstrap_tokens).get();
        } else {
            bootstrap_with_repair(_db, _messaging, tmptr, _bootstrap_tokens).get();
        }
    } else {
        dht::boot_strapper bs(_db, _abort_source, get_broadcast_address(), _bootstrap_tokens, *tmptr);
...
void storage_service::handle_state_normal(inet_address endpoint) {
    ...
    if (slogger.is_enabled(logging::log_level::debug)) {
        auto ver = tmptr->get_ring_version();
        for (auto& x : tmptr->get_token_to_endpoint()) {
    ...
    return seastar::async([this, do_bind] {
        ...
        if (get_property_load_ring_state()) {
            ...
            if (_updated_token_metadata) {
                _token_metadata.cmp_and_set_shared_ptr(std::move(_updated_token_metadata));
            }
        }
...
future<> storage_service::replicate_tm_only() {
    ...
    if (!_updated_token_metadata) {
        slogger.debug("No updated token_metadata");
        return make_ready_future<>();
    }
    return do_with(std::move(_updated_token_metadata), [this] (token_metadata_ptr& tmptr) {
        slogger.debug("setting token_metadata");
        _token_metadata.cmp_and_set_shared_ptr(tmptr);
        return get_storage_service().invoke_on_all([&tmptr] (storage_service& local_ss) {
            if (this_shard_id() != 0) {
                slogger.debug("copying token_metadata locally");
                auto local_tmptr = local_ss._token_metadata.clone_shared_ptr_from(*tmptr);
                local_ss._token_metadata.cmp_and_set_shared_ptr(std::move(local_tmptr));
            }
        });
    });
}
...
future<> storage_service::replicate_to_all_cores() {
    ...
    slogger.debug("Scheduling replicate_action");
...
future<> storage_service::decommission() {
    return run_with_api_lock(sstring("decommission"), [] (storage_service& ss) {
        return seastar::async([&ss] {
            ...
            auto& db = ss.db().local();
            if (!tmptr->is_member(ss.get_broadcast_address())) {
            ...
            for (const auto& keyspace_name : non_system_keyspaces) {
                if (tmptr->get_pending_ranges(keyspace_name, ss.get_broadcast_address()).size() > 0) {
...
void storage_service::update_topology(inet_address endpoint) {
    assert(!_updated_token_metadata);
    auto tmptr = _token_metadata.clone_shared_ptr();

    // initiate the token metadata endpoints cache reset
    tmptr->invalidate_cached_rings();
    // re-read local rack and DC info
    tmptr->update_topology(endpoint);
    _token_metadata.cmp_and_set_shared_ptr(std::move(tmptr));
}
...
future<> init_storage_service(sharded<abort_source>& abort_source, distributed<database>& db, sharded<gms::gossiper>& gossiper,
        ...
        storage_service_config config, sharded<service::migration_notifier>& mn, sharded<locator::shared_token_metadata>& tm,
