...
future<> storage_service::handle_state_normal(inet_address endpoint) {
    ...
    for (auto t : tokens) {
        ...
        auto current_owner = token_to_endpoint_map.find(t);
        if (current_owner == token_to_endpoint_map.end()) {
            slogger.debug("handle_state_normal: New node {} at token {}", endpoint, t);
            owned_tokens.insert(t);
        } else if (endpoint == current_owner->second) {
            slogger.debug("handle_state_normal: endpoint={} == current_owner={} token {}", endpoint, *current_owner, t);
            // set state back to normal, since the node may have tried to leave, but failed and is now back up
            owned_tokens.insert(t);
            token_to_endpoint_map.erase(current_owner);
        } else if (_gossiper.compare_endpoint_startup(endpoint, current_owner->second) > 0) {
            slogger.debug("handle_state_normal: endpoint={} > current_owner={}, token {}", endpoint, current_owner->second, t);
            owned_tokens.insert(t);
            slogger.info("handle_state_normal: remove endpoint={} token={}", *current_owner, t);
            token_to_endpoint_map.erase(current_owner);
            candidates_for_removal.insert(current_owner->second);
