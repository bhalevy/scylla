    ...
    co_await mutate_token_metadata(seastar::coroutine::lambda([this, &id2ip, &am] (mutable_token_metadata_ptr tmptr) -> future<> {
        ...
        auto add_normal_node = [&] (raft::server_id id, const replica_state& rs) -> future<> {
            ...
            if (!is_me(ip)) {
                ...
                info.insert_or_assign("tokens", db::system_keyspace::make_tokens_data_value(rs.ring.value().tokens));
                info.insert_or_assign("data_center", rs.datacenter);
                info.insert_or_assign("rack", rs.rack);
                info.insert_or_assign("host_id", id.uuid());
                info.insert_or_assign("release_version", rs.release_version);
            ...
            case node_state::bootstrapping:
                if (rs.ring.has_value()) {
                    if (!is_me(ip)) {
                        ...
                        data_values_map info;
                        info.emplace("tokens", db::system_keyspace::make_tokens_data_value({}));
                        info.emplace("host_id", id.uuid());
                        co_await _sys_ks.local().update_peer_info(ip, std::move(info));
                    }
                    update_topology(host_id, ip, rs);
...
future<> storage_service::join_token_ring(sharded<db::system_distributed_keyspace>& sys_dist_ks,
    ...
    gms::application_states_map app_states;
...
future<> storage_service::handle_state_normal(inet_address endpoint, gms::permit_id pid) {
    ...
    if (!is_me(endpoint) && !owned_tokens.empty() && !endpoints_to_remove.count(endpoint)) {
        try {
            ...
            info.insert_or_assign("tokens", db::system_keyspace::make_tokens_data_value(owned_tokens));
            co_await _sys_ks.local().update_peer_info(endpoint, std::move(info));
        } catch (...) {
...
future<> storage_service::on_change(gms::inet_address endpoint, const gms::application_states_map& states_, gms::permit_id pid) {
