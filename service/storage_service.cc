...
std::unordered_multimap<dht::token_range, inet_address> storage_service::get_changed_ranges_for_leaving(sstring keyspace_name, inet_address endpoint) {
    ...
    for (auto& r : ranges) {
        ...
        auto eps = ks.get_effective_replication_strategy()->get_natural_endpoints(end_token);
...
storage_service::construct_range_to_endpoint_map(
    ...
    for (auto r : ranges) {
        res[r] = _db.local().find_keyspace(keyspace).get_effective_replication_strategy()->get_natural_endpoints(
