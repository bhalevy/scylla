...
public:
    ...
    void start(sstring desc_) {
        ...
        if (!nodes_down.empty()) {
            auto msg = format("{}[{}]: nodes={} needed for {} operation are down. It is highly recommended to fix the down nodes and try again.", desc, uuid(), nodes_down, desc);
            slogger.warn("{}", msg);
            throw std::runtime_error(msg);
        }
    ...
    future<> abort(node_ops_cmd cmd, std::exception_ptr ex) noexcept {
    ...
    future<> send_to_all(node_ops_cmd cmd) {
        ...
        bool is_prepare = op_desc.find("prepare") != sstring::npos;
        slogger.info("{}[{}]: Started {}, sync_nodes={}", desc, uuid(), op_desc, sync_nodes);
        co_await coroutine::parallel_for_each(sync_nodes, [&] (const gms::inet_address& node) -> future<> {
            if (nodes_unknown_verb.contains(node) || nodes_down.contains(node) ||
                    (nodes_failed.contains(node) && op_desc != "abort")) {
                // Note that we still send to failed nodes that 
                co_return;
            }
            try {
                ...
                slogger.warn("{}[{}]: Node {} does not support {} verb", desc, uuid(), node, op_desc);
        ...
        if (!nodes_failed.empty()) {
            msg = format("nodes={} failed {} operation", nodes_failed, op_desc);
            sep = ";";
        }
        if (!nodes_unknown_verb.empty()) {
            msg += format("{}nodes={} do not support {} operation{}", sep, nodes_unknown_verb, op_desc,
                    is_prepare ? ". Please upgrade your cluster and run operation again" : "");
            sep = ";";
        }
        if (!nodes_down.empty()) {
            msg += format("{}nodes={} needed for {} operation are down. It is highly recommended to fix the down nodes and try again", sep, nodes_down, op_desc);
            sep = ";";
        }
        if (!msg.empty()) {
            co_await coroutine::return_exception(std::runtime_error(msg));
        }
        slogger.info("{}[{}]: Finished {}, sync_nodes={}", desc, uuid(), op_desc, sync_nodes);
    }

    future<> heartbeat_updater(node_ops_cmd cmd) {
        slogger.info("{}[{}]: Started heartbeat_updater", desc, uuid());
        while (!as.abort_requested()) {
            ...
            co_await sleep_abortable(std::chrono::seconds(10), as).handle_exception([] (std::exception_ptr) {});
        }
    ...
    return run_with_api_lock(sstring("decommission"), [] (storage_service& ss) {
        return seastar::async([&ss] {
            ...
            try {
                ...
                ctl.abort(node_ops_cmd::decommission_abort, std::current_exception()).get();
                throw;
            }
...
void storage_service::run_bootstrap_ops(std::unordered_set<token>& bootstrap_tokens) {
    ...
    try {
        ...
        ctl.abort(node_ops_cmd::bootstrap_abort, std::current_exception()).get();
        throw;
    }
}
...
void storage_service::run_replace_ops(std::unordered_set<token>& bootstrap_tokens, replacement_info replace_info) {
    ...
    try {
        ...
        ctl.abort(node_ops_cmd::replace_abort, std::current_exception()).get();
        throw;
    }
}
        ...
        return seastar::async([&ss, host_id, ignore_nodes_params = std::move(ignore_nodes_params)] () mutable {
            ...
            if (endpoint_opt) {
                ...
                try {
                    ...
                    ctl.abort(node_ops_cmd::removenode_abort, std::current_exception()).get();
                    throw;
                }
            }
