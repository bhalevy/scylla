...
future<> storage_service::handle_state_bootstrap(gms::endpoint_id node, gms::permit_id pid) {
    ...
    tmptr->update_topology(addr, get_dc_rack_for(node), locator::node::state::bootstrapping);
...
future<> storage_service::handle_state_normal(gms::endpoint_id node, gms::permit_id pid) {
    ...
    if (endpoints_to_remove.contains(addr)) [[unlikely]] {
        ...
        tmptr->update_topology(addr, get_dc_rack_for(node), locator::node::state::normal);
...
future<> storage_service::handle_state_leaving(gms::endpoint_id node, gms::permit_id pid) {
    slogger.debug("endpoint={} handle_state_leaving: permit_id={}", node, pid);

    const auto& addr = node.addr;
    auto tokens = get_tokens_for(addr);

    slogger.debug("Node {} state leaving, tokens {}", node, tokens);
    ...
    if (!tmptr->is_normal_token_owner(addr)) {
        ...
        slogger.info("Node {} state jump to leaving", node);

        tmptr->update_topology(addr, get_dc_rack_for(node), locator::node::state::being_decommissioned);
        co_await tmptr->update_normal_tokens(tokens, addr);
    } else {
        auto tokens_ = tmptr->get_tokens(addr);
        std::set<token> tmp(tokens.begin(), tokens.end());
        if (!std::includes(tokens_.begin(), tokens_.end(), tmp.begin(), tmp.end())) {
            slogger.warn("Node {} 'leaving' token mismatch. Long network partition?", node);
    ...
    // normally
    tmptr->add_leaving_endpoint(addr);

    co_await update_topology_change_info(tmptr, ::format("handle_state_leaving", node));
    co_await replicate_to_all_cores(std::move(tmptr));
}
...
future<> storage_service::on_alive(gms::endpoint_id node, gms::endpoint_state_ptr state, gms::permit_id pid) {
    ...
    if (is_normal_token_owner) {
        ...
        auto tmlock = co_await get_token_metadata_lock();
        auto tmptr = co_await get_mutable_token_metadata_ptr();
        tmptr->update_topology(addr, get_dc_rack_for(node));
        co_await replicate_to_all_cores(std::move(tmptr));
    }
}
...
future<> storage_service::on_change(gms::endpoint_id node, application_state state, const versioned_value& value, gms::permit_id pid) {
    ...
    if (state == application_state::STATUS) {
        ...
        if (move_name == sstring(versioned_value::STATUS_BOOTSTRAPPING)) {
            ...
            co_await handle_state_leaving(node, pid);
