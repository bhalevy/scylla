...
void storage_service::prepare_to_join(
    ...
    replicate_to_all_cores(std::move(tmlock), std::move(tmptr)).get();
...
void storage_service::bootstrap() {
    ...
    if (!db().local().is_replacing()) {
        ...
        if (db().local().get_config().check_experimental(db::experimental_features_t::CDC)) {
            ...
            update_pending_ranges(std::move(tmlock), std::move(tmptr), format("bootstrapping node {}", endpoint)).get();
...
void storage_service::handle_state_replacing(inet_address replacing_node) {
    ...
    update_pending_ranges(std::move(tmlock), std::move(tmptr), format("handle_state_replacing {}", replacing_node)).get();
}
...
void storage_service::handle_state_bootstrap(inet_address endpoint) {
    ...
    update_pending_ranges(std::move(tmlock), std::move(tmptr), format("handle_state_bootstrap {}", endpoint)).get();
}
...
void storage_service::handle_state_normal(inet_address endpoint) {
    ...
    // Order Matters, tmptr->updateHostID() should be called before tmptr->updateNormalToken(), (see CASSANDRA-4300).
    if (_gossiper.uses_host_id(endpoint)) {
    ...
    update_pending_ranges(std::move(tmlock), std::move(tmptr), format("handle_state_normal {}", endpoint)).get();
...
void storage_service::handle_state_leaving(inet_address endpoint) {
    ...
    update_pending_ranges(std::move(tmlock), std::move(tmptr), format("handle_state_leaving", endpoint)).get();
}
...
void storage_service::handle_state_removing(inet_address endpoint, std::vector<sstring> pieces) {
    ...
    if (get_token_metadata().is_member(endpoint)) {
        ...
        if (sstring(gms::versioned_value::REMOVED_TOKEN) == state) {
            ...
            tmptr->add_leaving_endpoint(endpoint);
            update_pending_ranges(std::move(tmlock), std::move(tmptr), format("handle_state_removing {}", endpoint)).get();
...
void storage_service::on_remove(gms::inet_address endpoint) {
    ...
    auto tmptr = get_mutable_token_metadata_ptr().get0();
    tmptr->remove_endpoint(endpoint);
    update_pending_ranges(std::move(tmlock), std::move(tmptr), format("on_remove {}", endpoint)).get();
}
...
future<> storage_service::replicate_to_all_cores(token_metadata_lock tmlock, token_metadata_ptr tmptr) noexcept {
    assert(this_shard_id() == 0);

    slogger.debug("Replicating token_metadata to all cores");
    _shared_token_metadata.set(tmptr);
    return get_storage_service().invoke_on_others([tmptr] (storage_service& local_ss){
      return tmptr->clone_async().then([&local_ss] (token_metadata tm) {
        local_ss._shared_token_metadata.set(make_token_metadata_ptr(std::move(tm)));
      });
    }).handle_exception([tmlock = std::move(tmlock)] (auto e) {
...
future<> storage_service::removenode(sstring host_id_string) {
    return run_with_api_lock(sstring("removenode"), [host_id_string] (storage_service& ss) mutable {
        return seastar::async([&ss, host_id_string] {
            ...
            auto tmlock = ss.get_token_metadata_lock().get0();
            ...
            tmptr->add_leaving_endpoint(endpoint);
            ss.update_pending_ranges(std::move(tmlock), std::move(tmptr), format("removenode {}", endpoint)).get();
...
void storage_service::excise(std::unordered_set<token> tokens, inet_address endpoint) {
    ...
    update_pending_ranges(std::move(tmlock), std::move(tmptr), format("excise {}", endpoint)).get();
}
...
void storage_service::leave_ring() {
    ...
    auto tmlock = get_token_metadata_lock().get0();
    auto tmptr = get_mutable_token_metadata_ptr().get0();
    tmptr->remove_endpoint(get_broadcast_address());
    update_pending_ranges(std::move(tmlock), std::move(tmptr), format("leave_ring {}", endpoint)).get();
...
future<> storage_service::start_leaving() {
    return _gossiper.add_local_application_state(application_state::STATUS, versioned_value::leaving(db::system_keyspace::get_local_tokens().get0())).then([this] {
        ...
        auto tmptr = get_mutable_token_metadata_ptr().get0();
        auto endpoint = get_broadcast_address();
        tmptr->add_leaving_endpoint(get_broadcast_address());
        return update_pending_ranges(std::move(tmlock), std::move(tmptr), format("start_leaving {}", endpoint));
    });
}
...
future<locator::token_metadata_lock> storage_service::get_token_metadata_lock() {
    assert(this_shard_id() == 0);
    return _shared_token_metadata.get_lock();
}

future<> storage_service::update_pending_ranges(token_metadata_lock tmlock, token_metadata_ptr tmptr, sstring reason) {
    assert(this_shard_id() == 0);
    ...
    return do_with(_db.local().get_non_system_keyspaces(), [this, tmptr] (auto& keyspaces){
    ...
    }).then_wrapped([this, reason = std::move(reason), tmlock = std::move(tmlock), tmptr] (future<> f) mutable {
        if (f.failed()) {
        ...
        return replicate_to_all_cores(std::move(tmlock), std::move(tmptr));
    });
...
future<> storage_service::update_pending_ranges(sstring reason) {
    return get_token_metadata_lock().then([this, reason = std::move(reason)] (token_metadata_lock tmlock) mutable {
      return get_mutable_token_metadata_ptr().then([this, reason = std::move(reason), tmlock = std::move(tmlock)] (token_metadata_ptr tmptr) mutable {
        return update_pending_ranges(std::move(tmlock), std::move(tmptr), std::move(reason));
      });
    });
}
...
future<> storage_service::update_topology(inet_address endpoint) {
    return service::get_storage_service().invoke_on(0, [endpoint] (auto& ss) {
      return ss.get_token_metadata_lock().then([&ss, endpoint] (token_metadata_lock tmlock) {
       return ss.get_mutable_token_metadata_ptr().then([&ss, endpoint, tmlock = std::move(tmlock)] (token_metadata_ptr tmptr) mutable {
        // initiate the token metadata endpoints cache reset
        tmptr->invalidate_cached_rings();
        // re-read local rack and DC info
        tmptr->update_topology(endpoint);
        return ss.replicate_to_all_cores(std::move(tmlock), std::move(tmptr));
       });
      });
    });
}
