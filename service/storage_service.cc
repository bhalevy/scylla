...
storage_service::storage_service(abort_source& abort_source, distributed<database>& db, gms::gossiper& gossiper, sharded<db::system_distributed_keyspace>& sys_dist_ks,
        ...
        locator::shared_token_metadata& tm, sharded<netw::messaging_service>& ms, bool for_testing)
        ...
        , _token_metadata(tm.get_mutable())
    ...
    if (should_bootstrap()) {
        ...
        if (_bootstrap_tokens.empty()) {
            ...
            if (initial_tokens.size() < 1) {
                _bootstrap_tokens = boot_strapper::get_random_tokens(tm, num_tokens);
                if (num_tokens == 1) {
...
void storage_service::bootstrap() {
    ...
    if (!db().local().is_replacing()) {
        ...
        if (db().local().get_config().check_experimental(db::experimental_features_t::CDC)) {
            ...
            tm.add_bootstrap_tokens(_bootstrap_tokens, get_broadcast_address());
            update_pending_ranges().get();
    ...
    if (is_repair_based_node_ops_enabled()) {
        if (db().local().is_replacing()) {
            replace_with_repair(_db, _messaging, tm, _bootstrap_tokens).get();
        } else {
            bootstrap_with_repair(_db, _messaging, tm, _bootstrap_tokens).get();
        }
    } else {
        dht::boot_strapper bs(_db, _abort_source, get_broadcast_address(), _bootstrap_tokens, tm);
