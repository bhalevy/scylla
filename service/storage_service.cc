...
future<> storage_service::join_token_ring(cdc::generation_service& cdc_gen_service,
    ...
    if (_feature_service.quarantined_hosts) {
        ...
        if (!quarantined_hosts.empty()) {
            auto v = versioned_value::host_infos(quarantined_hosts);
...
storage_service::host_info storage_service::get_host_info(const locator::token_metadata& tm, gms::inet_address endpoint) const noexcept {
    ...
    try {
        host.id = tm.get_host_id(endpoint);
    } catch (...) {
    ...
    if (topo.has_endpoint(endpoint, locator::topology::pending::yes)) {
        host.info.dc_rack =  topo.get_location(endpoint);
    }
    return host;
}
...
future<> storage_service::remove_endpoint(host_info host) {
    const auto& endpoint = host.info.endpoint;
    co_await quarantine_host(host);
    slogger.info("Removing endpoint {}/{}", host.id, endpoint);
