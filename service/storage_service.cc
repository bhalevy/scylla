    ...
    co_await mutate_token_metadata(seastar::coroutine::lambda([this, &id2ip, &am] (mutable_token_metadata_ptr tmptr) -> future<> {
        ...
        auto add_normal_node = [&] (raft::server_id id, const replica_state& rs) -> future<> {
            ...
            if (!is_me(ip)) {
                ...
                info.update("tokens", db::system_keyspace::make_tokens_data_value(rs.ring.value().tokens));
                info.update("data_center", rs.datacenter);
...
future<> storage_service::join_token_ring(sharded<db::system_distributed_keyspace>& sys_dist_ks,
    ...
    gms::endpoint_state::map_type app_states;
...
future<> storage_service::handle_state_normal(inet_address endpoint, gms::permit_id pid) {
    ...
    if (!is_me(endpoint) && !owned_tokens.empty() && !endpoints_to_remove.count(endpoint)) {
        try {
            ...
            info.update("tokens", db::system_keyspace::make_tokens_data_value(owned_tokens));
...
future<> storage_service::on_change(gms::inet_address endpoint, const gms::endpoint_state::map_type& states_, gms::permit_id pid) {
...
data_values_map storage_service::get_peer_info_for_update(inet_address endpoint, const gms::endpoint_state::map_type& app_state_map) {
