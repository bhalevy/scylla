...
std::unordered_set<locator::host_id_and_or_endpoint> storage_service::parse_node_list(sstring comma_separated_list) {
    std::vector<sstring> ignore_nodes_strs = utils::split_comma_separated_list(std::move(comma_separated_list));
    std::unordered_set<locator::host_id_and_or_endpoint> ignore_nodes;
    for (const sstring& n : ignore_nodes_strs) {
        try {
            ignore_nodes.insert(locator::host_id_and_or_endpoint(n));
        } catch (...) {
