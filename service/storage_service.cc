...
future<> storage_service::join_token_ring(cdc::generation_service& cdc_gen_service,
    ...
    if (is_replacing()) {
        ...
        auto replace_address = ri->address;
...
future<> storage_service::bootstrap(cdc::generation_service& cdc_gen_service, std::unordered_set<token>& bootstrap_tokens, std::optional<cdc::generation_id>& cdc_gen_id, const std::optional<replacement_info>& ri) {
    return seastar::async([this, &bootstrap_tokens, &cdc_gen_id, &cdc_gen_service, &ri] {
...
future<> storage_service::on_change(inet_address endpoint, application_state state, const versioned_value& value) {
    ...
    if (state == application_state::STATUS) {
        ...
        if (move_name == sstring(versioned_value::STATUS_BOOTSTRAPPING)) {
            ...
            slogger.error("endpoint={} went into HIBERNATE state, this is no longer supported.  Upgrade the node to the current version.", endpoint);
        } else {
...
// Runs inside seastar::async context
void storage_service::run_replace_ops(std::unordered_set<token>& bootstrap_tokens, const replacement_info& ri) {
    auto replace_address = ri.address;
    auto uuid = utils::make_random_uuid();
    auto tmptr = get_token_metadata_ptr();
