...
future<> storage_service::topology_state_load() {
    ...
    for (const auto& e: tmptr->get_all_endpoints()) {
        if (!is_me(e) && !_gossiper.get_endpoint_state_ptr(e)) {
            ...
            co_await _gossiper.add_saved_endpoint(e, std::move(st));
        }
    }
    ...
    for (auto hioa : ignore_hioas) {
        ...
        if (!hioa.has_endpoint()) {
            ...
            if (res.size() == 0) {
                ...
                throw std::runtime_error(::format("Find multiple nodes to ignore with host_id {}: {}", hioa.id, res));
            }
        ...
        tmptr->update_topology(hioa.endpoint, opt_dc_rack);
        if (hioa.id) {
            tmptr->update_host_id(hioa.id, hioa.endpoint);
        }
        co_await tmptr->update_normal_tokens(esp->get_tokens(), hioa.endpoint);
