...
public:
    ...
    future<> send_to_all(node_ops_cmd cmd) {
        ...
        if (!nodes_failed.empty()) {
            errors.emplace_back(format("The {} command failed on nodes={}", op_desc, nodes_failed));
        }
        ...
        if (!nodes_down.empty()) {
            errors.emplace_back(format("The {} command failed on nodes={}: the needed nodes are down. It is highly recommended to fix the down nodes and try again", op_desc, nodes_failed));
        }
        if (!errors.empty()) {
            co_await coroutine::return_exception(std::runtime_error(boost::algorithm::join(errors, "; ")));
        }
