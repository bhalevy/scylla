...
void storage_service::bootstrap() {
    ...
    slogger.info("bootstrap: rbno={} replacing={}", bootstrap_rbno, _db.local().is_replacing());
    if (!_db.local().is_replacing()) {
    ...
    try {
        ...
        co_await container().invoke_on_others([&, base_shard] (storage_service& ss) -> future<> {
            ...
            for (auto& ks_name : keyspaces) {
                ...
                auto erm = make_effective_replication_map_ptr(std::move(local_rs), std::move(local_tmptr), std::move(local_replication_map));
