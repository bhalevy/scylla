...
std::unordered_set<gms::inet_address> storage_service::parse_ignore_nodes(sstring comma_separated_list, const token_metadata& tm) {
...
void storage_service::run_bootstrap_ops(std::unordered_set<token>& bootstrap_tokens) {
    ...
    ctl.start("bootstrap", [this_node = get_broadcast_address()] (gms::inet_address node) {
    ...
    for (;;) {
        ...
        if (pending_ops.empty()) {
            ...
            ctl.refresh_sync_nodes([this_node = get_broadcast_address()] (gms::inet_address node) {
                return node != this_node;
            });
        }
    }
...
void storage_service::run_replace_ops(std::unordered_set<token>& bootstrap_tokens, replacement_info replace_info) {
    ...
    ctl.ignore_nodes = parse_ignore_nodes(_db.local().get_config().ignore_dead_nodes_for_replace(), *ctl.tmptr);

    // Step 1: Decide who needs to sync data for replace operation
    ctl.start("replace", [&, this_node = get_broadcast_address()] (gms::inet_address node) {
        return node != replace_address && node != this_node;
    });
