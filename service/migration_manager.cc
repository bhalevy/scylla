...
void migration_manager::schedule_schema_pull(locator::host_id host_id, const gms::endpoint_state& state)
{
    ...
    if (!_gossiper.is_me(host_id) && value) {
        // FIXME: discarded future
        (void)maybe_schedule_schema_pull(table_schema_version(utils::UUID{value->value()}), host_id).handle_exception([host_id] (auto ep) {
            mlogger.warn("Fail to pull schema from {}: {}", host_id, ep);
        });
    }
}
 ...
 */
future<> migration_manager::maybe_schedule_schema_pull(const table_schema_version& their_version, locator::host_id host_id)
{
...
bool migration_manager::has_compatible_schema_tables_version(locator::host_id host_id) {
    auto* version = _gossiper.get_application_state_ptr(host_id, gms::application_state::SCHEMA_TABLES_VERSION);
    return version && version->value() == db::schema_tables::version;
}

bool migration_manager::should_pull_schema_from(locator::host_id host_id) {
...
future<> migration_manager::on_join(locator::host_id host_id, gms::endpoint_state_ptr ep_state, gms::permit_id) {
    schedule_schema_pull(node, *ep_state);
    return make_ready_future();
}

future<> migration_manager::on_change(locator::host_id host_id, gms::application_state state, const gms::versioned_value& value, gms::permit_id) {
...
future<> migration_manager::on_alive(locator::host_id host_id, gms::endpoint_state_ptr state, gms::permit_id) {
