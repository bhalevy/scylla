    ...
    try {
        ...
        co_await seastar::async([&] {
            ...
            for (auto&& name : diff.entries_only_on_left) {
                ...
                schema_ptr view;
                auto index_table_name = secondary_index::index_table_name(name);
                try {
                    view = db.find_schema(old_schema->ks_name(), index_table_name);
                } catch (...) {
                    mlogger.error("Could not find schema for index {}.{} id={}", old_schema->ks_name(), index_table_name, index.id());
                    continue;
                }
