...
storage_proxy::create_write_response_handler_helper(schema_ptr s, const dht::token& token, std::unique_ptr<mutation_holder> mh,
    ...
    inet_address_vector_replica_set natural_endpoints = ers->get_natural_endpoints_without_node_being_replaced(token);
    inet_address_vector_topology_change pending_endpoints = ers->get_token_metadata_ptr()->pending_endpoints_for(token, keyspace_name);
...
storage_proxy::get_paxos_participants(const sstring& ks_name, const dht::token &token, db::consistency_level cl_for_paxos) {
    ...
    auto ers = ks.get_effective_replication_strategy();
    inet_address_vector_replica_set natural_endpoints = ers->get_natural_endpoints_without_node_being_replaced(token);
    inet_address_vector_topology_change pending_endpoints = ers->get_token_metadata_ptr()->pending_endpoints_for(token, ks_name);
...
inet_address_vector_replica_set storage_proxy::get_live_endpoints(keyspace& ks, const dht::token& token) const {
    auto& rs = ks.get_replication_strategy();
    auto ers = ks.get_effective_replication_strategy();
    inet_address_vector_replica_set eps = ers->get_natural_endpoints_without_node_being_replaced(token);
