...
storage_proxy::query_partition_key_range_concurrent(storage_proxy::clock_type::time_point timeout,
  ...
  for (;;) {
    ...
    if (!wrapped_result) {
        auto error = bo::failure(std::move(wrapped_result).assume_error()).error();
        auto ex = error.as_exception_future().get_exception();
        p->handle_read_error(std::move(error), true);
        co_return coroutine::return_exception_ptr(std::move(ex));
    }
