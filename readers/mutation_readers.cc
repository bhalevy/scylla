...
public:
    compacting_reader(flat_mutation_reader_v2 source, utils::optional_reference<const compaction_manager> cm, gc_clock::time_point compaction_time,
            std::function<api::timestamp_type(const dht::decorated_key&)> get_max_purgeable,
            streamed_mutation::forwarding fwd = streamed_mutation::forwarding::no)
        : impl(source.schema(), source.permit())
        , _reader(std::move(source))
        , _compactor(*_schema, cm, compaction_time, get_max_purgeable)
...
flat_mutation_reader_v2 make_compacting_reader(flat_mutation_reader_v2 source, utils::optional_reference<const compaction_manager> cm, gc_clock::time_point compaction_time,
    ...
    return make_flat_mutation_reader_v2<compacting_reader>(std::move(source), cm, compaction_time, get_max_purgeable, fwd);
}
