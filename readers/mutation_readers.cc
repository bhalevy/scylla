...
make_flat_mutation_reader_from_mutations_v2(
    ...
    class reader final : public reader_from_mutation_base {
        mutation _mutation;
        bool _reversed;
        mutation_consume_cookie _cookie;
    ...
    public:
        ...
        virtual future<> fill_buffer() override {
            ...
            auto res = std::move(_mutation).consume(*this, consume_in_reverse(_reversed), std::move(_cookie));
        ...
        virtual future<> fast_forward_to(const dht::partition_range& pr) override {
            ...
            if (pr.after(_mutation.decorated_key(), cmp)) {
                _end_of_stream = true;
            }
            return make_ready_future<>();
        }
...
make_flat_mutation_reader_from_mutations_v2(schema_ptr schema, reader_permit permit, std::vector<mutation> ms, const query::partition_slice& slice, streamed_mutation::forwarding fwd) {
    if (ms.size() == 1) {
        auto m = std::move(ms.back());
        ms.pop_back();
        return make_flat_mutation_reader_from_mutations_v2(std::move(schema), std::move(permit), std::move(m), slice, fwd);
    }
...
make_flat_mutation_reader_from_mutations_v2(schema_ptr s, reader_permit permit, std::vector<mutation> mutations, const dht::partition_range& pr, streamed_mutation::forwarding fwd) {
    if (mutations.size() == 1 && &pr == &query::full_partition_range) {
...
flat_mutation_reader_v2 make_flat_mutation_reader_from_mutations_v2(
    ...
    if (ms.size() == 1) {
        auto m = std::move(ms.back());
        ms.pop_back();
        return make_flat_mutation_reader_from_mutations_v2(std::move(schema), std::move(permit), std::move(m), fwd);
    }
