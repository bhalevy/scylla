...
make_flat_mutation_reader_from_mutation_v2(
    ...
    mutation&& sliced_mutation = reversed
        ? slice_mutation(s->make_reversed(), std::move(m), query::half_reverse_slice(*s, slice))
        : slice_mutation(s, std::move(m), slice);
    return make_flat_mutation_reader_from_mutation_v2(std::move(s), std::move(permit), std::move(sliced_mutation), fwd, reversed);
}
