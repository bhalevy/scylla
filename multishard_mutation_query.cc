...
future<> read_context::destroy_reader(shard_id shard, future<stopped_reader> reader_fut) noexcept {
    return try_with_gate(_dismantling_gate, [this, shard, reader_fut = std::move(reader_fut)] () mutable {
...
future<> read_context::stop() {
    if (_dismantling_gate.is_closed()) {
        co_return;
    }
    co_await _dismantling_gate.close();
...
future<> read_context::save_reader(shard_id shard, const dht::decorated_key& last_pkey, const std::optional<clustering_key_prefix>& last_ckey) {
    return _db.invoke_on(shard, [this, shard, query_uuid = _cmd.query_uuid, query_ranges = _ranges, rm = std::exchange(_readers[shard], {}),
        ...
        try {
          ...
          auto maybe_next_partition = rm.has_pending_next_partition ? reader->next_partition() : make_ready_future<>();
          return maybe_next_partition.then([this, shard, query_uuid, query_ranges, rm = std::move(rm), &last_pkey, &last_ckey, gts = std::move(gts), &db, reader = std::move(reader)] () mutable {
