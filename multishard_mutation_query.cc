...
public:
    ...
    virtual future<reader_permit> obtain_reader_permit(schema_ptr schema, const char* const description, db::timeout_clock::time_point timeout) override {
        ...
        if (rm.state == reader_state::successful_lookup) {
            ...
            // FIXME: The following check can be removed eventually once tested throughly.
            auto& permit = rm.rparts->permit;
            if (permit.timeout() != timeout) {
                if (permit.timeout() != db::no_timeout) {
                    on_internal_error_noexcept(mmq_log, format("obtain_reader_permit: timeout={} != permit.timeout={}",
...
flat_mutation_reader read_context::create_reader(
    ...
    if (rm.state == reader_state::successful_lookup) {
        if (auto reader_opt = semaphore().unregister_inactive_read(std::move(*rm.rparts->handle))) {
            ...
            // The reader_meta permit is expected to be the same one used for create_reader.
            // FIXME: The following check can be removed eventually once tested throughly.
            if (reader_opt->timeout() != permit.timeout()) {
                on_internal_error_noexcept(mmq_log, format("read_context::create_reader: reader.timeout={} != permit.timeout={}",
                    reader_opt->timeout().time_since_epoch().count(),
                    permit.timeout().time_since_epoch().count()));
                reader_opt->set_timeout(permit.timeout());
            }
