...
node_ptr topology::add_node(mutable_node_ptr node) {
    ...
    // Note that _all_nodes contains also the _lock_node
...
node_ptr topology::update_node(node_ptr node, std::optional<host_id> opt_id, std::optional<inet_address> opt_ep, std::optional<endpoint_dc_rack> opt_dr) {
    ...
    if (opt_id) {
        if (*opt_id != node->host_id()) {
            if (!*opt_id) {
                on_internal_error(tlogger, format("Updating node host_id to null is disallowed: node={} new host_id={}", node, *opt_id));
            }
            if (node->is_local() && node->host_id()) {
                on_internal_error(tlogger, format("Local node host_id must not change: node={} new host_id={}", node, *opt_id));
            }
