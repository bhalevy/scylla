...
std::weak_ordering topology::compare_endpoints(const inet_address& address, const inet_address& a1, const inet_address& a2) const {
    // The following will create default metrics for addresses not yet in topology.
    // Previously, we'd use the default dc/rack for them.
    auto& metrics = const_cast<std::unordered_map<inet_address, location_metric>&>(_node_metrics);
    auto m = metrics[address];
    auto d1 = std::abs(metrics[a1] - m);
    auto d2 = std::abs(metrics[a2] - m);
