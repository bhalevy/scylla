...
topology::node::node(::locator::host_id id, inet_address endpoint, endpoint_dc_rack dc_rack, local is_local)
    : _host_id(id)
    , _endpoint(endpoint)
...
void topology::node::unlink() noexcept {
...
topology::node_ptr topology::find_node(host_id id, must_exist must_exist) const noexcept {
    auto it = _nodes_by_host_id.find(id);
    if (it != _nodes_by_host_id.end()) {
...
// Returns nullptr if not found
topology::node_ptr topology::find_node(const inet_address& ep, must_exist must_exist) const noexcept {
...
void topology::update_endpoint(inet_address ep, std::optional<host_id> opt_id, std::optional<endpoint_dc_rack> opt_dr, must_exist must_exist)
{
    tlogger.trace("topology[{}]: update_endpoint: ep={} host_id={} dc={} rack={} must_exist={}, at {}", fmt::ptr(this),
            ep, opt_id.value_or(host_id::create_null_id()), opt_dr.value_or(endpoint_dc_rack{}).dc, opt_dr.value_or(endpoint_dc_rack{}).rack, must_exist,
            current_backtrace());
    node_ptr n = find_node(ep);
    if (n) {
        update_node(const_cast<node*>(n.get())->shared_from_this(), opt_id, std::nullopt, std::move(opt_dr));
    } else if (must_exist) {
        on_internal_error(tlogger, format("endpoint {} not found", ep));
    } else {
