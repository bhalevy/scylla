...
void topology::update_endpoint(const inet_address& ep, endpoint_dc_rack dr)
{
    ...
    _node_location_map[ep] = (dc_metric << 48) | (rack_metric << 32) | node_metric;
}
...
std::strong_ordering topology::compare_endpoints(const inet_address& address, const inet_address& a1, const inet_address& a2) const {
    auto m = _node_location_map.at(address);
    auto d1 = std::abs(_node_location_map.at(a1) - m);
    auto d2 = std::abs(_node_location_map.at(a2) - m);
