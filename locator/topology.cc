...
template <typename T>
int topology::distance(const T& address, const endpoint_dc_rack& loc, const T& address1, const endpoint_dc_rack& loc1) noexcept {
    ...
    int same_dc = loc1.dc == loc.dc;
    int same_rack = same_dc & (loc1.rack == loc.rack);
    int same_node = address1 == address;
    return ((same_dc << 2) | (same_rack << 1) | same_node) ^ 7;
}
