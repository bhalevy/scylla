...
future<> token_metadata_impl::clear_gently() noexcept {
    co_await _clear_gently(_token_to_endpoint_map);
    co_await _clear_gently(_endpoint_to_host_id_map);
    co_await _clear_gently(_bootstrap_tokens);
    co_await _clear_gently(_leaving_endpoints);
    co_await _clear_gently(_replacing_endpoints);
    co_await _clear_gently(_pending_ranges_interval_map);
    co_await _clear_gently(_sorted_tokens);
...
inline future<> topology::clear_gently() noexcept {
    co_await _clear_gently(_dc_endpoints);
    co_await _clear_gently(_dc_racks);
    co_await _clear_gently(_current_locations);
    co_return;
}
