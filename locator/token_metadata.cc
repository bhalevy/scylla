...
private:
    const std::unordered_multimap<range<token>, inet_address>& get_pending_ranges_mm(sstring keyspace_name) const;
...
public:
    ...
    const std::unordered_map<range<token>, std::unordered_set<inet_address>>& get_pending_ranges(sstring keyspace_name) const;
...
const std::unordered_multimap<range<token>, inet_address>&
token_metadata_impl::get_pending_ranges_mm(sstring keyspace_name) const {
    return _pending_ranges.at(keyspace_name);
}

const std::unordered_map<range<token>, std::unordered_set<inet_address>>&
token_metadata_impl::get_pending_ranges(sstring keyspace_name) const {
    return _pending_ranges_map.at(keyspace_name);
}
...
std::vector<gms::inet_address> token_metadata_impl::pending_endpoints_for(const token& token, const sstring& keyspace_name) const {
    ...
    const auto& keyspace_map = _pending_ranges_interval_map.at(keyspace_name);
    if (keyspace_map.empty()) {
        return {};
    }
    ...
    const auto it = keyspace_map.find(interval);
    if (it != keyspace_map.end()) {
...
const std::unordered_map<range<token>, std::unordered_set<inet_address>>&
token_metadata::get_pending_ranges(sstring keyspace_name) const {
