...
future<tablet_map> network_topology_strategy::allocate_tablets_for_new_table(schema_ptr s, token_metadata_ptr tm, uint64_t target_tablet_size, std::optional<unsigned> initial_scale) const {
    ...
    if (tablet_count == 0) {
        tablet_count = tablet_hints().min_tablet_count.value_or(0);
        ...
        tablet_count = std::max<size_t>(tablet_count, calculate_initial_tablets_from_topology(*s, tm, _dc_rep_factor, tablet_hints().min_per_shard_tablet_count.value_or(0)));
    }
