...
class local_replication_strategy;
...
public:
    ...
    const local_replication_strategy* maybe_as_local() const;
...
/// Replication strategies which support local replication extend this trait.
///
/// It will be accessed only if the replication strategy is actually node-local,
/// that is after mark_as_local() is called, and as a result
/// abstract_replication_strategy::is_local() returns true.
class local_replication_strategy {
public:
    virtual ~local_replication_strategy() = default;
    virtual effective_replication_map_ptr make_replication_map(table_id, token_metadata_ptr) const = 0;
};
