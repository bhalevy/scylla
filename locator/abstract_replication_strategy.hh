...
public:
    ...
    virtual stop_iteration for_each_natural_endpoint_until(const noncopyable_function<stop_iteration(const inet_address&)>&, const token& search_token, const effective_replication_map& erm) const;
...
public:
    ...
    virtual stop_iteration for_each_natural_endpoint_until(const noncopyable_function<stop_iteration(const inet_address&)>&, const token& search_token) const;
...
private:
    dht::token_range_vector do_get_ranges(noncopyable_function<std::optional<bool>(const inet_address&)> should_add_range) const;
