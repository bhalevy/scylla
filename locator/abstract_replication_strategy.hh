...
public:
    explicit effective_replication_map(abstract_replication_strategy::ptr_type rs, token_metadata_ptr tmptr, replication_map all_endpoints) noexcept
        : _rs(std::move(rs))
...
private:
    ...
    requires std::is_invocable_r_v<bool, Pred, inet_address_vector_replica_set>
    dht::token_range_vector do_get_ranges(Pred should_add_range) const;
};
...
inline mutable_effective_replication_map_ptr make_effective_replication_map_ptr(abstract_replication_strategy::ptr_type rs, token_metadata_ptr tmptr, replication_map all_endpoints) {
    return make_lw_shared<effective_replication_map>(std::move(rs), std::move(tmptr), std::move(all_endpoints));
}
