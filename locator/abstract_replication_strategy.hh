...
public:
    ...
    dht::token_range_vector get_ranges(inet_address ep, token_metadata_ptr tmptr) const;
    // Caller must ensure that token_metadata will not change throughout the call
    dht::token_range_vector get_ranges_in_thread(inet_address ep, token_metadata_ptr tmptr) const;
private:
    // Caller must ensure that token_metadata will not change throughout the call if can_yield==true.
    dht::token_range_vector do_get_ranges(inet_address ep, token_metadata_ptr tmptr, bool can_yield) const;
...
public:
    ...
    dht::token_range_vector get_pending_address_ranges(token_metadata_ptr tmptr, token pending_token, inet_address pending_address) const;

    dht::token_range_vector get_pending_address_ranges(token_metadata_ptr tmptr, std::unordered_set<token> pending_tokens, inet_address pending_address) const;
};
