...
stop_iteration everywhere_replication_strategy::for_each_natural_endpoint_until(const noncopyable_function<stop_iteration(const inet_address&)>& func, const token&, const effective_replication_map& erm) const {
    const auto& tm = *erm.get_token_metadata_ptr();
    if (tm.sorted_tokens().empty()) {
