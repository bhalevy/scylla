...
effective_replication_map::do_get_ranges(Pred should_add_range) const {
    dht::token_range_vector ret;
...
future<mutable_effective_replication_map_ptr> make_effective_replication_map(abstract_replication_strategy::ptr_type rs, token_metadata_ptr tmptr) {
    ...
    for (const auto &t : tmptr->sorted_tokens()) {
        all_endpoints.emplace(t, co_await rs->calculate_natural_endpoints(t, *tmptr));
    }

    co_return make_effective_replication_map_ptr(std::move(rs), std::move(tmptr), std::move(all_endpoints));
}
