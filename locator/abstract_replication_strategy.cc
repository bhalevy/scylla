...
const local_replication_strategy* abstract_replication_strategy::maybe_as_local() const {
    if (!is_local()) {
        return nullptr;
    }
    return dynamic_cast<const local_replication_strategy*>(this);
}
...
future<keyspace_effective_replication_map_ptr> effective_replication_map_factory::create_keyspace_effective_replication_map(replication_strategy_ptr rs, token_metadata_ptr tmptr) {
    ...
    if (rs->is_local()) {
        // Local replication strategy does not use effective replication maps
        // and should not be registered in the factory.
