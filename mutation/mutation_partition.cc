...
void mutation_partition::for_each_row(const schema& schema, const query::clustering_range& row_range, query::reversed reversed, Func&& func) const
{
    check_schema(schema);
    ...
    const tombstone_gc_state& gc_state)
{
    ...
    bool drop_tombstones_unconditionally = false;
    do_compact(s, dk, compaction_time, all_rows, true, query::reversed::no, query::partition_max_rows, can_gc, drop_tombstones_unconditionally, gc_state);
}
...
void mutation_partition::compact_for_compaction_drop_tombstones_unconditionally(const schema& s, const dht::decorated_key& dk)
{
    ...
    do_compact(s, dk, compaction_time, all_rows, true, query::reversed::no, query::partition_max_rows, always_gc, drop_tombstones_unconditionally, tombstone_gc_state(nullptr));
}
...
to_data_query_result(const reconcilable_result& r, schema_ptr s, const query::partition_slice& slice, uint64_t max_rows, uint32_t max_partitions,
    ...
    if (slice.is_reversed()) {
        ...
        mplog.debug("to_data_query_result: schema={} consume_in_reverse::yes", s->version());
    } else {
        mplog.debug("to_data_query_result: schema={} consume_in_reverse::no", s->version());
    }
...
query_mutation(mutation&& m, const query::partition_slice& slice, uint64_t row_limit, gc_clock::time_point now, query::result_options opts) {
    ...
    if (slice.is_reversed()) {
        reverse = consume_in_reverse::yes;
        mplog.debug("query_mutation: schema={} consume_in_reverse::yes", m.schema()->version());
    } else {
        mplog.debug("query_mutation: schema={} consume_in_reverse::no", m.schema()->version());
    }
