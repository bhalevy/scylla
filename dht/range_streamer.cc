...
// Must be called from a seastar thread
std::unordered_map<dht::token_range, std::vector<inet_address>>
range_streamer::get_all_ranges_with_strict_sources_for(gms::gossiper& gossiper, const sstring& keyspace_name, dht::token_range_vector desired_ranges) {
    logger.debug("{} ks={}", __func__, keyspace_name);
...
future<> range_streamer::add_ranges(gms::gossiper& gossiper, const sstring& keyspace_name, dht::token_range_vector ranges) {
  return seastar::async([this, keyspace_name, ranges= std::move(ranges), &gossiper] () mutable {
    ...
    auto ranges_for_keyspace = use_strict_sources_for_ranges(keyspace_name)
        ? get_all_ranges_with_strict_sources_for(gossiper, keyspace_name, ranges)
