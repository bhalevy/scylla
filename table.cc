...
filter_sstable_for_reader_by_ck(std::vector<sstables::shared_sstable>&& sstables, column_family& cf, const schema_ptr& schema,
    ...
    auto skipped = std::partition(sstables.begin(), sstables.end(), [ranges = std::move(ck_filtering_all_ranges)] (const sstables::shared_sstable& sst) {
                                 ...
                                 mutation_reader::forwarding fwd_mr)
{
    ...
    //
    // This is trivial when `readers` is empty (otherwise we would have returned
    // an empty flat reader based on !num_sstables above), so use
    // `flat_mutation_reader_from_mutations` with an empty mutation to emit
    // the partition_start/end pair.  Otherwise, add a `flat_mutation_reader_from_mutations`
    // to the list of readers to ensure partition_start/end are emitted in any case.
    auto num_readers = readers.size();
    if (!num_readers) {
        return flat_mutation_reader_from_mutations({mutation(schema, *pk)}, slice, fwd);
    } else if (num_readers != num_sstables) {
