...
filter_sstable_for_reader_by_ck(std::vector<sstables::shared_sstable>&& sstables, column_family& cf, const schema_ptr& schema,
    ...
    auto sstable_has_tombstone_or_clustering_key = [&schema, &ck_filtering_all_ranges] (const sstables::shared_sstable& sst) {
        ...
        if (!sst->has_scylla_component()) {
            ...
            if (stats.estimated_tombstone_drop_time.bin.size()) {
                return true;
            }
        }
                                 ...
                                 mutation_reader::forwarding fwd_mr)
{
    ...
    flat_mutation_reader reader = num_readers ?
            make_combined_reader(schema, std::move(readers), fwd, fwd_mr) :
            make_empty_flat_reader(schema);
    if (num_readers) {
    ...
    return make_partition_exist(std::move(reader), *pk);
}
