...
filter_sstable_for_reader(std::vector<sstables::shared_sstable>&& sstables, column_family& cf, const schema_ptr& schema,
    ...
    if (filtering_broken || !schema->clustering_key_size() || !cf.get_compaction_strategy().use_clustering_key_filter() || slice.static_columns.size()) {
         return sstables;
    }
    ...
    auto sstable_has_tombstone_or_clustering_key = [&schema, &ranges] (const sstables::shared_sstable& sst) {
        ...
        return stats.estimated_tombstone_drop_time.bin.size() || contains_rows(*sst, schema, ranges);
    };
