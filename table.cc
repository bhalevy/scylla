...
table::query(schema_ptr s,
    ...
    auto accounter = co_await (opts.request == query::result_request::only_digest
        ...
        lw_shared_ptr<query::result> res;
        try {
            res = make_lw_shared<query::result>(qs.builder.build());
        } catch (...) {
            ex = std::current_exception();
        }
        qs_ptr.reset();
        auto finally = [lc, this, leave = std::move(leave)]() mutable {
    ...
    try {
        ...
        if (ex) {
            throw seastar::nested_exception(std::current_exception(), std::move(ex));
        }
        throw;
    }
    if (ex) {
