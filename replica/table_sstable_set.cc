...
public:
    table_incremental_selector(table& t)
            ...
            , _sg_manager(t.get_storage_group_manager())
    {
        if (auto cg = t.single_compaction_group_if_available()) {
    ...
    virtual std::tuple<dht::partition_range, std::vector<shared_sstable>, dht::ring_position_ext> select(const dht::ring_position_view& pos) override {
        ...
        if (!_cur_selector) {
            ...
            auto* sg = _sg_manager.storage_group_at(idx);
            if (!sg) {
...
private:
    ...
    dht::token find_lowest_next_token(dht::token token) {
        ...
        storage_group* sg;
        while ((sg = _sg_manager.storage_group_at(idx)) == nullptr) {
...
table_sstable_set::create_single_key_sstable_reader(
    ...
    auto* sg = _sg_manager.storage_group_for_token(token);
    if (!sg) {
