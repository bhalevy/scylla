...
future<> database::snapshot_table_on_all_shards(sharded<database>& sharded_db, std::string_view ks_name, sstring table_name, sstring tag, bool skip_flush, bool snap_views) {
    if (!skip_flush) {
    ...
    if (snap_views) {
        for (const auto& vp : table_shards[this_shard_id()]->views()) {
            bool snap_views_of_view = false;
            co_await snapshot_table_on_all_shards(sharded_db, ks_name, vp->cf_name(), tag, skip_flush, snap_views_of_view);
        }
    }
}

future<> database::snapshot_tables_on_all_shards(sharded<database>& sharded_db, std::string_view ks_name, std::vector<sstring> table_names, sstring tag, bool skip_flush, bool snap_views) {
    return parallel_for_each(table_names, [&sharded_db, ks_name, tag = std::move(tag), skip_flush, snap_views] (auto& table_name) {
        return snapshot_table_on_all_shards(sharded_db, ks_name, std::move(table_name), tag, skip_flush, snap_views);
    });
}
