...
future<compaction::owned_ranges_ptr> maybe_make_owned_ranges_ptr(locator::effective_replication_map_ptr erm, locator::host_id host_id) {
    if (!erm) {
        co_return nullptr;
    }
    ...
    co_return compaction::make_owned_ranges_ptr(co_await erm->get_ranges(host_id));
}
...
future<> database::add_column_family(keyspace& ks, schema_ptr schema, column_family::config cfg, is_new_cf is_new, locator::token_metadata_ptr not_commited_new_metadata) {
    ...
    compaction::owned_ranges_ptr owned_ranges_ptr;  // For vnode-based replication
