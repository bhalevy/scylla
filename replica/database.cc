...
database::database(const db::config& cfg, database_config dbcfg, service::migration_notifier& mn, gms::feature_service& feat, const locator::shared_token_metadata& stm,
        service::system_controller& sc, sharded<semaphore>& sst_dir_sem, utils::cross_shard_barrier barrier)
    ...
    , _compaction_manager(make_compaction_manager(_cfg, dbcfg, sc.get_abort_source()))
...
void database::add_column_family(keyspace& ks, schema_ptr schema, column_family::config cfg) {
    ...
    if (cfg.enable_commitlog && _commitlog) {
       cf = make_lw_shared<column_family>(schema, std::move(cfg), *_commitlog, _system_controller, *_compaction_manager, *_cl_stats, _row_cache_tracker);
    } else {
       cf = make_lw_shared<column_family>(schema, std::move(cfg), column_family::no_commitlog(), _system_controller, *_compaction_manager, *_cl_stats, _row_cache_tracker);
    }
