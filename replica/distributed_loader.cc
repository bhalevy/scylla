...
distributed_loader::reshape(sharded<sstables::sstable_directory>& dir, sharded<replica::database>& db, compaction::reshape_mode mode,
    ...
    auto& compaction_module = db.local().get_compaction_manager().get_task_manager_module();
    bool skip_orphaned_sstables = true;
    auto task = co_await compaction_module.make_and_start_task<compaction::table_reshaping_compaction_task_impl>({}, std::move(ks_name), std::move(table_name), dir, db, mode, std::move(creator), std::move(filter), skip_orphaned_sstables);
    co_await task->done();
}
