...
collect_sstables_for_reshard(sharded<sstables::sstable_directory>& dir, compaction::owned_ranges_ptr owned_ranges_ptr) {
    ...
    co_await dir.invoke_on_all([&info_vec, coordinator, owned_ranges_ptr] (sstables::sstable_directory& d) -> future<> {
        ...
        if (owned_ranges_ptr) {
            ...
            if (!need_cleanup.empty()) {
                co_await smp::submit_to(coordinator, [&info_vec, need_cleanup = std::move(need_cleanup)] () mutable {
...
future<> run_resharding_jobs(sharded<sstables::sstable_directory>& dir, std::vector<reshard_shard_descriptor> reshard_jobs,
    ...
    co_await dir.invoke_on_all(coroutine::lambda([&] (sstables::sstable_directory& d) -> future<> {
        ...
        auto local_owned_ranges_ptr = [&owned_ranges_ptr] () -> compaction::owned_ranges_ptr {
            if (!owned_ranges_ptr) {
                return nullptr;
            }
            auto owned_ranges = *owned_ranges_ptr;
            return compaction::make_owned_ranges_ptr(std::move(owned_ranges));
        };
        co_await d.reshard(std::move(info_vec), cm, table, max_threshold, creator, local_owned_ranges_ptr());
        co_await d.move_foreign_sstables(dir);
    }));
...
//    assigned.
future<>
distributed_loader::reshard(sharded<sstables::sstable_directory>& dir, sharded<replica::database>& db, sstring ks_name, sstring table_name,
        sstables::compaction_sstable_creator_fn creator, compaction::owned_ranges_ptr owned_ranges_ptr) {
    auto all_jobs = co_await collect_sstables_for_reshard(dir, owned_ranges_ptr);
