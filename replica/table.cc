...
public:
    ...
    void update_effective_replication_map(const locator::effective_replication_map& erm) override {}
    void make_storage_groups() override {
        compaction_group_list list;
        storage_group_vector ret;
        auto cg = std::make_unique<compaction_group>(_t, size_t(0), dht::token_range::make_open_ended_both_sides());
        ret.push_back(std::make_unique<storage_group>(std::move(cg), &list));
        _storage_groups_ptr = make_lw_shared<storage_group_vector>(std::move(ret));
        _compaction_groups = std::move(list);
    }
...
public:
    tablet_storage_group_manager(replica::table& t, const locator::effective_replication_map& erm)
        : _t(t)
    ...
    lw_shared_ptr<sstables::sstable_set> make_sstable_set() const override {
        ...
        return make_tablet_sstable_set(schema(), *this);
    }
};
...
void tablet_storage_group_manager::handle_tablet_split_completion(const locator::tablet_map& old_tmap, const locator::tablet_map& new_tmap) {
    ...
    _storage_groups_ptr = std::move(new_storage_groups_ptr);
}

void tablet_storage_group_manager::update_effective_replication_map(const locator::effective_replication_map& erm) {
...
void table::update_effective_replication_map(locator::effective_replication_map_ptr erm) {
    ...
    if (uses_tablets()) {
        auto old_groups = _sg_manager->storage_group_vector_ptr();

        _sg_manager->update_effective_replication_map(*_erm);

        // Remove old main groups in background, they're unused, but they need to be deregistered properly
        (void) do_with(std::move(old_groups), _async_gate.hold(), [] (const storage_group_vector_ptr& groups, gate::holder&) {
