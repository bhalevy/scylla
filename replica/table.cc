...
table::update_cache(compaction_group& cg, lw_shared_ptr<memtable> m, std::vector<sstables::shared_sstable> ssts) {
    ...
    struct updater : public table_sstables_adder {
        ...
        {
            if (sstables.size() == 1) {
                ms_opt = sstables.front()->as_mutation_source();
            } else {
                std::vector<mutation_source> sources;
                sources.reserve(sstables.size());
                for (const auto& sst : sstables) {
...
compaction_group::update_sstable_lists_on_off_strategy_completion(sstables::compaction_completion_desc desc) {
    ...
    public:
        ...
        virtual future<> prepare() override {
            ...
            _new_backlog_tracker = co_await _cg.get_backlog_tracker().clone_async();
...
future<db::replay_position> table::discard_sstables(db_clock::time_point truncated_at) {
    ...
    struct pruner {
        ...
        future<> prune(compaction_group& cg, db_clock::time_point truncated_at) {
            ...
            if (!st.main.remove.empty()) {
                st.new_backlog_tracker.emplace(co_await cg.get_backlog_tracker().clone_async());
    ...
    struct updater_impl : public row_cache::external_updater_impl {
        ...
        virtual future<> prepare() override {
            for (const compaction_group_ptr& cg : t.compaction_groups()) {
                co_await p.prune(*cg, truncated_at);
            }
        }
