...
// mutation with 16K tablets (eith 9 replicas each) on a
...
tablet_map_to_mutations(const tablet_map& tablets, table_id id, const sstring& keyspace_name, const sstring& table_name,
                       api::timestamp_type ts, std::function<void(mutation)> process_mutation) {
    ...
    for (auto&& tablet : tablets.tablets()) {
        if (++tablets_in_mutation >= min_tablets_in_mutation && seastar::need_preempt()) {
            ...
            process_mutation(std::exchange(m, make_mutation()));
        }
...
future<> save_tablet_metadata(replica::database& db, const tablet_metadata& tm, api::timestamp_type ts) {
    ...
    for (auto&& [id, tablets] : tm.all_tables()) {
        ...
        co_await tablet_map_to_mutations(tablets, id, s->ks_name(), s->cf_name(), ts, [&] (mutation m) {
            muts.emplace_back(freeze(m));
        });
    }
...
future<> read_tablet_mutations(seastar::sharded<replica::database>& db, std::function<void(mutation)> process_mutation) {
    auto s = db::system_keyspace::tablets();
    ...
    for (auto& p: rs->partitions()) {
        co_await p.mut().unfreeze_and_split_gently(s, max_rows, process_mutation);
    }
}
