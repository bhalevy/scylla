...
When allocating tablets for a new table, ScyllaDB first considers the ``initial`` tablets configured for the keyspace.
If the keyspace initial tablets is set to 0, the table's ``tablet_hints`` are considered.
If multiple tablet hints are provided, ScyllaDB uses the maximum tablet count derived by each hint individually.
If no ``tablet_hints`` are provided, ScyllaDB automatically calculates the number of tablets so that each shard
would own at least one tablet replica, scaled up by the ``tablets_initial_scale_factor`` configuration option.
