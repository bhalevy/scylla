...
1. Benign collision can be caused by "replay" of a mutation, e.g. due to client retry, or due to internal processes.
In such case the cells are equivalent and any of them can be selected, arbitrarily.

2. ``TIMESTAMP`` collisions might be normally caused by parallel queries that are served
...
1. Collisions might also happen with user-provided timestamps, if the application does not guarantee
