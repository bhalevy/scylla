...
--seeds SEED
------------
The ``--seeds`` command line option configures ScyllaDB's seed node. If no ``--seeds`` option is specified and the ``--developer-mode=1`` option is used, ScyllaDB uses the localhost address as the seed.
