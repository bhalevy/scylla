...
private:
    ...
    //
    // The called function returns a modified live_and_unreachable_endpoints
    // that is then copied to temporary copies for all shards
    // and then applied atomcically on all shards.
    //
    // The `on_success` callback is called on shard 0 iff replication was successful;
    future<> mutate_live_and_unreachable_endpoints(std::function<live_and_unreachable_endpoints(gossiper&)> func,
