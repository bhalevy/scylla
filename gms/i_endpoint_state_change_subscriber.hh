...
public:
     ...
     */
    virtual future<> on_join(locator::host_id host_id, endpoint_state_ptr ep_state, permit_id) = 0;

    virtual future<> before_change(locator::host_id host_id, endpoint_state_ptr current_state, application_state new_statekey, const versioned_value& newvalue) = 0;

    virtual future<> on_change(locator::host_id host_id, application_state state, const versioned_value& value, permit_id) = 0;

    virtual future<> on_alive(locator::host_id host_id, endpoint_state_ptr state, permit_id) = 0;

    virtual future<> on_dead(locator::host_id host_id, endpoint_state_ptr state, permit_id) = 0;

    virtual future<> on_remove(locator::host_id host_id, permit_id) = 0;
     ...
     */
    virtual future<> on_restart(locator::host_id host_id, endpoint_state_ptr state, permit_id) = 0;
};
