...
public:
    ...
    future<> on_join(locator::host_id, endpoint_state_ptr state, gms::permit_id) override {
        return enable_features();
    }
    future<> on_change(locator::host_id, application_state state, const versioned_value&, gms::permit_id) override {
        if (state == application_state::SUPPORTED_FEATURES) {
            return enable_features();
        }
        return make_ready_future();
    }
    future<> before_change(locator::host_id, endpoint_state_ptr, application_state, const versioned_value&) override { return make_ready_future(); }
    future<> on_alive(locator::host_id, endpoint_state_ptr, gms::permit_id) override { return make_ready_future(); }
    future<> on_dead(locator::host_id, endpoint_state_ptr, gms::permit_id) override { return make_ready_future(); }
    future<> on_remove(locator::host_id, gms::permit_id) override { return make_ready_future(); }
    future<> on_restart(locator::host_id, endpoint_state_ptr, gms::permit_id) override { return make_ready_future(); }
