...
future<> gossiper::do_apply_state_locally(gms::inet_address node, const endpoint_state& remote_state, bool listener_notification) {
    ...
    if (es) {
        ...
        if (remote_generation > generation_type(utils::get_generation_number().value() + MAX_GENERATION_DIFFERENCE)) {
...
future<> gossiper::start_gossiping(utils::generation_type suggested_generation_nbr, std::map<application_state, versioned_value> preload_local_states, gms::advertise_myself advertise) {
    ...
    build_seeds_list();
    gms::generation_type generation_nbr(suggested_generation_nbr.value());
...
future<> gossiper::add_saved_endpoint(inet_address ep) {
    ...
    auto ep_state = endpoint_state(heart_beat_state());
    auto es = get_endpoint_state_for_endpoint_ptr(ep);
    if (es) {
        ep_state = *es;
