...
gossiper::handle_get_endpoint_states_msg(gossip_get_endpoint_states_request request) {
    ...
    _endpoint_state_map.do_for_each([&] (const inet_address& node, const endpoint_state& state) {
...
future<> gossiper::do_apply_state_locally(gms::inet_address addr, endpoint_state remote_state, bool listener_notification) {
    ...
    assert(this_shard_id() == 0);
    auto node = endpoint_id(remote_state.get_host_id(), addr);
    auto permit = co_await _endpoint_state_map.lock_endpoint(node);

    const auto& remote_host_id = node.host_id;
    locator::host_id local_host_id;

    if (!remote_host_id) {
        logger.warn("do_apply_state_locally: node {} sent no host_id in remote state", addr);
    }
    ...
    if (es) {
        ...
        if (remote_generation > generation_type(get_generation_number().value() + MAX_GENERATION_DIFFERENCE)) {
            ...
            if (listener_notification) {
                ...
                _endpoint_state_map.set(node, std::move(remote_state));
            }
        } else if (remote_generation == local_generation) {
            if (listener_notification) {
                // find maximum state
    ...
    if (!remote_host_id) {
        ...
        if (local_host_id) {
            ...
            logger.warn("Node {} host_id is {}", node, remote_host_id);
        }
        map_endpoint(node);
    }
}
...
future<> gossiper::do_status_check() {
    ...
    for (const auto& endpoint : get_endpoints()) {
        ...
        if (is_gossip_only_member(endpoint)
            ...
            co_await evict_from_membership(node); // can get rid of the state immediately
        }
        ...
        if (!is_alive && (now > expire_time)
            ...
            co_await evict_from_membership(node);
        }
    }
...
future<> gossiper::failure_detector_loop() {
    ...
    while (is_enabled() && !_abort_source.abort_requested()) {
        try {
            ...
            auto generation_number = _endpoint_state_map.get_or_create(my_endpoint_id()).get_heart_beat_state().get_generation();
...
future<> gossiper::replicate_live_endpoints_on_change() {
    ...
    if (live_endpoint_changed || unreachable_endpoint_changed) {
        ...
        auto holder = co_await _endpoint_state_map.get_lock().hold_read_lock();
        std::unordered_map<endpoint_id, bool> is_alive_map;
        _endpoint_state_map.do_for_each([&] (const inet_address& addr, const endpoint_state& eps) {
            is_alive_map.emplace(endpoint_id(eps.get_host_id(), addr), eps.is_alive());
        });
        co_await container().invoke_on_others([this, live_endpoint_changed, unreachable_endpoint_changed,
                                   is_alive_map = std::move(is_alive_map)] (gossiper& local_gossiper) {
            if (live_endpoint_changed) {
            ...
            for (const auto& [ep, is_alive] : is_alive_map) {
                local_gossiper._endpoint_state_map.get_or_create(ep).set_alive(is_alive);
            }
        });
    }
}
...
void gossiper::run() {
  ...
  (void)seastar::with_semaphore(_callback_running, 1, [this] {
    return seastar::async([this, g = this->shared_from_this()] {
            ...
            heart_beat_state& hbs = _endpoint_state_map.get_or_create(my_endpoint_id()).get_heart_beat_state();
        ...
        if (logger.is_enabled(logging::log_level::trace)) {
            _endpoint_state_map.do_for_each([] (const inet_address& ep, const endpoint_state& eps) {
...
future<> gossiper::convict(endpoint_id endpoint) {
...
future<> gossiper::evict_from_membership(endpoint_id endpoint) {
    assert(this_shard_id() == 0);
    auto permit = co_await _endpoint_state_map.lock_endpoint(endpoint);
    const auto& addr = endpoint.addr;
    const auto& host_id = endpoint.host_id;
    _unreachable_endpoints.erase(addr);
    co_await container().invoke_on_all([endpoint] (auto& g) {
        g._endpoint_state_map.erase(endpoint);
    });
    // unmap endpoint
    if (auto it = _host_id_to_endpoint_map.find(host_id); it != _host_id_to_endpoint_map.end()) {
        if (it->second == addr) {
            _host_id_to_endpoint_map.erase(it);
        }
    }
    if (auto it = _endpoint_to_host_id_map.find(addr); it != _endpoint_to_host_id_map.end()) {
        if (it->second == host_id) {
            _endpoint_to_host_id_map.erase(it);
        }
    }
...
void gossiper::make_random_gossip_digest(utils::chunked_vector<gossip_digest>& g_digests) {
    ...
    _endpoint_state_map.do_for_each([&endpoints] (const inet_address& ep, const endpoint_state& eps) {
...
future<> gossiper::replicate(endpoint_id ep, const endpoint_state& es) {
    map_endpoint(ep);
    return container().invoke_on_others([ep, es, self = shared_from_this()] (gossiper& g) {
        g.map_endpoint(ep);
        g._endpoint_state_map.get_or_create(ep).add_application_state(es);
    });
}

future<> gossiper::replicate(endpoint_id ep, const std::map<application_state, versioned_value>& src, const utils::chunked_vector<application_state>& changed) {
    map_endpoint(ep);
    return container().invoke_on_others([ep, &src, &changed, self = shared_from_this()] (gossiper& g) {
        g.map_endpoint(ep);
        for (auto&& key : changed) {
            g._endpoint_state_map.get_or_create(ep).add_application_state(key, src.at(key));
        }
    });
}

future<> gossiper::replicate(endpoint_id ep, application_state key, const versioned_value& value) {
    map_endpoint(ep);
    return container().invoke_on_others([ep, key, &value, self = shared_from_this()] (gossiper& g) {
        g.map_endpoint(ep);
        g._endpoint_state_map.get_or_create(ep).add_application_state(key, value);
    });
}
...
future<> gossiper::advertise_token_removed(inet_address endpoint, locator::host_id host_id) {
    ...
    co_await replicate(endpoint_id(eps.get_host_id(), endpoint), eps);
...
future<> gossiper::assassinate_endpoint(sstring address) {
    return container().invoke_on(0, [address] (auto&& gossiper) {
        return seastar::async([&gossiper, g = gossiper.shared_from_this(), address] {
            auto endpoint = g->get_endpoint_id(address);
            ...
            if (es) {
                tokens = gossiper.get_token_metadata_ptr()->get_tokens(address);
                if (tokens.empty()) {
            ...
            gossiper.handle_major_state_change(endpoint, std::move(ep_state)).get();
...
future<> gossiper::reset_endpoint_state_map() {
    assert(this_shard_id() == 0);
    auto holder = co_await _endpoint_state_map.get_lock().hold_write_lock();
...
locator::host_id gossiper::get_host_id(inet_address endpoint) const {
    ...
    if (!_endpoint_to_host_id_map.contains(endpoint)) {
        logger.warn("get_host_id: node {}/{} has no mapped host_id", host_id, endpoint);
    } else if (host_id != _endpoint_to_host_id_map.at(endpoint)) {
        logger.warn("get_host_id: node {}/{} has mismatching mapped host_id{}", host_id, endpoint, _endpoint_to_host_id_map.at(endpoint));
    }
    return host_id;
}
...
inet_address gossiper::get_address(const locator::host_id& host_id) const {
    if (auto it = _host_id_to_endpoint_map.find(host_id); it != _host_id_to_endpoint_map.end()) {
        ...
        if (addr == inet_address()) {
            on_internal_error(logger, format("address for {} is null", host_id));
        }
        return addr;
    }
    if (is_me(host_id)) {
        return get_broadcast_address();
    }
    return inet_address();
}
...
std::set<gms::inet_address> gossiper::get_nodes_with_host_id(locator::host_id host_id) const {
    ...
    _endpoint_state_map.do_for_each([&nodes, host_id] (const inet_address& node, const endpoint_state& eps) {
...
void gossiper::mark_alive(const endpoint_id& node, endpoint_state& local_state) {
    ...
    auto generation = _endpoint_state_map.get_or_create(my_endpoint_id()).get_heart_beat_state().get_generation();
    logger.debug("Sending a EchoMessage to {}, with generation_number={}", id, generation);
    // Do it in the background.
    (void)_messaging.send_gossip_echo(id, generation.value(), std::chrono::milliseconds(15000)).then([this, addr = node] {
...
future<> gossiper::real_mark_alive(endpoint_id node, endpoint_state& local_state) {
    const auto& addr = node.addr;
    logger.trace("marking as alive {}", node);
    ...
    co_await _subscribers.for_each([node, state] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) -> future<> {
        co_await subscriber->on_alive(node, state);
        logger.trace("Notified {}: is_alive node={}", fmt::ptr(subscriber.get()), node);
    });
}

future<> gossiper::mark_dead(endpoint_id node, endpoint_state& local_state) {
    const auto& addr = node.addr;
    ...
    logger.info("InetAddress {} is now DOWN, status = {}", node, get_gossip_status(state));
    co_await _subscribers.for_each([node, state] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) -> future<> {
        co_await subscriber->on_dead(node, state);
        logger.trace("Notified {} mark_dead node={}", fmt::ptr(subscriber.get()), node);
    });
}

future<> gossiper::handle_major_state_change(endpoint_id node, endpoint_state eps) {
    std::optional<endpoint_state> eps_old;
    if (auto* p = get_endpoint_state_for_endpoint_ptr(node); p) {
        eps_old = *p;
    }

    if (!is_dead_state(eps) && !is_in_shadow_round()) {
        if (_endpoint_state_map.contains(node))  {
    ...
    const auto& new_eps = _endpoint_state_map.set(node, std::move(eps));
    co_await replicate(node, new_eps);
    ...
    if (eps_old) {
        ...
        co_await _subscribers.for_each([node, eps_old] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
            return subscriber->on_restart(node, *eps_old);
        });
    }

    auto& ep_state = _endpoint_state_map.at(node);
    if (!is_dead_state(ep_state)) {
        mark_alive(node, ep_state);
    } else {
        logger.debug("Not marking {} alive due to dead state {}", node, get_gossip_status(eps));
        co_await mark_dead(node, ep_state);
    }

    auto* eps_new = get_endpoint_state_for_endpoint_ptr(node);
    if (eps_new) {
        co_await _subscribers.for_each([node, eps_new] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
            return subscriber->on_join(node, *eps_new);
        });
    }
    // check this at the end so nodes will learn about the endpoint
    if (is_shutdown(node)) {
        co_await mark_as_shutdown(node);
    }
}
...
future<> gossiper::apply_new_states(endpoint_id node, endpoint_state& local_state, const endpoint_state& remote_state) {
    ...
    co_await replicate(node, remote_map, changed);
    ...
    for (auto&& key : changed) {
        co_await do_on_change_notifications(node, key, remote_map.at(key));
    }
...
void gossiper::examine_gossiper(utils::chunked_vector<gossip_digest>& g_digest_list,
    ...
    if (g_digest_list.size() == 0) {
        ...
        _endpoint_state_map.do_for_each([&g_digest_list] (const inet_address& ep, const endpoint_state& eps) {
            g_digest_list.emplace_back(ep);
        });
    }
...
future<> gossiper::start_gossiping(gms::generation_type generation_nbr, std::map<application_state, versioned_value> preload_local_states, gms::advertise_myself advertise) {
    ...
    endpoint_state& local_state = _endpoint_state_map.get_or_create(my_endpoint_id());
    ...
    co_await replicate(my_endpoint_id(), local_state);
...
future<> gossiper::add_saved_endpoint(inet_address ep, locator::host_id host_id) {
    ...
    ep_state.mark_dead();
    const auto& new_ep_state = _endpoint_state_map.set(node, std::move(ep_state));
    map_endpoint(node);
    co_await replicate(node, new_ep_state);
    _unreachable_endpoints[ep] = now();
    logger.info("Adding saved endpoint {}: generation={}", node, ep_state.get_heart_beat_state().get_generation());
}
    ...
    return container().invoke_on(0, [states = std::move(states)] (gossiper& gossiper) mutable {
        return seastar::async([g = gossiper.shared_from_this(), states = std::move(states)]() mutable {
            ...
            for (auto& p : states) {
                ...
                gossiper.replicate(endpoint, state, value).get();
...
void gossiper::dump_endpoint_state_map() {
    ...
    _endpoint_state_map.do_for_each([] (const inet_address& ep, const endpoint_state& eps) {
        logger.info("endpoint={}, endpoint_state={}", ep, eps);
    });
 ...
 */
future<> gossiper::mark_as_shutdown(endpoint_id endpoint) {
    auto es = get_endpoint_state_for_endpoint_ptr(endpoint);
    if (es) {
        auto& ep_state = *es;
...
bool gossiper::is_safe_for_bootstrap() {
...
bool gossiper::is_safe_for_restart() {
    ...
    bool allowed = _endpoint_state_map.do_for_each([&] (const inet_address& node, const endpoint_state& eps) {
        try {
            auto status = get_gossip_status(eps);
...
std::set<sstring> gossiper::get_supported_features(const versioned_value* app_state) const {
    if (!app_state) {
        return {};
    }
...
std::set<sstring> gossiper::get_supported_features(const endpoint_state* eps) const {
    if (!eps) {
        return {};
    }
    return get_supported_features(eps->get_application_state_ptr(application_state::SUPPORTED_FEATURES));
}

std::set<sstring> gossiper::get_supported_features(inet_address endpoint) const {
    return get_supported_features(get_endpoint_state_for_endpoint_ptr(endpoint));
}
...
std::set<sstring> gossiper::get_supported_features(const std::unordered_map<gms::inet_address, sstring>& loaded_peer_features, ignore_features_of_local_node ignore_local_node) const {
    ...
    _endpoint_state_map.do_for_each([&] (const inet_address& endpoint, const endpoint_state& eps) {
        auto features = get_supported_features(&eps);
...
void gossiper::check_snitch_name_matches(sstring local_snitch_name) const {
    _endpoint_state_map.do_for_each([&] (const inet_address& address, const endpoint_state& state) {
