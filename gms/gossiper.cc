...
future<> gossiper::handle_syn_msg(const rpc::client_info& cinfo, gossip_digest_syn syn_msg) {
    auto from = co_await get_endpoint_id(cinfo);
    logger.debug("handle_syn_msg():from={},cluster_name:peer={},local={},group0_id:peer={},local={},partitioner_name:peer={},local={}",
...
future<> gossiper::handle_ack_msg(const rpc::client_info& cinfo, gossip_digest_ack ack_msg) {
    auto from = co_await get_endpoint_id(cinfo);
    logger.debug("handle_ack_msg():from={},msg={}", from, ack_msg);
...
future<> gossiper::handle_ack2_msg(const rpc::client_info& cinfo, gossip_digest_ack2 msg) {
    auto from = co_await get_endpoint_id(cinfo);
    logger.debug("handle_ack2_msg():from={},msg={}", from, msg);
    if (!is_enabled()) {
        co_return;
    }
...
future<> gossiper::do_apply_state_locally(gms::inet_address addr, endpoint_state remote_state, bool listener_notification) {
    auto remote_addr = remote_state.get_address();
    if (addr != remote_addr && remote_addr != inet_address()) {
        logger.warn("Endpoint address {} mismatches address in remote endpoint_state: {}. Ignoring.", addr, remote_addr);
        co_return;
    }
        ...
        if (!local_host_id) {
            ...
            if (remote_host_id) {
                if (local_host_id != remote_host_id) {
                    logger.warn("Node {} changed host_id from {} to {}", addr, local_host_id, remote_host_id);
...
future<> gossiper::force_remove_endpoint(inet_address endpoint, permit_id pid) {
    ...
    return container().invoke_on(0, [node = get_endpoint_id(endpoint), pid] (auto& gossiper) mutable -> future<> {
        ...
        pid = permit.id();
        try {
            co_await gossiper.remove_endpoint(node.addr, pid);
...
future<> gossiper::remove_endpoint(endpoint_id node, permit_id pid) {
    ...
    try {
        co_await _subscribers.for_each([node, pid] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
            return subscriber->on_remove(node.host_id, pid);
        });
    } catch (...) {
...
future<> gossiper::do_status_check() {
    ...
    for (const auto& endpoint : get_endpoints()) {
        ...
        if (is_gossip_only_member(node)
            ...
            co_await remove_endpoint(node.addr, pid); // will put it in _just_removed_endpoints to respect quarantine delay
...
future<> gossiper::evict_from_membership(endpoint_id node, permit_id pid) {
    ...
    co_await erase_address_mapping(node.host_id, addr);
...
future<> gossiper::advertise_token_removed(inet_address endpoint, locator::host_id host_id, permit_id pid) {
    ...
    co_await replicate(endpoint_id(eps.get_host_id(), endpoint), std::move(eps), pid);
...
future<> gossiper::assassinate_endpoint(sstring address) {
    co_await container().invoke_on(0, [&] (auto&& gossiper) -> future<> {
        ...
        auto node = gossiper.get_endpoint_id(endpoint);
        ...
        if (es) {
            ...
            if (tokens.empty()) {
                logger.warn("Unable to calculate tokens for {}.  Will use a random one", node);
            ...
            if (!es) {
                logger.warn("Endpoint {} disappeared while trying to assassinate, continuing anyway", node);
            } else {
                auto& new_state = *es;
                if (new_state.get_heart_beat_state().get_generation() != generation) {
                    throw std::runtime_error(format("Endpoint still alive: {} generation changed while trying to assassinate it", node));
                } else if (new_state.get_heart_beat_state().get_heart_beat_version() != heartbeat) {
                    throw std::runtime_error(format("Endpoint still alive: {} heartbeat changed while trying to assassinate it", node));
                }
            }
...
bool gossiper::is_gossip_only_member(inet_address endpoint, endpoint_state_ptr es) const {
    if (!es) {
        return false;
    }
...
endpoint_state_ptr gossiper::get_endpoint_state_ptr(locator::host_id host_id) const noexcept {
    if (!host_id) {
        on_internal_error(logger, "get_endpoint_state_ptr called with null host_id");
    }
    return get_endpoint_state_ptr(host_id_address(host_id));
}
...
future<endpoint_id> gossiper::get_endpoint_id(const rpc::client_info& cinfo) noexcept {
    ...
    if (host_id) {
        co_await update_address_map(host_id, addr);
    } else {
        host_id = get_host_id(addr, throw_on_error::no);
    }
    co_return endpoint_id(host_id, addr);
}
...
inet_address gossiper::get_address(locator::host_id host_id, throw_on_error throw_on_error) const {
    auto addr = host_id_address(host_id);
    if (addr == inet_address() && throw_on_error) {
        auto msg = format("Host {} has no inet_address", host_id);
        logger.warn("{}", msg);
        if (throw_on_error) {
            throw std::runtime_error(msg);
        }
...
std::optional<endpoint_state> gossiper::get_state_for_version_bigger_than(endpoint_id node, const endpoint_state& eps, version_type version) const {
    ...
    std::optional<versioned_value> rpc_address_opt;
    for (const auto& [key, value] : eps.get_application_state_map()) {
        if (value.version() > version) {
            ...
            host_id_opt = value;
        } else if (key == application_state::RPC_ADDRESS) {
            rpc_address_opt = value;
        }
    }
    /* Always send the HOST_ID and RPC_ADDRESS states even if their version is not greater than the requested version */
    if (reqd_endpoint_state) {
        if (host_id_opt) {
            reqd_endpoint_state->add_application_state(application_state::HOST_ID, std::move(*host_id_opt));
        }
        if (rpc_address_opt) {
            reqd_endpoint_state->add_application_state(application_state::RPC_ADDRESS, std::move(*rpc_address_opt));
        }
    }
...
bool gossiper::is_shutdown(const endpoint_state& ep_state) const noexcept {
    return get_gossip_status(ep_state) == sstring(versioned_value::SHUTDOWN);
}

bool gossiper::is_normal(const endpoint_state& ep_state) const noexcept {
    return get_gossip_status(ep_state) == sstring(versioned_value::STATUS_NORMAL);
}

bool gossiper::is_left(const endpoint_state& ep_state) const noexcept {
...
future<> gossiper::do_before_change_notifications(endpoint_id node, endpoint_state_ptr ep_state, const application_state& ap_state, const versioned_value& new_value) const {
    co_await _subscribers.for_each([node, ep_state, ap_state, new_value] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
        return subscriber->before_change(node.host_id, ep_state, ap_state, new_value);
    });
}
...
future<> gossiper::do_on_change_notifications(endpoint_id node, const application_state& state, const versioned_value& value, permit_id pid) const {
    co_await _subscribers.for_each([this, node, state, value, pid] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
        ...
        _abort_source.check();
        return subscriber->on_change(node.host_id, state, value, pid);
    });
}
...
future<> gossiper::do_on_dead_notifications(endpoint_id node, endpoint_state_ptr state, permit_id pid) const {
    co_await _subscribers.for_each([node, state = std::move(state), pid] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
        return subscriber->on_dead(node.host_id, state, pid);
    });
}
    ...
    try {
        co_await container().invoke_on(0, [&] (gossiper& gossiper) mutable -> future<> {
            ...
            for (auto& p : states) {
                ...
                co_await gossiper.do_before_change_notifications(node_ops_cmd_request, ep_state_before, state, value);
            }

            auto es = gossiper.get_endpoint_state_ptr(node);
            if (!es) {
                on_internal_error(logger, format("endpoint_state_map does not contain this node {}", node));
            }
...
const versioned_value* gossiper::get_application_state_ptr(endpoint_state_ptr eps, application_state appstate) const noexcept {
    if (!eps) {
        return nullptr;
    }
...
sstring gossiper::get_application_state_value(endpoint_state_ptr eps, application_state appstate) const noexcept {
    if (eps) {
        auto v = eps->get_application_state_ptr(appstate);
        if (v) {
            return v->value();
        }
    }
...
std::string_view gossiper::get_gossip_status(endpoint_state_ptr eps) const noexcept {
    return eps ? do_get_gossip_status(*eps) : gms::versioned_value::STATUS_UNKNOWN;
}
