    ...
    if (es) {
        ...
        if (remote_generation > generation_type(get_generation_number().value() + MAX_GENERATION_DIFFERENCE)) {
            ...
            if (listener_notification) {
                ...
                bool should_mark_alive = !is_alive(node) && !this->is_dead_state(local_state);
                ...
                if (should_mark_alive) { // unless of course, it was dead
...
std::unordered_map<inet_address, endpoint_state_ptr> gms::gossiper::get_endpoint_states() const noexcept {
    return _endpoint_state_map;
}

std::unordered_map<inet_address, endpoint_state_ptr> gms::gossiper::get_live_endpoint_states() const {
    std::unordered_map<inet_address, endpoint_state_ptr> res;
...
future<> gossiper::real_mark_alive(inet_address addr) {
    ...
    const_cast<endpoint_state*>(es.get())->update_timestamp(); // prevents do_status_check from racing us and evicting if it was down > A_VERY_LONG_TIME
