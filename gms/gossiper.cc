...
future<> gossiper::advertise_removing(inet_address endpoint, ::host_id host_id, ::host_id local_host_id) {
...
future<> gossiper::advertise_token_removed(inet_address endpoint, ::host_id host_id) {
    auto& eps = get_endpoint_state(endpoint);
    eps.update_timestamp(); // make sure we don't evict it too soon
    eps.get_heart_beat_state().force_newer_generation_unsafe();
...
host_id gossiper::get_host_id(inet_address endpoint) const {
    ...
    return host_id(utils::UUID(app_state->value));
}
...
bool gossiper::is_safe_for_restart(inet_address endpoint, ::host_id host_id) {
