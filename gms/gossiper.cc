...
future<> gossiper::apply_new_states(inet_address addr, endpoint_state local_state, const endpoint_state& remote_state, permit_id pid) {
    ...
    application_states_map changed;
...
future<> gossiper::do_on_change_notifications(inet_address addr, const gms::application_states_map& states, permit_id pid) const {
...
future<> gossiper::start_gossiping(gms::generation_type generation_nbr, application_states_map preload_local_states, gms::advertise_myself advertise) {
...
future<> gossiper::add_local_application_state(std::initializer_list<std::pair<application_state, utils::in<versioned_value>>> args) {
    ...
    return add_local_application_state(boost::copy_range<application_states_map>(args | boost::adaptors::transformed([](const in_pair_type& p) {
...
//
future<> gossiper::add_local_application_state(application_states_map states) {
    if (states.empty()) {
        co_return;
    }
    try {
