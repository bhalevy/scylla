...
future<> gossiper::add_saved_endpoint(inet_address ep) {
    ...
    if (es) {
        if (es->get_heart_beat_state().get_generation()) {
            // This will abort if configured to abort on internal error (in testing)
            // Otherwise, it prints an error and returns, doing nothing (in production).
            on_internal_error_noexcept(logger, format("Attempted to add saved endpoint after endpoint_state was already established with gossip: {}", es->get_heart_beat_state()));
            co_return;
        }
        ep_state = *es;
