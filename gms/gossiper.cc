...
bool gossiper::endpoint_permit::release() noexcept {
    if (auto permit = std::move(_permit)) {
        ...
        return true;
    } else if (auto pid = std::move(_permit_id)) {
...
future<gossiper::endpoint_permit> gossiper::lock_endpoint(inet_address ep, permit_id pid, seastar::compat::source_location l) {
    ...
    if (pid) {
        if (eptr->pid == pid) {
            ...
            co_return endpoint_permit(std::move(eptr), semaphore_units<>{}, pid, std::move(ep), std::move(caller));
        } else {
    ...
    eptr->pid = permit_id::create_random_id();
...
future<> gossiper::real_mark_alive(inet_address addr) {
    auto permit = co_await lock_endpoint(addr);
    ...
    co_await _subscribers.for_each([&] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) -> future<> {
        co_await subscriber->on_alive(addr, local_state, permit.id());
...
future<> gossiper::start_gossiping(gms::generation_type generation_nbr, std::map<application_state, versioned_value> preload_local_states, gms::advertise_myself advertise) {
    auto permit = co_await lock_endpoint(get_broadcast_address());
...
future<> gossiper::add_saved_endpoint(inet_address ep) {
    ...
    auto permit = co_await lock_endpoint(ep);
...
future<> gossiper::add_local_application_state(std::list<std::pair<application_state, versioned_value>> states) {
    ...
    return container().invoke_on(0, [states = std::move(states)] (gossiper& gossiper) mutable {
        return seastar::async([g = gossiper.shared_from_this(), states = std::move(states)]() mutable {
            ...
            // As we don't guarantee that the on_change notifications
            // will be called in any particular order anyhow.
