...
{
    fmr_logger.debug("mutation_fragment_stream_validator: will validate {} monotonicity.", compare_keys ? "keys" : "only partition regions");
}

mutation_fragment_stream_validator::~mutation_fragment_stream_validator() {
    if (_prev_kind != mutation_fragment::kind::partition_end) {
        on_internal_error(fmr_logger, format("Stream ended with unclosed partition: {}", _prev_kind));
    }
}
...
bool mutation_fragment_stream_validator::operator()(const mutation_fragment& mv) {
    ...
    fmr_logger.debug("[validator {}] {}:{}", this, kind, pos);
    ...
    if (__builtin_expect(!valid, false)) {
        auto fmt = "Unexpected mutation fragment: previous {}:{}, current {}:{}";
        sstring msg = _compare_keys ?
                format(fmt, _prev_kind, _prev_pos, kind, pos) :
                format(fmt, _prev_kind, _prev_region, kind, pos);
