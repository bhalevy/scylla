...
class RpcVerb(ASTBase):
    ...
    - [[with_timeout]] - an additional time_point parameter is supplied
      ...
      Incompatible with [[abortable]].
    - [[cancellable]] - an additional rpc::cancellable& parameter is supplied
      to the handler function and send* method uses send_message_*_cancellable
      variant of internal function to actually send the message.
    - [[abortable]] - an additional abort_source& parameter is supplied
      to the handler function and send* method uses send_message_*_abortable
      variant of internal function to actually send the message.
    ...
    def __init__(self, name, parameters, return_values, with_client_info, with_timeout, cancellable, abortable, one_way, ip):
        super().__init__(name)
    ...
    def __str__(self):
        return f"<RpcVerb(name={self.name}, params={self.params}, return_values={self.return_values}, with_client_info={self.with_client_info}, with_timeout={self.with_timeout}, cancellable={self.cancellable}, abortable={self.abortable}, one_way={self.one_way}, ip={self.ip})>"
    ...
    def send_function_signature_params_list(self, include_placeholder_names, dst_type):
        ...
        if self.cancellable:
            res += ', seastar::rpc::cancellable& cancel'
        if self.abortable:
            res += ', abort_source& as'
        if self.params:
...
def rpc_verb_parse_action(tokens):
    ...
    abortable = not raw_attrs.empty() and 'abortable' in raw_attrs.attr_items
    ...
    if with_timeout and abortable:
        raise Exception(f"Error in verb {name}: [[with_timeout]] cannot be used together with [[abortable]] in the same verb")
    return RpcVerb(name=name, parameters=params, return_values=tokens.get('return_values'), with_client_info=with_client_info, with_timeout=with_timeout, cancellable=cancellable, abortable=abortable, one_way=one_way, ip=ip)
