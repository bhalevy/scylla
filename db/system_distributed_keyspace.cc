...
future<std::unordered_map<host_id, sstring>> system_distributed_keyspace::view_status(sstring ks_name, sstring view_name) const {
    return _qp.execute_internal(
        ...
        return boost::copy_range<std::unordered_map<host_id, sstring>>(*cql_result
                | boost::adaptors::transformed([] (const cql3::untyped_result_set::row& row) {
                    auto host_id = ::host_id(row.get_as<utils::UUID>("host_id"));
