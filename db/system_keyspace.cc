...
future<std::unordered_map<gms::inet_address, host_id>> system_keyspace::load_host_ids() {
    sstring req = format("SELECT peer, host_id FROM system.{}", PEERS);
    return execute_cql(req).then([] (::shared_ptr<cql3::untyped_result_set> cql_result) {
        std::unordered_map<gms::inet_address, host_id> ret;
        for (auto& row : *cql_result) {
            auto peer = gms::inet_address(row.get_as<net::inet_address>("peer"));
            if (row.has("host_id")) {
                ret.emplace(peer, host_id(row.get_as<utils::UUID>("host_id")));
            }
        }
        return ret;
    });
    ...
    future<> execute(std::function<void(mutation)> mutation_sink) override {
        return _ss.get_ownership().then([&, mutation_sink] (std::map<gms::inet_address, float> ownership) {
            ...
            for (auto&& e : _gossiper.get_endpoint_states()) {
                ...
                std::optional<host_id> hostid = tm.get_host_id_if_known(endpoint);
                if (hostid) {
...
future<host_id> system_keyspace::load_local_host_id() {
    ...
    if (msg->empty() || !msg->one().has("host_id")) {
        co_return co_await set_local_host_id(host_id::create_random_id());
    } else {
        auto host_id = ::host_id(msg->one().get_as<utils::UUID>("host_id"));
        slogger.info("Loaded local host id: {}", host_id);
        co_return host_id;
    }
}

future<host_id> system_keyspace::set_local_host_id(host_id host_id) {
