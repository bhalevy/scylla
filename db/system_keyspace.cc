...
future<utils::generation_type> system_keyspace::increment_and_get_generation() {
    ...
    utils::generation_type generation;
    if (rs->empty() || !rs->one().has("gossip_generation")) {
        ...
        utils::generation_type stored_generation(rs->one().template get_as<int>("gossip_generation") + 1);
        utils::generation_type now = utils::get_generation_number();
