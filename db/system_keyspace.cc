...
future<replay_positions> system_keyspace::get_truncated_positions(table_id cf_id) {
    ...
    auto result_set = co_await execute_cql_query(req, std::initializer_list<data_value>({cf_id.uuid()}));
...
future<::shared_ptr<cql3::untyped_result_set>> system_keyspace::execute_cql_query(const sstring& query_string, const Range& values) {
    return _qp.do_execute_internal(query_string, db::consistency_level::ONE, values, cql3::query_processor::cache_internal::yes);
}
