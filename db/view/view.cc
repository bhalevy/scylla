...
get_view_natural_endpoint(
    ...
    auto simple_rack_aware_pairing = rack_aware_pairing && (network_topology->get_replication_factor(my_datacenter) % topology.get_datacenter_racks().at(my_datacenter).size()) == 0;
    ...
    auto resolve = [&] (const locator::topology& topology, const locator::host_id& ep, bool is_view) -> const locator::node* {
        ...
        if (!np) {
            vlogger.error("get_view_natural_endpoint: {} replica {} not found in topology", is_view ? "view" : "base", ep);
        } else if (np->endpoint() == gms::inet_address{}) {
            vlogger.warn("get_view_natural_endpoint: dropping {} replica {} that already left topology", is_view ? "view" : "base", ep);
            return nullptr;
        }
        return np;
    };
    ...
    auto process_candidate = [&] (node_vector& nodes, const locator::topology& topology, const locator::host_id& ep, bool is_view) {
        auto np = resolve(topology, ep, is_view);
        if (np && is_candidate(*np)) {
    ...
    if (use_legacy_self_pairing) {
        for (auto&& view_endpoint : view_erm->get_replicas(view_token)) {
            ...
            if (it != base_endpoints.end()) {
                ...
                auto np = resolve(view_topology, view_endpoint, true);
                if (np && (!network_topology || np->dc() == my_datacenter)) {
                    view_endpoints.push_back(*np);
                }
            }
        }
    } else {
    ...
    if (rack_aware_pairing && !simple_rack_aware_pairing) {
        ...
        // so just stable_sort by rack (and keep ordinal order within the racks)
        std::ranges::stable_sort(base_endpoints, std::ranges::less{}, std::mem_fn(&locator::node::rack));
        std::ranges::stable_sort(view_endpoints, std::ranges::less{}, std::mem_fn(&locator::node::rack));
    }
    ...
    return view_endpoints[idx].get().endpoint();
}
