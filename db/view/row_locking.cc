...
row_locker::lock_ck(const dht::decorated_key& pk, const clustering_key_prefix& cpk, bool exclusive, db::timeout_clock::time_point timeout, stats& stats) {
    ...
    auto ck = cpk;
    // The two-level lock is guaranteed to be kept alive as long as it's locked.
    // Initiating read locking below ensures that even if the two-level lock is currently
