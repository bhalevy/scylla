...
private:
    ...
    uint64_t _collection_items_count_threshold;
    mutable large_data_handler::stats _stats;

public:
    explicit large_data_handler(uint64_t partition_threshold_bytes, uint64_t row_threshold_bytes, uint64_t cell_threshold_bytes, uint64_t rows_count_threshold, uint64_t collection_items_count_threshold);
    ...
    future<bool> maybe_record_large_cells(const sstables::sstable& sst, const sstables::key& partition_key,
            const clustering_key_prefix* clustering_key, const column_definition& cdef, uint64_t cell_size, uint64_t collection_items) {
        assert(running());
        if (__builtin_expect(cell_size > _cell_threshold_bytes || collection_items > _collection_items_count_threshold, false)) {
            return with_sem([&sst, &partition_key, clustering_key, &cdef, cell_size, collection_items, this] {
                return record_large_cells(sst, partition_key, clustering_key, cdef, cell_size, collection_items);
            }).then([] {
                return true;
            });
        }
    ...
    uint64_t get_collection_items_count_threshold() const noexcept {
        return _collection_items_count_threshold;
    }
...
protected:
    virtual future<> record_large_cells(const sstables::sstable& sst, const sstables::key& partition_key,
            const clustering_key_prefix* clustering_key, const column_definition& cdef, uint64_t cell_size, uint64_t collection_items) const = 0;
...
public:
    explicit cql_table_large_data_handler(uint64_t partition_threshold_bytes, uint64_t row_threshold_bytes, uint64_t cell_threshold_bytes, uint64_t rows_count_threshold, uint64_t collection_items_count_threshold)
