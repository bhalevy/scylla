...
future<> store_column_mapping(distributed<service::storage_proxy>& proxy, schema_ptr s, bool with_ttl) {
    ...
    partition_key pk = partition_key::from_exploded(*history_tbl, {uuid_type->decompose(s->id().to_uuid())});
    ...
    for (const auto& cdef : boost::range::join(s->static_columns(), s->regular_columns())) {
        ...
        auto ckey = clustering_key::from_exploded(*history_tbl, {uuid_type->decompose(s->version().to_uuid()),
...
static schema_mutations make_table_mutations(schema_ptr table, api::timestamp_type timestamp, bool with_columns_and_triggers)
{
    ...
    mutation m{s, pkey};
    auto ckey = clustering_key::from_singular(*s, table->cf_name());
    m.set_clustered_cell(ckey, "id", table->id().to_uuid(), timestamp);
...
static schema_mutations make_view_mutations(view_ptr view, api::timestamp_type timestamp, bool with_columns)
{
    ...
    m.set_clustered_cell(ckey, "base_table_id", view->view_info()->base_id().to_uuid(), timestamp);
    ...
    m.set_clustered_cell(ckey, "id", view->id().to_uuid(), timestamp);
...
future<column_mapping> get_column_mapping(::table_id table_id, table_schema_version version) {
    shared_ptr<cql3::untyped_result_set> results = co_await qctx->qp().execute_internal(
        ...
        {table_id.to_uuid(), version.to_uuid()},
