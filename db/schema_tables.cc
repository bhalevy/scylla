...
schema_ptr scylla_tables(schema_features features) {
    ...
    bool has_tablet_options = features.contains(schema_feature::TABLETS);

    schema_ptr& s = schemas[has_in_memory][has_group0_schema_versioning][has_tablet_options];
    if (!s) {
...
mutation make_scylla_tables_mutation(schema_ptr table, api::timestamp_type timestamp) {
    ...
    if (auto tablet_options_cdef = scylla_tables()->get_column_definition("tablets")) {
        if (table->has_tablet_options()) {
            auto& map = table->raw_tablet_options();
            m.set_clustered_cell(ckey, *tablet_options_cdef, make_map_mutation(map, *tablet_options_cdef, timestamp));
        } else {
            m.set_clustered_cell(ckey, *tablet_options_cdef, atomic_cell::make_dead(timestamp, gc_clock::now()));
        }
    }
