    ...
    std::map<utils::UUID, schema_mutations>&& views_after)
{
    ...
    co_await coroutine::parallel_for_each(views_diff.dropped, [&db = proxy.local().get_db().local()] (schema_diff::dropped_schema& dt) {
        auto& s = *dt.schema.get();
        return db.drop_table_on_all(s.ks_name(), s.cf_name(), [&] { return dt.jp.value(); });
    });
    co_await coroutine::parallel_for_each(tables_diff.dropped, [&db = proxy.local().get_db().local()] (schema_diff::dropped_schema& dt) -> future<> {
        auto& s = *dt.schema.get();
        return db.drop_table_on_all(s.ks_name(), s.cf_name(), [&] { return dt.jp.value(); });
    });
