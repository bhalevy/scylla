...
mutation make_scylla_tables_mutation(schema_ptr table, api::timestamp_type timestamp) {
    ...
    if (auto in_memory_cdef = scylla_tables()->get_column_definition("in_memory")) {
        // If IN_MEMORY_TABLES feature is enabled, just delete the unused column
        // so we can safely drop the feature in the future, when it becomes unused by all live releases.
        m.set_clustered_cell(ckey, *in_memory_cdef, atomic_cell::make_dead(timestamp, gc_clock::now()));
    }
    return m;
}
