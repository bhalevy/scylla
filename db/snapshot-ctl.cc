...
snapshot_ctl::get_snapshot_details() {
    static auto details_result_hash = [] (const replica::database::snapshot_details_result& sdr) {
    ...
    using snapshot_acc = std::unordered_set<replica::database::snapshot_details_result, decltype(details_result_hash)>;
    ...
    public:
        future<> operator()(const std::vector<replica::database::snapshot_details_result>& value) {
            for (auto&& vp : value) {
    ...
    co_return co_await run_snapshot_list_operation([this] () -> future<snapshot_map> {
        snapshot_acc collected = co_await _db.map_reduce(snapshot_reducer(), [] (replica::database& db) -> future<std::vector<replica::database::snapshot_details_result>> {
...
future<int64_t> snapshot_ctl::true_snapshots_size() {
    co_return co_await run_snapshot_list_operation([this] () -> future<int64_t> {
        co_return co_await _db.map_reduce(adder<int64_t>(), [] (replica::database& db) -> future<int64_t> {
