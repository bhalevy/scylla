...
future<> large_data_handler::maybe_delete_large_data_entries(sstables::shared_sstable sst) {
    ...
    if (sst->get_large_data_count(sstables::large_data_type::large_partitions).value_or(data_size > _partition_threshold_bytes)) {
    ...
    if (sst->get_large_data_count(sstables::large_data_type::large_rows).value_or(data_size > _row_threshold_bytes)) {
    ...
    if (sst->get_large_data_count(sstables::large_data_type::large_cells).value_or(data_size > _cell_threshold_bytes)) {
