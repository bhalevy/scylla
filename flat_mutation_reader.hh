...
public:
    ...
    struct trivially_abortable_impl : public impl {
        trivially_abortable_impl(schema_ptr s, reader_permit permit, sstring description) : impl(std::move(s), std::move(permit), std::move(description)) { }
...
public:
    ...
    future<> abort(std::exception_ptr ex) noexcept {
        return _impl ? _impl->abort(std::move(ex)) : make_ready_future<>();
    }
...
protected:
    // underlying reader moved to delegating_reader
    // is now owned by it.
    delegating_reader(flat_mutation_reader&& r)
            : impl(r.schema(), r.permit(), format("delegating_reader({})", r.description()))
            , _underlying_holder(std::move(r))
            , _underlying(&*_underlying_holder)
    { }
public:
    // underlying reader passed by reference, owned by caller.
    delegating_reader(flat_mutation_reader& r)
            : impl(r.schema(), r.permit(), format("delegating_reader(&{})", r.description()))
            , _underlying_holder()
            , _underlying(&r)
...
template <typename Func>
requires requires (Func f, flat_mutation_reader& fmr) { f(fmr); } && std::is_nothrow_move_constructible_v<Func>
inline auto with_flat_mutation_reader_in_thread(flat_mutation_reader fmr, Func func) {
    auto close_fmr = defer([&fmr] { fmr.close().get(); });
    return func(fmr);
}
