...
netw::msg_addr messaging_service::get_source(const rpc::client_info& cinfo) {
    ...
    if (auto* host_id_ptr = cinfo.retrieve_auxiliary_opt<locator::host_id>("host_id")) {
        return netw::msg_addr(std::move(addr), std::move(cpu_id), *host_id_ptr);
    } else {
        return netw::msg_addr(std::move(addr), std::move(cpu_id));
    }
}
...
void messaging_service::register_gossip_shutdown(std::function<rpc::no_wait_type (const rpc::client_info& cinfo, inet_address, rpc::optional<int64_t> generation_number)>&& func) {
