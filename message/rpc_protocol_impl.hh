...
template <typename MsgIn, typename Timeout, typename... MsgOut>
auto send_message_cancellable(messaging_service* ms, messaging_verb verb, std::optional<locator::host_id> host_id, msg_addr id, std::optional<Timeout> timeout, seastar::rpc::cancellable& cancel, MsgOut&&... msg) {
    auto rpc_handler = ms->rpc()->make_client<MsgIn(MsgOut...)>(verb);
    using futurator = futurize<std::invoke_result_t<decltype(rpc_handler), rpc_protocol::client&, MsgOut...>>;
    if (ms->is_shutting_down()) {
        return futurator::make_exception_future(rpc::closed_error());
    ...
    return rpc_handler.send(rpc_client, timeout, &cancel, std::forward<MsgOut>(msg)...).handle_exception([ms = ms->shared_from_this(), id, host_id, verb, rpc_client_ptr = std::move(rpc_client_ptr)] (std::exception_ptr&& eptr) {
        ms->increment_dropped_messages(verb);
        if (try_catch<rpc::closed_error>(eptr)) {
            // This is a transport error
            if (host_id) {
...
template <typename MsgIn, typename... MsgOut>
auto send_message_cancellable(messaging_service* ms, messaging_verb verb, locator::host_id id, seastar::rpc::cancellable& cancel, MsgOut&&... msg) {
    return send_message_cancellable<MsgIn, MsgOut...>(ms, verb, std::optional{id}, ms->addr_for_host_id(id), std::nullopt, cancel, std::forward<MsgOut>(msg)...);
}

template <typename MsgIn, typename Timeout, typename... MsgOut>
auto send_message_timeout_cancellable(messaging_service* ms, messaging_verb verb, locator::host_id id, Timeout timeout, seastar::rpc::cancellable& cancel, MsgOut&&... msg) {
    return send_message_cancellable<MsgIn, Timeout, MsgOut...>(ms, verb, std::optional{id}, ms->addr_for_host_id(id), timeout, cancel, std::forward<MsgOut>(msg)...);
}
