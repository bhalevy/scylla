...
keyspace::keyspace(lw_shared_ptr<keyspace_metadata> metadata, config cfg, locator::registry& locator_registry)
...
future<> database::parse_system_tables(distributed<service::storage_proxy>& proxy) {
    ...
    co_await do_parse_schema_tables(proxy, db::schema_tables::KEYSPACES, [&] (schema_result_value_type &v) -> future<> {
        ...
        co_return co_await create_keyspace(ksm, proxy.local().get_locator_registry(), true /* bootstrap. do not mark populated yet */, system_keyspace::no);
    });
...
future<> database::create_in_memory_keyspace(const lw_shared_ptr<keyspace_metadata>& ksm, locator::registry& locator_registry, system_keyspace system) {
...
future<>
database::create_keyspace(const lw_shared_ptr<keyspace_metadata>& ksm, locator::registry& locator_registry) {
    return create_keyspace(ksm, locator_registry, false, system_keyspace::no);
}

future<>
database::create_keyspace(const lw_shared_ptr<keyspace_metadata>& ksm, locator::registry& locator_registry, bool is_bootstrap, system_keyspace system) {
    if (_keyspaces.contains(ksm->name())) {
        co_return;
    }

    co_await create_in_memory_keyspace(ksm, locator_registry, system);
