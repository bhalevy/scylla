...
future<> distributed_loader::cleanup_column_family_sstables(distributed<database>& db, sstring sstdir, sstring ks, sstring cf) {
    ...
    return do_with(std::vector<future<>>(), std::unordered_map<unsigned long, sstable_descriptor>(),
        ...
        return lister::scan_dir(sstdir, { directory_entry_type::regular }, [&db, &verifier, ks = std::move(ks), cf = std::move(cf), &futures] (fs::path sstdir, directory_entry de) {
            sstables::entry_descriptor entry = sstables::entry_descriptor::make_descriptor(sstdir.native(), de.name);
            ...
            return make_ready_future<>();
        }, &column_family::manifest_json_filter).then([&verifier, sstdir, ks = std::move(ks), cf = std::move(cf)] {
            return do_for_each(verifier, [sstdir = std::move(sstdir)] (auto v) {
...
future<> distributed_loader::cleanup_column_family(distributed<database>& db, sstring sstdir, sstring ks, sstring cf) {
    ...
    return async([&db, sstdir = std::move(sstdir), ks = std::move(ks), cf = std::move(cf)] {
        ...
        cleanup_column_family_sstables(db, sstdir, ks, cf).get();
    });
}
