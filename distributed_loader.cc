...
future<> distributed_loader::do_populate_column_family(distributed<database>& db, sstring sstdir, sstring ks, sstring cf) {
    ...
    return do_with(std::vector<future<>>(), std::unordered_map<unsigned long, sstable_descriptor>(), std::move(sstdir), std::move(ks), std::move(cf),
            [&db] (std::vector<future<>>& futures, auto& verifier, sstring& sstdir, sstring& ks, sstring& cf) {
