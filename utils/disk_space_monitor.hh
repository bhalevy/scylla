...
public:
    ...
    struct config {
        ...
        Clock::duration normal_polling_interval = 10s;
        Clock::duration high_polling_interval = 1s;
        // Use high_polling_interval above this threshold
        float polling_interval_threshold = 0.9;
    };
...
public:
    explicit disk_space_monitor(abort_source& as, config cfg)
    ...
    void start(std::filesystem::path data_dir) {
        ...
        return async([this] {
            try {
                while (!_as.abort_requested()) {
                    ...
                    sleep_abortable<Clock>(get_polling_interval(), _as).get();
                }
            } catch (const sleep_aborted&) {
            } catch (const abort_requested_exception&) {
            } catch (...) {
                dlmlog.error("poll loop exited with error: {}", std::current_exception());
            }
        });
    }
