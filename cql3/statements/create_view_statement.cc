...
view_ptr create_view_statement::prepare_view(data_dictionary::database db) const {
    ...
    if (!missing_pk_columns.empty()) {
        auto column_names = utils::join(", ", missing_pk_columns | boost::adaptors::transformed(std::mem_fn(&column_definition::name_as_text)));
    ...
    if (non_pk_restrictions.size() == 1 && has_non_pk_column &&
        ...
        auto column_names = utils::join(", ", non_pk_restrictions | boost::adaptors::map_keys | boost::adaptors::transformed(std::mem_fn(&column_definition::name_as_text)));
