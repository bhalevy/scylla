...
const sstring cf_prop_defs::KW_TABLET_HINTS = "tablet_hints";
...
void cf_prop_defs::validate(const data_dictionary::database db, sstring ks_name, const schema::extensions_map& schema_extensions) const {
    ...
    static std::set<sstring> keywords({
        ...
        KW_SYNCHRONOUS_UPDATES, KW_TABLET_HINTS,
    });
    ...
    auto tablet_hints_map = get_tablet_hints();
    if (!tablet_hints_map.empty()) {
        if (!ks.uses_tablets()) {
            throw exceptions::configuration_exception("tablet_hints cannot be used when tablets are disabled for the keyspace");
        }
        db::tablet_hints::validate(tablet_hints_map);
    }
}
...
db::tablet_hints::map_type cf_prop_defs::get_tablet_hints() const {
    auto tablet_hints = get_map(KW_TABLET_HINTS);
    if (tablet_hints) {
        return tablet_hints.value();
    }
    return db::tablet_hints::map_type{};
}
...
void cf_prop_defs::apply_to_builder(schema_builder& builder, schema::extensions_map schema_extensions, const data_dictionary::database& db, sstring ks_name) const {
    ...
    if (auto tablet_hints_opt = get_map(KW_TABLET_HINTS)) {
        builder.set_tablet_hints(std::move(*tablet_hints_opt));
    }
}
