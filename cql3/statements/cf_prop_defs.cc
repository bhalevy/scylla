...
void cf_prop_defs::validate(const data_dictionary::database db, sstring ks_name, const schema::extensions_map& schema_extensions) const {
    ...
    auto tablet_options_map = get_tablet_options();
    if (!tablet_options_map.empty()) {
        if (!ks.uses_tablets()) {
...
db::tablet_options::map_type cf_prop_defs::get_tablet_options() const {
    auto tablet_options = get_map(KW_TABLETS);
    if (tablet_options) {
        return tablet_options.value();
    }
    return db::tablet_options::map_type{};
}
