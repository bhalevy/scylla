        ...
        , _attrs(std::move(attrs))
{
    ...
    if (_attrs->time_to_live) {
        throw exceptions::invalid_request_exception("TRUNCATE does not support USING TTL");
    }
    if (_attrs->timestamp) {
        throw exceptions::invalid_request_exception("TRUNCATE does not support USING TIMESTAMP");
    }
}
