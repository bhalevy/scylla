...
public:
    virtual future<> on_join(gms::endpoint_id endpoint, endpoint_state ep_state) override { return make_ready_future(); }
    virtual future<> before_change(gms::endpoint_id endpoint, endpoint_state current_state, application_state new_state_key, const versioned_value& new_value) override { return make_ready_future(); }
    virtual future<> on_change(gms::endpoint_id endpoint, application_state state, const versioned_value& value) override { return make_ready_future(); }
    virtual future<> on_alive(gms::endpoint_id endpoint, endpoint_state state) override { return make_ready_future(); }
    virtual future<> on_dead(gms::endpoint_id endpoint, endpoint_state state) override;
    virtual future<> on_remove(gms::endpoint_id endpoint) override;
    virtual future<> on_restart(gms::endpoint_id endpoint, endpoint_state ep_state) override;

private:
    void fail_all_sessions();
    void fail_sessions(const gms::endpoint_id& endpoint);
    bool has_peer(const gms::endpoint_id& endpoint) const;
