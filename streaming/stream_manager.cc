...
future<> stream_manager::on_remove(locator::host_id host_id, gms::permit_id) {
    // FIXME: keep track of session peer as hostid
    auto endpoint = _gossiper.get_address(host_id);
...
future<> stream_manager::on_restart(locator::host_id host_id, endpoint_state_ptr ep_state, gms::permit_id) {
    // FIXME: keep track of session peer as hostid
    auto endpoint = _gossiper.get_address(host_id);
    if (has_peer(endpoint)) {
...
future<> stream_manager::on_dead(locator::host_id host_id, endpoint_state_ptr ep_state, gms::permit_id) {
    // FIXME: keep track of session peer as hostid
    auto endpoint = _gossiper.get_address(host_id);
    if (has_peer(endpoint)) {
