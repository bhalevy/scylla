...
void set_storage_service(http_context& ctx, routes& r, sharded<service::storage_service>& ss) {
    ...
    ss::get_natural_endpoints.set(r, [&ctx] (std::unique_ptr<request> req) {
        return async([&db = ctx.db.local(),
                keyspace = validate_keyspace(ctx, req->param), 
                cf = req->get_query_param("cf"),
                key = req->get_query_param("key")] {
            ...
            auto rs = db.find_keyspace(keyspace).get_replication_strategy().get_natural_endpoints(token, locator::can_yield::yes);
