            ...
            try {
                ...
                if (!n.empty()) {
                    ...
                    if (!ignore_nodes.empty()) {
                        if (hoep.has_host_id() && !ignore_nodes.front().has_host_id()) {
                            throw std::runtime_error("Expected host UUID. Use the same method for all nodes to ignore: either Host IDs or ip addresses.");
                        }
                        if (hoep.has_endpoint() && !ignore_nodes.front().has_endpoint()) {
                            throw std::runtime_error("Expected ip address. Use the same method for all nodes to ignore: either Host IDs or ip addresses.");
                        }
                    }
